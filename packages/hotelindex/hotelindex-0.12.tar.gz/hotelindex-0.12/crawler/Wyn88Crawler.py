...
class Wyn88Crawler:
    def __init__(self):
        ...
        self.sem = asyncio.BoundedSemaphore(50)
    ...
    # def InitRooms(self):
    #     tasks = [self.fetch(k) for k in self.hotelDetais]
    ...
    # async def getHotelList(self,hotelcode):
    #     # 限制同时运行协程数量
    #     with(await self.sem):
    #         urlstr = "http://wap.wyn88.com/Hotel/GetHotelList?pageindex=1&pagesize=500&cityno=" + hotelcode
    #         async with self.session.get(urlstr) as resp:
    #             assert resp.status == 200
    #             resp_json =  await resp.json()
    #             # print(resp_json)
    #             assert resp_json["IsError"] == False
    #             self.hotels+=json.loads(resp_json.get("JsonData"))["Rows"]
