...
class utility:
    ...
    def WYN88RoomPrice_to_mysql(self,retryCount=10,sem=30,days=7,pause=10):
        cr = Wyn88Crawler()
        for _ in range(retryCount):
            time.sleep(pause)
            try:
                roomPrices = cr.getRoomPrice(sem=sem,days=days)
                ...
                # 更新酒店信息
                self.WYN88Hotel_to_mysql(cr.hotelDetais)
                break
