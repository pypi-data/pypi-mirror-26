...
with collab.manager(suppress_exceptions_after_logging_them=False, redirect_stdout_to_log=True) as col:
    ...
    this_program_id, this_program_name, this_program_version = collab.get_own_program_details()
    ...
    for argument_name, input_object in inputs.items():
    ...
    if len(input_numbers) != 2:
        ...
        next_number = input_numbers['last'] + input_numbers['second_to_last']
    ...
    # create a new global tag using a preexisting Collab Symbol (global Tags are just Tags without arguments).
    # the only required argument here is the name of the symbol, which is given in create_tag().

    collab.create_tag("continue-executing-fibonacci-sequence").weight(10).comment('this is a comment')
    ...
    collab.create_tag("last-in-fibonacci-sequence").arguments([output_object])
    if identifier_of_last_input_object is not None:
        collab.create_tag("second-to-last-in-fibonacci-sequence").arguments([identifier_of_last_input_object])
    ...
    # create a hidden message that is only visible in development-mode
    collab.request_to_display_message().add_text("this is a hidden message that's only visible in development mode.").is_only_for_developers()
    ...
    #    collab.request_program_execution(this_program_id, last=output_object_identifier)
    #else:
    #    collab.request_program_execution(this_program_id, second_to_last=identifier_of_last_input_object, last=output_object)
    ...
    if len(inputs) == 0:
            ...
            "arguments" : [
                {
                    ...
                    "filter" : {
                        "type" : "tag",
                        "symbol" : "last-in-fibonacci-sequence",
                    }
                },
                {
                    "name" : "second_to_last",
                    ...
                    "filter" : {
                        "type" : "tag",
                        "symbol" : "second-to-last-in-fibonacci-sequence",
                    }
                },
            ]
        }
        ...
        actions = [
            {
                ...
                "condition" : {
                    ...
                    "reference" : "second_to_last"
                },
                "then_actions" : [{
                    "type" : "execute_program",
                    "program" : "fibonaccilocal",
                    "arguments" : {
                        "second_to_last" : {
                            "nullable" : True,
                            "of" : {
        ...
        option = collab.create_option(trigger_rule, display, actions, description=None)
        collab.create_tag('!set_option_weight').arguments([option]).weight(1.0)
