...
class Superset(BaseSupersetView):
    ...
    def explore_json(self, datasource_type, datasource_id):
        ...
        if payload.get('status') == QueryStatus.FAILED:
            status = 400
        #mutilanguage 
        vizT = ['echarts_bar','echarts_bar_waterfall','echarts_bar_h','echarts_area_stack',
                    ...
                    'echarts_big_number','big_number_viz','echarts_radar_map','echarts_pie_h_g','pivot_table',
                    'table']
        # vizT = []
            ...
            if 'data' in payload:
                if  datasource_type == 'table':
                                ...
                                for key in keys:
                                    for metric in metrics:
                                        if key  == metric.metric_name:
                                            r[str(metric.nick_name)] = pjr[key]
                                    for column in columns:
                                        if key == column.column_name:
                                            r[str(column.nick_name)] = pjr[key]
                                recs.append(r)
                        if len(recs) != 0:
                    ...
                    for column in columns:
                        ...
                        cols.append(col)
                    payload['metrics'] = mets
                    payload['groupby'] = cols            
    ...
    def explore(self, datasource_type, datasource_id):
        ...
        if action == 'overwrite' and not slice_overwrite_perm:
            return json_error_response(__("You don't have the rights to alter this slice"), status=400)
    ...
    def results(self, key):
        ...
        if not blob:
            return json_error_response(
                __("Data could not be retrieved. " +
