...
CleverTabs.prototype.add = function (options) {
    ...
    $(window).on('resize', () => {
        ...
        resizeTimer = setTimeout(() => {
            ...
            var header_height = $('.ui-tabs-nav.ui-helper-reset.ui-helper-clearfix.ui-widget-header.ui-corner-all').outerHeight();
            var dash_height = $('.page-sidebar.navbar-collapse.collapse').outerHeight();
            available_height = available_height - header_height;
            if (available_height < dash_height - header_height + 20) {
                available_height = dash_height - header_height + 20;
            }
            if (self.panelContainer.attr('id').indexOf('cleverTabsPanelContainer') === 0) {
                self.panelContainer.css('height', available_height);
            }
        }, 500);
    });
    //Tab头绑定mouseover事件
    ...
    if (self.panelContainer.attr('id').indexOf('cleverTabsPanelContainer') === 0) {
        ...
        self.panelContainer.css('height', available_height - 27);
        // self.panelContainer.css('height', /*Math.floor(height / self.wrapper.height() * 100) + '%'*/ '95%');
        self.panelContainer.css('padding', '0px');
    }
