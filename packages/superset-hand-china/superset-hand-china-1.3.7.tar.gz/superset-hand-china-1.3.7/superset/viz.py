...
class BaseViz(object):
    ...
    def get_df(self, query_obj=None):
        ...
        form_datas = query_obj['form_data']
        viz_types = form_datas['viz_type']
        ...
        if df is None or df.empty:
            if viz_types != 'filter_box_tree' and viz_types != 'filter_box':
    ...
    def get_payload(self, force=False):
        ...
        if not payload:
            ...
            payload = {
                ...
                'json_data': self.get_json_data(df),
...
class PivotTableViz(BaseViz):
    ...
    def get_data(self, df):
        ...
        from superset import db
        datasource_id = self.form_data.get('datasource').partition('__')[0]
        ...
        group = []
        column ={}
        for g in self.form_data.get('groupby'):
            for c in columns:
                if c.column_name == g:
                    group.append(c.nick_name)
                    column[g] =c.nick_name
        colu = []
        for col in self.form_data.get('columns'):
            for c in columns:
                if c.column_name == col:
                    colu.append(c.nick_name)
                    column[col] = c.nick_name
        metr = []
        for met in self.form_data.get('metrics'):
            for m in metrics:
                if m.metric_name == met:
                    metr.append(m.nick_name)
                    column[met] = m.nick_name
        df.rename(inplace = True, columns=column)
        df = df.pivot_table(
            index=group,
            columns=colu,
            values=metr,
