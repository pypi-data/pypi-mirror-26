...
def get_ids(item: Item):
    return {'assetid': item.id, 'classid': item.classid, 'instanceid': item.instanceid, 'amount': item.amount}
...
class TradeOffer:
    ...
    def _load(self, data, desc, manager, parse_items):
        ...
        if not parse_items:
            self.items_to_give = data.get('items_to_give')
            self.items_to_receive = data.get('items_to_receive')
            return
    ...
    async def ship(self, counter='', token=''):
        ...
        our_offer = {
            ...
            "version": 4,
            "me": {
                "assets": [get_ids(item) for item in self.items_to_give],
                "currency": [],
                "ready": False
            },
            "them": {
                "assets": [get_ids(item) for item in self.items_to_receive],
                "currency": [],
                "ready": False
            }}
        ...
        headers = {'Referer': SteamUrls.Community.value + '/tradeoffer/new/?partner=' + str(self.steamid_other.accountid),
