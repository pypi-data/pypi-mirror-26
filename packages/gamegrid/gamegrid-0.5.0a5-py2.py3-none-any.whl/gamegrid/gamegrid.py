"""This is the GameGrid Python Class that sends messages to the client
class to render in javascript"""

import ipywidgets as widgets
from traitlets import Unicode, validate, List, Tuple, Int, All, Bool
import numpy as np
...
class GameGrid(widgets.DOMWidget):
    ...
    _overlay_text = Unicode('').tag(sync=True)
    ...
    def on_click(self, callback, remove=False):
        self._click_handlers.register_callback(
            lambda s, a: callback(self, a['row'], a['col']), remove=remove)
    ...
    def is_overlay_on(self):
        return self._is_overlay_on

    @is_overlay_on.setter
    def is_overlay_on(self, is_overlay_on):
        self._is_overlay_on = is_overlay_on
