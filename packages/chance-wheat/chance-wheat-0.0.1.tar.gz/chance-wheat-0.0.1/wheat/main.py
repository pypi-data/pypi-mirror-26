...
from wheat.constant import PACKAGE_VERSION, CONF_RULES, PACKAGES
from wheat.utils import directories_to_create, files_to_generate
...
def main():
    ...
    generate_files(files, output_dir, project_name, author_name, packages)
    ...
    for package in packages:
        try:
            ...
            install_status = pip.main([
                'install', '-i',
                'https://pypi.douban.com/simple', '-U', '--user', package
            ])
...
def generate_files(files, output_dir, project_name, author_name, packages):
    ...
    for f in files:
        for key, value in match_rule.iteritems():
            if key not in f:
                continue
            template_name = value
