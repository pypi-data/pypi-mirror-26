...
def quandlPrice(tics=[], mkt='US', year=3,
                begdate='', enddate='', attribute='simple_ret', APIkey=''):
    ...
    while True:
        try:
            ...
            if (mkt == 'US' or mkt == 'us'):
                ...
                column_dict = {'unadj_close': '1',
                               'simple_ret': '3'}
                df = quandl.get(['HKEX/%s.%s' % (x, column_dict[attribute]) for x in tics],
                                start_date=begdate, end_date=enddate)
                df.columns = [['%s.HK' % x for x in tics]]
                if attribute=='simple_ret':
