...
class operator:
    class Utils:
        ...
        def broadcast_attr(self, l, r):
            if operator.is_tensor(l):
                lattr = l.attr
                lndim = l.ndim
            else:
                lattr = [None]
                lndim = 0
            if operator.is_tensor(r):
                rattr = r.attr
                rndim = r.ndim
            else:
                rattr = [None]
    ...
    class Reduction:
        ...
        def nonzero(self, t, keepdims=False):
            o = Backend.operator.reduction.nonzero(t.graph, keepdims=keepdims)
        ...
        def getitem(self, t, key):
            def check(t, k):
                ...
                if has_none:
                    ...
                    if n > t.ndim:
                        raise AssertionError('Too Much Dim!')

            def parse_step(s):
                if s is None:
                    return s
                elif isinstance(s, slice):
                    if operator.is_tensor(s.start):
                        s.start = operator.as_graph(s.start)
                    if operator.is_tensor(s.stop):
                        s.stop = operator.as_graph(s.stop)
                    if operator.is_tensor(s.step):
                        s.step = operator.as_graph(s.step)
                    return s
                elif operator.is_tensor(s):
                    return operator.as_graph(s)
            ...
            if not isinstance(key, tuple):
                ...
                check(t, key)
                gkey = tuple([parse_step(k) for k in key])
            ...
            for k in lkey:
                if k is not None:
                    if operator.is_tensor(k):
                        ...
                        if operator.is_tensor(k.start) or operator.is_tensor(k.stop) or operator.is_tensor(k.step):
                            length = None
                        else:
                            start = k.start
                            stop = k.stop
    ...
    class Alloc:
        ...
        def arange(self, *range, **kwargs):
            ...
            if len(range) == 1:
                ...
                step = range[2]
            if operator.is_tensor(start) or operator.is_tensor(end) or operator.is_tensor(step):
                length = None
            else:
    ...
    class Nnet:
        ...
        def conv(self, input, filters, mode='valid', pad=None, stride=None,
                 dilation=None):
            '''
            ...
            :param mode: 'normal','autopad','same','pad'
            ...
            outchannel = filters_size[0]
            if pad is None:
                pad = tuple([0] * (input.ndim - 2))
            if mode is 'valid':
                outimagesize = [int(np.ceil((a - (f + 2 * (d - 1)) + p * 2 + 1) / float(s))) for a, f, s, p, d in
                                zip(size[2:], filters_size[2:], stride, pad, dilation)]
            elif mode is 'autopad':
                outimagesize = [int(np.ceil((a + (f + 2 * (d - 1)) + p * 2 - 1) / float(s))) for a, f, s, p, d in
                                zip(size[2:], filters_size[2:], stride, pad, dilation)]
            elif mode is 'same':
                outimagesize = [int(np.ceil(a / s)) for a, s in zip(size[2:], stride)]
            else:
                outimagesize = [int(np.ceil((a - (f + 2 * (d - 1)) + p * 2 + 1) / float(s))) for a, f, s, p, d in
                                zip(size[2:], filters_size[2:], stride, pad, dilation)]
            size = [size[0], outchannel] + outimagesize
            o = Backend.operator.conv(input=operator.utils.as_graph(input),
                                      filters=operator.utils.as_graph(filters),
                                      input_shape=size, filter_shape=filters_size,
                                      mode=mode, pad=pad, stride=stride, dilation=dilation)
            return tensor(graph=o, size=size, attr=attr)
        ...
        def pool(self, input, window, mode='max', stride=None, pad=None, autopad=False):
            ...
            if pad is None:
                pad = tuple([0] * (input.ndim - 2))
            o = Backend.operator.pool(input=operator.utils.as_graph(input), window=window, mode=mode, stride=stride,
                                      pad=pad,
                                      autopad=autopad)
            if pad is None:
                pad = tuple([0] * (input.ndim - 2))
            if autopad:
                outimagesize = [int(np.ceil(a / float(s))) for a, s in zip(input.size[2:], stride)]
            else:
                outimagesize = [int(np.ceil((a - w + p * 2 + 1) / float(s))) for a, w, s, p in
