...
def eoldet(file: 'bytes') -> '_coconut.typing.Optional[bytes]':  # line 151
    ...
    if lf > cr:  # Linux/Unix  # line 160
        return b"\n"  # Linux/Unix  # line 160
    if cr > lf:  # older 8-bit machines  # line 161
        return b"\r"  # older 8-bit machines  # line 161
    return None  # no new line contained, cannot determine  # line 162

def usage() -> 'None':  # line 164
    print("""/// {appname}
...
Usage: {cmd} <command> [<argument>] [<option1>, ...]        For command "offline" and when in offline mode
  ...
  Common options:
    ...
    --{cmd}                       When executing {cmd.upper()} not being offline, pass arguments to {cmd.upper() instead (e.g. {cmd} --{cmd} config set key value.)
    --log                       Enable internals logger
    --verbose                   Enable debugging output""".format(appname=APPNAME, cmd="sos"))  # line 216

@_coconut_tco  # line 218
def merge(file: '_coconut.typing.Optional[bytes]'=None, into: '_coconut.typing.Optional[bytes]'=None, filename: '_coconut.typing.Optional[str]'=None, intoname: '_coconut.typing.Optional[str]'=None, mergeOperation=MergeOperation.BOTH, conflictResolution=ConflictResolution.ASK) -> 'bytes':  # line 218
    ''' Merges binary text contents 'file' into file 'into', returning merged result. '''  # line 219
    encoding = None  # type: str  # line 220
    othr = None  # type: _coconut.typing.Sequence[str]  # line 220
    othreol = None  # type: _coconut.typing.Optional[bytes]  # line 220
    curr = None  # type: _coconut.typing.Sequence[str]  # line 220
    curreol = None  # type: _coconut.typing.Optional[bytes]  # line 220
    differ = difflib.Differ()  # line 221
    try:  # load files line-wise and normalize line endings (keep the one of the current file) TODO document  # line 222
        encoding, othreol, othr = detectAndLoad(filename=filename, content=file)  # line 223
        encoding, curreol, curr = detectAndLoad(filename=intoname, content=into)  # line 224
    except Exception as E:  # line 225
        Exit("Cannot merge '%s' into '%s': %r" % (filename, intoname, E))  # line 225
    if None not in [othreol, curreol] and othreol != curreol:  # line 226
        warn("Differing EOL-styles detected during merge. Using current file's style for merge output")  # line 226
    output = list(differ.compare(othr, curr))  # type: List[str]  # from generator expression  # line 227
    debug("Diff output: " + "".join([o.replace("\n", "\\n") for o in output]))  # line 228
    blocks = []  # type: List[MergeBlock]  # merged result in blocks  # line 229
    tmp = []  # type: List[str]  # block lines  # line 230
    last = " "  # line 231
    for no, line in enumerate(output + ["X"]):  # EOF marker  # line 232
        if line[0] == last:  # continue filling consecutive block  # line 233
            tmp.append(line[2:])  # continue filling consecutive block  # line 233
            continue  # continue filling consecutive block  # line 233
        if line == "X":  # EOF marker - perform action for remaining block  # line 234
            if len(tmp) == 0:  # nothing left to do  # line 235
                break  # nothing left to do  # line 235
        if last == " ":  # block is same in both files  # line 236
            blocks.append(MergeBlock(MergeBlockType.KEEP, [line for line in tmp], line=no - len(tmp)))  # line 237
        elif last == "-":  # may be a deletion or replacement, store for later  # line 238
            blocks.append(MergeBlock(MergeBlockType.REMOVE, [line for line in tmp], line=no - len(tmp)))  # line 239
        elif last == "+":  # may be insertion or replacement  # line 240
            blocks.append(MergeBlock(MergeBlockType.INSERT, [line for line in tmp], line=no - len(tmp)))  # line 241
            if len(blocks) >= 2 and len(blocks[-1].lines) == len(blocks[-2].lines):  # replaces previously removed section entirely if any  # line 242
                if len(blocks[-1].lines) >= 2 or (blocks[-1].changes is None and blocks[-2].changes is None):  # full block or no intra-line comment  # line 243
                    blocks[-2] = MergeBlock(MergeBlockType.REPLACE, blocks[-1].lines, line=no - len(tmp), replaces=blocks[-2])  # remember replaced stuff  # line 244
                else:  # may have intra-line modifications  # line 245
                    blocks[-2] = MergeBlock(MergeBlockType.MODIFY, blocks[-1].lines, line=no - len(tmp), replaces=blocks[-2], changes=blocks[-1].changes)  # line 246
                blocks.pop()  # remove TOS  # line 247
        elif last == "?":  # intra-line change comment  # line 248
            ilm = getIntraLineMarkers(tmp[0])  # tipe is one of MergeBlockType 1, 2, 4. # "? " line includes a trailing \n for some reason  # line 249
            blocks[-1] = dataCopy(MergeBlock, blocks[-1], tipe=MergeBlockType.MODIFY, changes=ilm)  # update to MODIFY, otherwise may be REPLACE instead  # line 250
        last = line[0]  # line 251
        tmp[:] = [line[2:]]  # line 252
    debug("Diff blocks: " + repr(blocks))  # line 253
    output = []  # line 254
    for block in blocks:  # line 255
        if block.tipe == MergeBlockType.KEEP:  # line 256
            output.extend(block.lines)  # line 257
        elif block.tipe == MergeBlockType.INSERT and not (mergeOperation & MergeOperation.REMOVE):  # line 258
            output.extend(block.lines)  # line 259
        elif block.tipe == MergeBlockType.REPLACE:  # complete block replacement  # line 260
            if mergeOperation & MergeOperation.INSERT:  # replaced stuff TODO allow insertion BEFORE and alternatively AFTER?  # line 261
                output.extend(block.replaces.lines)  # replaced stuff TODO allow insertion BEFORE and alternatively AFTER?  # line 261
            if not (mergeOperation & MergeBlockType.REMOVE):  # if remove, don't add replaced lines, otherwise do  # line 262
                output.extend(block.lines)  # if remove, don't add replaced lines, otherwise do  # line 262
        elif block.tipe == MergeBlockType.REMOVE and mergeOperation & MergeOperation.INSERT:  # line 263
            output.extend(block.lines)  # line 264
        elif block.tipe == MergeBlockType.MODIFY:  # line 265
            if block.changes is not None and block.changes.tipe == MergeBlockType.INSERT:  # cannot be REMOVE, because it's always "-" then "+"  # line 266
                output.extend(block.lines if mergeOperation & MergeOperation.INSERT else block.replaces.lines)  # TODO logic?  # line 267
            elif block.replaces.changes is not None and block.replaces.changes.tipe == MergeBlockType.REMOVE:  # line 268
                output.extend(block.lines if mergeOperation & MergeOperation.REMOVE else block.replaces.lines)  # line 269
            elif block.changes is not None and block.changes.tipe == MergeBlockType.MODIFY:  # always both sides modified, but may differ in markers  # line 270
                if conflictResolution == ConflictResolution.THEIRS:  # line 271
                    output.extend(block.replaces.lines)  # line 272
                elif conflictResolution == ConflictResolution.MINE:  # line 273
                    output.extend(block.lines)  # line 274
                elif conflictResolution == ConflictResolution.ASK:  # line 275
                    print(ajoin("THR ", block.replaces.lines, "\n"))  # line 276
                    print(ajoin("MIN ", block.lines, "\n"))  # line 277
                    reso = {"i": ConflictResolution.MINE, "t": ConflictResolution.THEIRS, "m": ConflictResolution.NEXT, "u": None}.get(user_input(" Resolve: *M[I]ne, [T]heirs, [M]erge, [U]ser defined: ").strip().lower(), ConflictResolution.MINE)  # type: _coconut.typing.Optional[int]  # line 278
                    debug("User selected %d" % reso)  # line 279
                    _coconut_match_check = False  # line 280
                    _coconut_match_to = reso  # line 280
                    if _coconut_match_to is None:  # line 280
                        _coconut_match_check = True  # line 280
                    if _coconut_match_check:  # line 280
                        warn("Manual input not implemented yet")  # TODO allow multi-line input (CTRL+D?)  # line 281
                    _coconut_match_check = False  # line 282
                    _coconut_match_to = reso  # line 282
                    if _coconut_match_to == ConflictResolution.MINE:  # line 282
                        _coconut_match_check = True  # line 282
                    if _coconut_match_check:  # line 282
                        debug("Using mine")  # line 283
                        output.extend(block.lines)  # line 284
                    _coconut_match_check = False  # line 285
                    _coconut_match_to = reso  # line 285
                    if _coconut_match_to == ConflictResolution.THEIRS:  # line 285
                        _coconut_match_check = True  # line 285
                    if _coconut_match_check:  # line 285
                        debug("Using theirs")  # line 286
                        output.extend(block.replaces.lines)  # line 287
                    _coconut_match_check = False  # line 288
                    _coconut_match_to = reso  # line 288
                    if _coconut_match_to == ConflictResolution.NEXT:  # line 288
                        _coconut_match_check = True  # line 288
                    if _coconut_match_check:  # line 288
                        warn("Intra-line merge not implemented yet, skipping line(s)")  # line 289
                        output.extend(block.replaces.lines)  # line 290
            else:  # e.g. contains a deletion, but user was asking for insert only??  # line 291
                warn("Investigate this case")  # line 292
                output.extend(block.lines)  # default or not .replaces?  # line 293
    debug("Merge output: " + "; ".join(output))  # line 294
    nl = b"\n" if othreol is None else othreol if curreol is None else curreol  # type: bytes  # line 295
    return _coconut_tail_call(nl.join, [line.encode(encoding) for line in output])  # line 296
# TODO handle check for more/less lines in found -/+ blocks to find common section and splitting prefix/suffix out


class Counter:  # line 300
    def __init__(_, initial: 'Number'=0) -> 'None':  # line 301
        _.value = initial  # type: Number  # line 301
    def inc(_, by=1) -> 'Number':  # line 302
        _.value += by  # line 302
        return _.value  # line 302
    def inc_old(_, by=1) -> 'Number':  # line 303
        old = _.value  # line 303
        _.value += by  # line 303
        return old  # line 303

class Logger:  # line 305
    ''' Logger that supports many items. '''  # line 306
    def __init__(_, log):  # line 307
        _._log = log  # line 307
    def debug(_, *s):  # line 308
        _._log.debug(sjoin(*s))  # line 308
    def info(_, *s):  # line 309
        _._log.info(sjoin(*s))  # line 309
    def warn(_, *s):  # line 310
        _._log.warning(sjoin(*s))  # line 310
    def error(_, *s):  # line 311
        _._log.error(sjoin(*s))  # line 311
  ...
  '''  # line 319
    ...
    def loadBranches(_) -> 'None':  # line 343
        ...
        try:  # fails if not yet created (on initial branch/commit)  # line 345
            ...
            try:  # line 355
                os.makedirs(os.path.join(_.root, metaFolder))  # line 355
                _.branches = {}  # line 355
            except Exception as F:  # line 356
                Exit("Error: Couldn't read branches metadata, but branches folder seems to already exist. %r %r" % (E, F))  # line 356
...
def changes(argument: 'str'=None, options: '_coconut.typing.Sequence[str]'=[]) -> 'ChangeSet':  # line 627
    ...
    revision = revision if revision >= 0 else len(m.commits) + revision  # type: int  # negative indexing  # line 635
...
def diff(argument: 'str', options: '_coconut.typing.Sequence[str]'=[]) -> 'None':  # line 643
    ...
    revision = revision if revision >= 0 else len(m.commits) + revision  # type: int  # negative indexing  # line 651
...
def commit(argument: '_coconut.typing.Optional[str]'=None, options: '_coconut.typing.Sequence[str]'=[]) -> 'None':  # line 676
    ...
    info("Created new revision r%d%s (+%d/-%d/~%d)" % (revision, ((" '%s'" % argument) if argument is not None else ""), len(changes.additions), len(changes.deletions), len(changes.modifications)))  # line 689
...
# Determine current changes
    ...
    if argument is not None:  # branch/revision specified  # line 728
        ...
        if revision < 0 or revision > max(m.commits):  # line 731
            Exit("Unknown revision r%d" % revision)  # line 731
...
# Determine file changes from other branch over current file tree
    ...
    changes = m.findChanges(checkContent=strict, inverse=True, considerOnly=None if not m.track else trackingUnion)  # type: ChangeSet  # determine difference of other branch vs. file tree. "addition" means exists now but not in other, and should be removed unless in tracking mode  # line 782
...
def log() -> 'None':  # line 881
    ...
    info((lambda _coconut_none_coalesce_item: "r%d" % m.branch if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)("Offline commit history of branch '%s':" % m.branches[m.branch].name))  # TODO also retain "from branch/revision" on branching?  # line 886
...
def findSosVcsBase() -> 'Tuple[_coconut.typing.Optional[str], _coconut.typing.Optional[str], _coconut.typing.Optional[str]]':  # line 953
    ...
    while not os.path.exists(os.path.join(path, metaFolder)):  # line 958
        ...
        vcss = [executable for folder, executable in vcsFolders.items() if folder in contents]  # type: _coconut.typing.Sequence[str]  # determine VCS type  # line 960
...
force_sos = '--sos' in sys.argv  # line 1019
_log = Logger(logging.getLogger(__name__))  # line 1020
debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 1020
if __name__ == '__main__':  # line 1021
    main()  # line 1021
