...
# Constants
APPNAME = "Subversion Offline Solution  V.%s  (C) Arne Bachmann" % version.__version__  # type: str  # line 49
...
if __name__ == '__main__':  # line 992
    level = logging.DEBUG if os.environ.get("DEBUG", "False").lower() == "true" or '--verbose' in sys.argv or '-v' in sys.argv else logging.INFO  # line 993
    logging.basicConfig(level=level, stream=sys.stderr if '--log' in sys.argv else sys.stdout, format=("%(asctime)-23s %(levelname)-8s %(name)s:%(lineno)d | %(message)s" if '--log' in sys.argv else "%(message)s"))  # line 994
    sos = '--sos' in sys.argv  # line 995
...
logger = Logger(logging.getLogger(__name__))  # line 998
debug, info, warn, error = logger.debug, logger.info, logger.warn, logger.error  # line 998


if __name__ == '__main__':  # line 1001
    if '--help' in sys.argv or len(sys.argv) < 2:  # line 1002
        usage()  # line 1002
        Exit()  # line 1002
    command = sys.argv[1] if len(sys.argv) > 1 else None  # type: _coconut.typing.Optional[str]  # line 1003
    root, vcs, cmd = findSosVcsBase()  # root is None if no .sos folder exists up the folder tree (still working online); vcs is checkout/repo root folder; cmd is the VCS base command  # line 1004
    debug("Found root folders for SOS/VCS: %s/%s" % ("" if root is None else root, "" if vcs is None else vcs))  # line 1005
    defaults["defaultbranch"] = vcsBranches.get(cmd, "trunk")  # sets dynamic default with SVN fallback  # line 1006
    if sos or root is not None or ("" if command is None else command) == "offline":  # in offline mode or just going offline TODO what about git config?  # line 1007
        os.chdir(os.getcwd() if command == "offline" else os.getcwd() if root is None else root)  # since all operatiosn use os.getcwd() and we save one argument to each function  # line 1008
        parse(root)  # line 1009
    else:  # online mode - delegate to VCS  # line 1010
        info("SOS: Running '%s %s'" % (cmd, " ".join(sys.argv[1:])))  # line 1011
        import subprocess  # only requuired in this section  # line 1012
        process = subprocess.Popen([cmd] + sys.argv[1:], shell=False, stdin=subprocess.PIPE, stdout=sys.stdout, stderr=sys.stderr)  # line 1013
        inp = ""  # type: str  # line 1014
        while True:  # line 1015
            so, se = process.communicate(input=inp)  # line 1016
            if process.returncode is not None:  # line 1017
                break  # line 1017
            inp = sys.stdin.read()  # line 1018
        if sys.argv[1] == "commit" and process.returncode == 0:  # successful commit - assume now in sync again (but leave meta data folder with potential other feature branches behind until "online")  # line 1019
            if root is None:  # line 1020
                Exit("Cannot determine VCS root folder: Unable to mark repository as synchronized and will show a warning when leaving offline mode")  # line 1020
            m = Metadata(root)  # line 1021
            m.loadBranches()  # read repo  # line 1022
            m.branches[m.branch] = dataCopy(BranchInfo, m.branches[m.branch], insync=True)  # mark as committed  # line 1023
