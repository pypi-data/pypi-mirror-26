...
# API.
def about(output_handle, server_host, server_port, server_debug, ssl_check,
          ca_bundle, output_format):
    client = TransferClient(server_host, server_port, server_debug,
                            ssl_check, ca_bundle)
    _write(output_handle, client.about(), output_format)


def consumer(
        output_handle, server_host, server_port, server_debug, project_name,
        ssl_check, ca_bundle, cert_path, key_path, output_format):
    client = TransferClient(server_host, server_port, server_debug, ssl_check,
                            ca_bundle, (cert_path, key_path))
...
def consumer_transfers(
        ...
        ssl_check, ca_bundle, cert_path, key_path, output_format):
    client = TransferClient(server_host, server_port, server_debug,
                            ssl_check, ca_bundle, (cert_path, key_path))
    _write(
...
def consumer_transfer(
        output_handle, server_host, server_port, server_debug, project_name,
        transfer_id, ssl_check, ca_bundle, cert_path, key_path, output_format):
    client = TransferClient(server_host, server_port, server_debug,
                            ssl_check, ca_bundle, (cert_path, key_path))
    _write(
...
def producer_transfer_start(
        output_handle, metadata_handle, server_host, server_port,
        server_debug, project_name, ssl_check, ca_bundle, cert_path, key_path,
        output_format):
    client = TransferClient(server_host, server_port, server_debug, ssl_check,
                            ca_bundle, (cert_path, key_path))
...
def producer_upload(
        output_handle, server_host, server_port, server_debug, project_name,
        transfer_id, file_handle, ssl_check, ca_bundle, cert_path, key_path,
        output_format):
    client = TransferClient(server_host, server_port, server_debug, ssl_check,
                            ca_bundle, (cert_path, key_path))
    _write(
...
def check_metadata(
        metadata_handle, server_host, server_port, server_debug, project_name,
        ssl_check, ca_bundle, cert_path, key_path):
...
def _transfer_chunked(
    ...
    for item, size in zip(metadata['files'][1:], sizes):
...
def transfer(
        log_handle, paths, server_host, server_port, server_debug,
        project_name, title, additional_handle, ssl_check, ca_bundle,
        cert_path, key_path):
...
def resume(
        log_handle, server_host, server_port, server_debug, project_name,
        ssl_check, ca_bundle, cert_path, key_path):
    """Resume an interrupted transfer."""
    cache = Cache()
    client = TransferClient(server_host, server_port, server_debug, ssl_check,
...
def cancel(log_handle, server_host, server_port, server_debug, project_name,
           ssl_check, ca_bundle, cert_path, key_path):
...
def main():
    ...
    server_parser.add_argument(
        '-s', dest='server_host', metavar='HOST', type=str,
        default=config.get_default('server_host'),
        help='server host (%(type)s default="%(default)s")')

    server_parser.add_argument(
        '-P', dest='server_port', metavar='PORT', type=int,
        default=config.get_default('server_port'),
        help='server port (%(type)s default="%(default)s")')

    server_parser.add_argument(
        '-d', dest='server_debug', default=False,
        action='store_true', help='enable http debugging')
    ...
    if 'project_name' in args and not args.project_name:
        parser.error('project not specified')

    if 'server_host' in args and not args.server_host:
