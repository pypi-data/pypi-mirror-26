...
class SessionProfile(UriIdentity):
    ...
    def open(self, parameter_file=None, agent_requirements=None, properties=None, response_map_lib=None, **kwargs):
        ...
        if response_map_lib != None and not (type(response_map_lib) in six.string_types):
    ...
    def slc_query_properties(self):
        """Query a running session for all session properties"""
        return self.invoke_action('slc_query_properties')

    def slc_query_step_properties(self, action, command=None, parameters=None, properties=None, response_map=None):
        """Query a running session for step actin properties"""
        return self.invoke_action('slc_query_step_properties_' + action, command=command, parameters=parameters, properties=properties, response_map=response_map )
