...
class EKeggNaming( Enum ):
    """
    How to name KEGG genes
    
    :data DIGEST:   Use the hex digest of the AA sequence.
    :data ID:       Use the KEGG ID (recommended)
    :data SYMBOL:   Use the first (alphabetical) symbol.
    :data SYMBOLS:  Use all the symbols.
    """
    DIGEST = 1
    ID = 2
    SYMBOL = 4
...
class BlastFile:
    def __init__( self, file_name: str ):
        ...
        with open( file_name, "r" ) as file_in:
            for line in file_in:
                self.contents.append( BlastLine( line.strip() ) )
...
class BlastLine:
    def __init__( self, line ):
        ...
        self.bit_score = int( elements[11] )
    ...
    def __init__( self, file_name: str ):
        ...
        for gene_list in self.clusters.values():
            for gene in gene_list:
                if gene.name in self.lookup and not warn_name:
                    warn_name = True
                    MCMD.warning( "There are at least two LECA genes with the same name (e.g. «{}»). Fix your data, do not continue.".format( gene.name ) )
                
                self.lookup[gene.name] = gene
...
class KeggFile:
    ...
    def __init__( self, file_name: str, naming: EKeggNaming, permit: List[str] ):
        """
        CONSTRUCTOR 
        """
        ...
        for entry in MCMD.iterate( entries, "Resolving names via HTTP" ):
            ...
            for gene_id in gene_ids:
                ...
                if not kegg_gene.symbols and not warn_symbol:
                    warn_symbol = True
                    MCMD.warning( "There is at least one gene (e.g. «{}») with symbol.".format( kegg_gene ) )
                ...
                # Set the name (probably unnecessary now that genes and classes are separate)
                if naming == EKeggNaming.DIGEST:
                    kegg_gene.name = kegg_gene.aaseq_hexdigest
                elif naming == EKeggNaming.ID:
                    kegg_gene.name = kegg_gene.gene_id
                elif naming == EKeggNaming.SYMBOL:
                    kegg_gene.name = array_helper.first( sorted( kegg_gene.symbols ) )
                elif naming == EKeggNaming.SYMBOLS:
                    kegg_gene.name = ",".join( x for x in kegg_gene.symbols )
                else:
                    raise SwitchError( "naming", naming )
                
                if kegg_gene.name in used_names:
                    repeated_names.add( kegg_gene.name )
                else:
                    used_names.add( kegg_gene.name )
        ...
        for kegg_gene in self.genes:
            if any( kegg_class.is_bad_class for kegg_class in kegg_gene.kegg_classes ):
                kegg_gene.is_in_any_bad_class = True


class EConfidence( Enum ):
    """
    Confidence level of the match. See the `readme.md` for details.
    ...
    See the `COLOURS` documentation for the corresponding colours.
...
COLOURS = { EConfidence.NAME_EXACT    : ("#00FF00", "#000000", "GREEN", "BLACK"),
            EConfidence.NAME_NUMERIC  : ("#FFFF00", "#000000", "YELLOW", "BLACK"),
            EConfidence.USER_FOUND    : ("#00FF00", "#FFFF00", "GREEN", "YELLOW"),
            EConfidence.USER_NOT_FOUND: ("#FF0000", "#FFFF00", "RED", "YELLOW"),
            EConfidence.USER_AMBIGUOUS: ("#0000FF", "#FFFF00", "BLUE", "YELLOW"),
            EConfidence.USER_DOMAIN   : ("#00FFFF", "#FFFF00", "CYAN", "YELLOW"),
            EConfidence.TEXT_CONTAINS : ("#0000FF", "#000000", "BLUE", "BLACK"),
            EConfidence.ERROR         : ("#808080", "#FF0000", "GRAY", "RED"),
            EConfidence.NO_MATCH      : ("#FF0000", "#000000", "RED", "BLACK") }
...
def load_leca( file_name: str ):
    ...
    MCMD.print( "{} clusters and {} genes loaded.".format( len( _leca_file.clusters ), _leca_file.total ) )


@command()
def load_kegg( file_name: str, permit: Optional[List[str]] = None, naming: EKeggNaming = EKeggNaming.ID ):
    """
    Loads the KEGG (KGML) file.
    
    :param permit:      List of gene types permitted, e.g. `hsa` for human or `eco` for E-coli. 
    :param naming:      Naming strategy. This is used for output only.
    :param file_name:   File to load.
                        Obtain this by selecting `download KGML` from a KEGG pathway. The `Reference Pathway (KO)` is recommended.
    """
    global _kegg_file
    _kegg_file = KeggFile( file_name, naming, permit )
...
@command()
def write_fasta( file_name: str,
                 naming: EKeggNaming = EKeggNaming.ID ) -> None:
    """
    ...
    with open( file_name, "w" ) as file_out:
        for kegg_gene in MCMD.iterate( _kegg_file.genes, "Writing genes" ):
            name = kegg_gene.get_name( naming )
...
def resolve( silent: bool = False,
             ...
             combine: bool = False,
             diagram: MOptional[Filename[__EXT_TXT]] = "stdout",
             report: MOptional[Filename[__EXT_TXT]] = "stdout" ):
    """
    Prints the overlap between the loaded KEGG, LECA and BLAST sets.
    
    :param diagram:         Output diagram file.
                            If set, a DIAGRAM file is written.
                            If not set, no DIAGRAM output is written.
    ...
    try:
        ...
        # - CLUSTERS file
        if diagram:
            diagram_file = __open_write( diagram, "diagram" )
        ...
        for index, kegg_gene in MCMD.enumerate( _kegg_file.genes, "Exhaustive search" ):
            ...
            blast_clusters: Dict[str, int] = find_blast_confidences( kegg_gene )
            ...
            if diagram is not None:
                ...
                diagram_file.write( "{}\t{},{}".format( kegg_gene.gene_id, background_colour, foreground_colour ) )
                diagram_file.write( "\n" )
...
def find_blast_confidences( gene: KeggGene ) -> Dict[str, int]:
    """
    Finds the BLAST matches.
    :param gene:    Gene to find 
    ...
    for blast in _blast_file.contents:
        if blast.subject_id == gene.name:
            leca_gene: LecaGene = _leca_file.lookup.get( blast.query_id )
            
            if leca_gene is None:
                raise ValueError( "LECA gene «{}» referenced in the BLAST file but it doesn't exist in the LECA file.".format( blast.query_id ) )
...
def legend():
    ...
    MCMD.print( "{} {} {} {} {} {}".format( "confidence".ljust(20), "back_hex".ljust(10), "fore_hex".ljust(10), "fore_name".ljust(10), "back_name".ljust(10), "description" ) )
    MCMD.print( "{} {} {} {} {} {}".format( "----------".ljust(20), "--------".ljust(10), "--------".ljust(10), "---------".ljust(10), "---------".ljust(10), "-----------" ) )
    
    for key, value in COLOURS.items():
        MCMD.print( "{} {} {} {} {} {}".format( key.name.ljust(20), value[0].ljust(10), value[1].ljust(10), value[2].ljust(10), value[3].ljust(10), documentation[key.name] ) )
