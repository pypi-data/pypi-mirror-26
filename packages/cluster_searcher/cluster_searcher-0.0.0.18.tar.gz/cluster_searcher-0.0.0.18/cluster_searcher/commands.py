...
class EUMode( Enum ):
    ...
    :data UNIQUE:   Only the genes uniquely identifying a class are written.
                    This means colours represent the underlying class.
                    But some classes are not coloured.
                    This requires report file format version 2.
                    This was the default in the previous version for TEXT-mode colours.
    :data NORMAL:   Colours are written best-first.
                    This means that classes with more than one gene get the colour of the least-matched gene.
    :data REVERSED: Colours are written best-last.
                    This means that classes with more than one gene get the colour of the best-batched gene.
    :data ORIGINAL: Colours are written in which they appear.
                    This means that classes with more than one gene get the colour of an arbitrary gene from their set.
                    This was the default in the previous version for BLAST-mode colours.
    """
    UNIQUE = 0
    NORMAL = 1
    REVERSED = 2
...
@command()
def translate( colour: EResolveColour, read: MOptional[Filename[__EXT_REPORT]] = None, write: MOptional[Filename[__EXT_UDM]] = None, unique: EUMode = EUMode.UNIQUE ):
    """
    Translates the report to KEGG user data mapping.
    
    :param unique:      Controls colouring of classes (boxes) containing more than one gene.
    ...
    if write is None:
        ...
        write = file_helper.replace_extension( read_file, ".udm({})".format( colour.name.lower() ) )
    ...
    for element in root:
        ...
        if unique == EUMode.UNIQUE:
            if not "unique" in element:
                raise ValueError("UNIQUE mode requires report format version 2.")
            
            is_unique = int( element.attrib["unique"] ) != -1
        ...
        lines.append( TranslateLine( report_name, gene_colour ) )
    
    if unique == EUMode.NORMAL:
        lines = list( sorted( lines, key = lambda x: x.colour[0] ) )
    elif unique == EUMode.REVERSED:
        lines = list( sorted( lines, key = lambda x: -x.colour[0] ) )
    
    try:
        for line in lines:
            foreground_colour = line.colour[1].html
            background_colour = line.colour[2].html
            diagram_file.write( "{}\t{},{}".format( line.name, background_colour, foreground_colour ) )
            diagram_file.write( "\n" )
    finally:
        diagram_file.close()
