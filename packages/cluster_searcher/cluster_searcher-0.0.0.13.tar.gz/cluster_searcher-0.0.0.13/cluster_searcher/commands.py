    ...
    def __init__( self, file_name: str ) -> None:
        ...
        with MCMD.action( "Loading BLAST" ) as action:
            with open( file_name, "r" ) as file_in:
                for line in file_in:
                    blast = BlastLine( line.strip(), warn_blast_line )
                    self.contents.append( blast )
                    self.lookup_by_kegg[blast.query_id].append( blast )
                    action.still_alive()


class BlastLine:
    def __init__( self, line, warn_blast_line: ByRef[bool] ) -> None:
        ...
        self.leca_gene = _leca_file.lookup_by_leca.get( self.subject_id.lower() ) if _leca_file else None
        
        if not self.leca_gene:
            if not warn_blast_line.value:
                MCMD.warning( "One or more BLAST genes (e.g. «{}» aka «{}») do not match any gene from the LECA set.".format( elements[1], self.subject_id ) )
...
class KeggFile:
    ...
    def __init__( self, file_name: str, naming: EKeggNaming, permit: List[str], verbose: bool ) -> None:
        """
        CONSTRUCTOR 
        """
...
def load_blast( file_name: str ):
    ...
    MCMD.print( "{} BLASTs loaded, {} matched to LECA.".format( len( _blast_file.contents ), sum(x.leca_gene is not None for x in _blast_file.contents) ) )
...
@command()
def load_kegg( file_name: str, permit: Optional[List[str]] = None, naming: EKeggNaming = EKeggNaming.ID, verbose: bool = False ):
    """
    ...
    """
    global _kegg_file
    _kegg_file = KeggFile( file_name, naming, permit, verbose )
...
@command()
def resolve( colour: EResolveColour,
             silent: bool = False,
             remove: Optional[List[str]] = None,
             combine: bool = False,
             diagram: MOptional[Filename[__EXT_TXT]] = "default",
             report: MOptional[Filename[__EXT_TXT]] = "default" ):
    """
    ...
    :param diagram:         Output diagram file.
                            ...
                            Accepts `default` and `stdout` as a value.
                            `default` is `stdout` when `colour` is not `NONE`, else it is `None`. 
                            
    :param report:          Output REPORT file.
                            If set, a CLUSTERS file is written.
                            If not set, no CLUSTERS output is written.
                            Accepts `default` and `stdout` as a value.
                            `default` is the same as `stdout`.
    ...
    if diagram.lower() == "default":
        if colour == EResolveColour.NONE:
            diagram = None
        else:
            diagram = "stdout"
    ...
    try:
        ...
        for index, kegg_gene in MCMD.enumerate( _kegg_file.genes, "Exhaustive search" ):
            ...
            blast_clusters: Dict[str, int] = find_blast_confidences( kegg_gene, wrn_mismatch )
            ...
            if diagram is not None:
                if colour == EResolveColour.TEXT:
                    gene_colour = COLOURS_TEXT[best_type]
                elif colour == EResolveColour.BLAST:
                    gene_colour = COLOURS_BLAST[best_blast]
                elif colour == EResolveColour.COMBINED:
                    gene_colour = COLOURS_COMBINED[best_type, best_blast]
...
def find_blast_confidences( kegg_gene: KeggGene, wrn_mismatch : ByRef[bool] ) -> Dict[str, int]:
    """
    ...
    blast_lines = _blast_file.lookup_by_kegg.get( kegg_gene.name )
    
    if blast_lines is not None:
        for blast_line in blast_lines:
            leca_gene = blast_line.leca_gene
