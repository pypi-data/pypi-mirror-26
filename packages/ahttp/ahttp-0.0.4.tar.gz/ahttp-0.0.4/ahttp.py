...
async def run(tasks, pool, exception_handle):
    ...
    async with aiohttp.ClientSession( cookies = status[0] , headers = status[1], cookie_jar = status[2] ) as session:
        await asyncio.wait( [ asyncio.ensure_future( bound_fetch(sem, i , exception_handle, session=session) ) for i in tasks ] )
...
async def bound_fetch(sem, i, exception_handle, session ):
    # 限制信号量
    async with sem:
