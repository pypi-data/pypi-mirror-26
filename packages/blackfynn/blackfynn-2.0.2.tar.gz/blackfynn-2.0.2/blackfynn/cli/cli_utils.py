...
def set_working_dataset(dataset):
    ...
    working_dataset_file = open(settings.working_dataset_file, 'w+')
    working_dataset_file.write(dataset.id)
    working_dataset_file.close()

    settings.working_dataset = dataset.id

def get_working_dataset(bf):
    from blackfynn import settings

    if settings.working_dataset:
        try:
            working_dataset = bf.get_dataset(settings.working_dataset)
...
def print_datasets(bf):
    ...
    for dataset in bf.datasets():
        if dataset.id == settings.working_dataset:
