...
class Settings(object):
    ...
    def _load_defaults(self):
        ...
        self.working_dataset_file = os.path.join(tempfile.gettempdir(), 'blackfynn_working_dataset_{}'.format(parent_shell_pid))
        if os.path.exists(self.working_dataset_file):
            f = open(self.working_dataset_file, 'r+')
            self.working_dataset = f.read()
            f.close()
        else:
