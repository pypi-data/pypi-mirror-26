...
class Ec2():
    ...
    def get_object_from_name(self, tag_name, object_type):
        ...
        filters_old = [{'Name':'tag:Name', 'Values':[tag_name]}]
        # filter all object of that type by our tag name
        result_old = list(getattr(self.ec2, object_type).filter(Filters=filters_old))
        print('result_old:', result_old)
        for instance in result_old:
...
#                if running_instance > 1:
            ...
            print(object_not_found_message)
            sys.exit(0)
        return aws_object
