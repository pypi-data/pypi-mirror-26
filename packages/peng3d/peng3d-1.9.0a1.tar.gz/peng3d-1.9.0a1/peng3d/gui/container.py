...
__all__ = [
    ...
    "WidgetList", "ContainerButtonBackground",
    ]
...
class _NoScrollGroup(pyglet.graphics.Group):
    def __init__(self,widget,*args,**kwargs):
        super(_NoScrollGroup,self).__init__(*args,**kwargs)
        self.widget = widget
    def __eq__(self,other):
        return (
            self.__class__ is other.__class__
            and self.widget == other.widget
            )
    def set_state(self):
        # Resets the offset temporarily
        glTranslatef(0,-self.widget.offset_y,0)
    def unset_state(self):
        glTranslatef(0,self.widget.offset_y,0)
    def __hash__(self):
        return hash(self.widget)

class WidgetList(Container):
    def __init__(self,name,submenu,window,peng,
                 pos=None,size=None,
                 scrollbar_width=12,
                 borderstyle="flat",
        ...
        super(WidgetList,self).__init__(name,submenu,window,peng,pos,size,_skip_draw=True)
        
        # 0 at the top
        # content height at the bottom
        self._scrollpos = 0
        self._no_scroll_group = _NoScrollGroup(self)
        self._scroll_vlist = self.batch2d.add(4,GL_QUADS,self._no_scroll_group,
                        "v2f",
                        ("c4B/static",[240,119,70,255]*4)
                        )
        
        self.content_height = 0
        self.content_border = content_border
        self.widget_order = []
    ...
    def on_redraw(self):
        ...
        sx,sy = self.size
        # scrollbar size
        ssx,ssy = 24, min(sy/max(1,self.content_height),1)*sy
        # scrollbar offset
        sox,soy = x-ssx, (self._scrollpos/sy)
        print(ssx,ssy)
        ########################################################################
        #self._scroll_vlist.vertices = None
    def appendOrderedWidget(self,widget):
        self.widget_order.append(widget)
        widget._pos = lambda sw,sh, bw,bh: (self.content_border/2,sh-self.content_border-sum(map(lambda i:i.size[1]+self.content_border,self.widget_order)))
        w = widget.size[1]
        widget._size = lambda sw,sh: (self.size[0],w)
        print(widget.pos)
        self.content_height += widget.size[1]
