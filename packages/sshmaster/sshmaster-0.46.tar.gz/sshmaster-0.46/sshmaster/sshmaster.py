...
def scp_cmd_multi_try(current_host, mode, username, local_file, remote_file, keyfile=None, key_string=None, recursive=False, preserve_times=False, port=22, file_owner='default', file_mode='default', print_stdout=True, trys=10, use_sudo=False, debug=False):
    ...
    while actual_trys != trys:
        ...
        try:
            scp_cmd(current_host=current_host, mode=mode, username=username, keyfile=keyfile, local_file=local_file, remote_file=remote_file, recursive=recursive, preserve_times=preserve_times, port=port, file_owner=file_owner, file_mode=file_mode, use_sudo=use_sudo, print_stdout=debug, trys=trys, debug=debug )
