...
def zookeeper_on_line(sweet_settings):
    ...
    app_port = sweet_settings['applicationInstanceConfigurations'].get('sweet.framework.socket.server.tcp.port',80) #settings.SWEETPY_EVUN_CLOUD_REGISTER_APP_PORT
...
def set_http_port():
    ...
    if sweet_settings.get('applicationInstanceConfigurations',None):
        Command.default_port = 80
    else:
        Command.default_port = sweet_settings['applicationInstanceConfigurations'].get('sweet.framework.socket.server.tcp.port',80)
