...
class ConstConfiger:
    ...
    FILE_PATH = 'FilePath'
    ENABLE = 'Enable'
    IS_REGISTER_ZOOKEEPER = 'IsRegisterZookeeper'
...
class Configer(object):
    ...
    def create_new_configer(self):
        ...
        self._configer_dict[ConstConfiger.MODULES] = _modules
        self._configer_dict[ConstConfiger.OPTIONS] = _options

        _options[ConstConfiger.LOGGER] = _logger
        _logger[ConstConfiger.LOGGER_LEVEL] = 10
        ...
        _database[ConstConfiger.DATABASE_DRIVER_NAME] = 'postgres'

    def create_new_module_node(self, module_name, enable, swagger_url, file_path, is_register_zookeeper, git_url, git_username, git_passwd, loop_time,
        ...
        _node = {}
        _node[ConstConfiger.FILE_PATH] = file_path
        _node[ConstConfiger.REISTER_SWAGGER_URL] = swagger_url
        _node[ConstConfiger.ENABLE] = enable
        _node[ConstConfiger.IS_REGISTER_ZOOKEEPER] = is_register_zookeeper
        ...
        _node[ConstConfiger.OTHER_PARAMS] = other_params
        _database = {}
        ...
        _database[ConstConfiger.DATABASE_DRIVER_NAME] = database_driver_name
        _node[ConstConfiger.DATABASE] = _database
        self._configer_dict[ConstConfiger.MODULES][module_name] = _node
        return True
    ...
    def create_new_options_node(self,debug,data_path,file_size,host,port,thread_count,white_list_ip,supper_user_name,supper_passwd,
                                ...
                                database_driver_name,database_ip, databse_port, database_db_name, database_user_name, database_passwd,database_pool_size):
        _options = {}
    ...
    def __init__(self,*args,**kwargs):
        ...
        if (self.__fileName == '') or (self.__fileName == None):
        ...
        admin_path = admin_path.replace('/common','') + '/TTAdmin'
        self.create_new_module_node(module_name = 'TTAdmin',
                                     enable= True,
                                     swagger_url = '',
    ...
    #     result = FuncHelper.get_curr_path() if self._work_directory == '' else  self._work_directory
    ...
    @property
    def modules(self):
        return self._configer_dict[ConstConfiger.MODULES]

    def get_module_enable_by_module_name(self,module_name):
        return self._configer_dict[ConstConfiger.MODULES][module_name][ConstConfiger.ENABLE]

    def get_module_path_by_module_name(self, module_name):
        return self._configer_dict[ConstConfiger.MODULES][module_name][ConstConfiger.FILE_PATH]
    ...
    def get_module_database_option_by_name(self,module_name):
        modules = self.modules[module_name]
        return modules[ConstConfiger.DATABASE]

    def get_module_is_enable_by_module_name(self,module_name):
        result = False
        try:
            modules = self.modules[module_name]
            result = modules[ConstConfiger.ENABLE]
        except Exception as e:
            pass
        return result

    def get_module_option_by_module_name(self,module_name):

        _module_option = self.modules[module_name]
        result = {}
        result['file_path'] = _module_option.get(ConstConfiger.FILE_PATH, '')
        ...
        result['database_driver_name'] = _module_option[ConstConfiger.DATABASE].get(ConstConfiger.DATABASE_DRIVER_NAME,
                                                                                    'postgres')
        ...
        result['other_params'] = FuncHelper.dict_to_json(_module_option.get(ConstConfiger.OTHER_PARAMS, {}))
        result['enable'] = _module_option.get(ConstConfiger.ENABLE, True)
        result['is_register_zookeeper'] = _module_option.get(ConstConfiger.IS_REGISTER_ZOOKEEPER, False)
        return result

    def delete_module_by_module_name(self,is_service,module_name):
        if is_service:
            _ = self._configer_dict[ConstConfiger.MODULES].pop(module_name, None)
        else:
    ...
    def get_system_option(self):
        ...
        result['database_user_name'] = _module_option[ConstConfiger.DATABASE].get(ConstConfiger.DATABASE_USER_NAME, '')
        result['database_passwd'] = _module_option[ConstConfiger.DATABASE].get(ConstConfiger.DATABASE_PASSWORD, '')
        result['database_driver_name'] = _module_option[ConstConfiger.DATABASE].get(ConstConfiger.DATABASE_DRIVER_NAME, 'postgres')
