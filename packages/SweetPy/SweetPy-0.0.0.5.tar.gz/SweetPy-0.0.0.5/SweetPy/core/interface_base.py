...
from SweetPy.common.configer import *
from .base import *
...
def get_message_and_httpstatus_by_code(code):
    key = ''
...
def create_response(code,data=None,message=None):
    http_status,_message = get_message_and_httpstatus_by_code(code.value)
    result = {}
...
class InterfaceBase(object): #,metaclass=DecoratorMetaClass
    def _create_response(self,code,data=None,message=None):
        return create_response(code,data,message)
    ...
    def _redirect(self,module_name,path):
        if module_name == '':
            return Response(code=302, text='redirect', headers={"Location":path})
        return Response(code=302, text='redirect' ,headers={"Location":'/' + module_name + '/' + path})
