...
class FormBlock(blocks.StructBlock):
    ...
    class Meta:
        icon = 'icon icon-form'
        template = 'wapps/forms/block.html'

    def get_context(self, value, parent_context=None):
        if not parent_context:
            context = super(FormBlock, self).get_context(value)
        else:
            context = super(FormBlock, self).get_context(value, parent_context)

        form = value['form']
        form.add_to_context(context)
        return context
