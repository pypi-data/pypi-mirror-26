    ...
    def add_request_for_rating(self, target, event=None):
        ...
        if not isinstance(target, Identifier):
            try:
                ...
                raise ValueError("the target or a feedback_request must be given as an Identifier or as an object with an identifier field")
        if target is not None and not isinstance(target, Identifier):
            try:
                event = event.identifier
            except:
                raise ValueError("the event or a feedback_request must be given as an Identifier or as an object with an identifier field")
        component = {
            'type' : 'request_for_rating',
            'feedback_request' : {
                'feedback_type' : 'rating',
                'target_identifier' : target.to_json(),
                'event_identifier' : event.to_json(),
            },
        }
        self.message_components.append(component)
        return self
            ...
            if identifier.type == 'tag':
                ...
                if tag.symbol_name == '_set_signal_weight':
                    if len(tag.argument_identifiers) != 1 or tag.argument_identifiers[0].type not in ['symbol', 'tag']:
                        raise ValueError("""a _set_signal_weight Tag must have only one argument, which must be the symbol to signal
                    ...
                    if tag.weight_field is None or tag.weight_field < 0:
                        raise ValueError("""a _set_signal_weight Tag must have a non-negative, non-null weight.
                            WARNING: This error should have been detected earlier, when the Tag was created.""")
