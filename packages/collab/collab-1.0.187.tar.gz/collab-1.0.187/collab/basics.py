...
class ObjectManager:
    ...
    def update_step_start(self, step_model):
        ...
        new_step_statistics = {
            ...
            'start_time' : step_model.created.strftime('%Y-%m-%d %H:%M:%S'),
    ...
    def update_step_end(self, step_model):
        """
        update the data created by self.update_step_start() and add additional information to it.
        """
        new_step_statistics = self._statistics_at_beginning_of_step[-1]
        new_step_statistics['end_time'] = step_model.finished.strftime('%Y-%m-%d %H:%M:%S')
        new_step_statistics['duration'] = (step_model.finished - step_model.created).total_seconds()
