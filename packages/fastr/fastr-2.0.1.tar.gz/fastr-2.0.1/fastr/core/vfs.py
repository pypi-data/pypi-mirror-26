...
class VirtualFileSystem(IOPlugin):
    ...
    def path_to_url(self, path, mountpoint=None, scheme=None):
        ...
        if mountpoint is None:
            ...
            options = {k: v for k, v in fastr.config.mounts.items() if abspath.startswith(v.rstrip(os.path.sep) + os.path.sep)}
            if len(options) == 0:
                raise exceptions.FastrMountUnknownError("Mountpath for {} cannot be found in config.mounts".format(abspath))

            options = sorted(options.items(), key=lambda x: len(x[1]))  # Sort by length of path matched
            mountpoint = options[-1][0]  # Pick the longest
