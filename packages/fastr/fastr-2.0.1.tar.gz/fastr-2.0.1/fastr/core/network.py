...
from fastr.utils.compare import compare_execution_dir
...
class Network(Serializable):
    ...
    def __setstate__(self, state):
        ...
        if 'nodegroups' in state:
            for group, nodes in state['nodegroups'].items():
                for node in nodes:
                    self.nodelist[node].nodegroup = group
    ...
    def create_node(self, tool, id_=None, stepid=None, cores=None, memory=None, walltime=None, nodegroup=None):
        ...
        node = NodeType(tool, id_, parent=self,
                        cores=cores, memory=memory,
                        walltime=walltime, nodegroup=nodegroup)
        self.add_node(node)
    ...
    def create_macro(self, network, id_=None):
        node = MacroNode(network, id_, parent=self)
    ...
    def create_constant(self, datatype, data, id_=None, stepid=None, nodegroup=None, sourcegroup=None):
        ...
        const_node = ConstantNode(datatype, data, id_, parent=self, nodegroup=nodegroup)
    ...
    def create_sink(self, datatype, id_=None, stepid=None):
        ...
        node = SinkNode(datatype=datatype, id_=id_, parent=self)
        self.add_node(node)
        self.add_stepid(stepid, node)
    ...
    def is_valid(self):
        ...
        valid = (
            all(check_object(x) for x in self.nodelist.itervalues()) and
            all(check_object(x) for x in self.linklist.itervalues())
        )
    ...
    @classmethod
    def test(cls, reference_data_dir, network=None, source_data=None, force_remove_temp=False):
        """
        ...
        try:
            ...
            temp_results_dir = os.path.normpath(tempfile.mkdtemp(
                prefix='fastr_network_test_'.format(network.id), dir=fastr.config.mounts['tmp']
            ))

            try:
                network.create_reference(source_data=source_data, output_directory=temp_results_dir)
            except Exception as exception:
                fastr.log.warning('Encountered exception during Network execution: {}'.format(exception))
            ...
            if len(validation_result) == 0:
                ...
                for line in validation_result:
                    fastr.log.info(line)
                fastr.log.info('Differences in execution directories:')
    ...
    def draw(self, name=None, image_format=None, draw_dimensions=False, context=None, graph=None, expand_macro=False):
        ...
        if graph is None:
            graph = Digraph(
                ...
                node_attr={'shape': 'record', 'fontsize': '16'}
            )
    ...
    def draw_network(self, name="network_layout", img_format='svg', draw_dimension=False, expand_macro=False):
        """
        ...
        graph = self.draw(draw_dimensions=draw_dimension, expand_macro=expand_macro)
        graph.save(filename=dot_file_path, directory=os.getcwd())
