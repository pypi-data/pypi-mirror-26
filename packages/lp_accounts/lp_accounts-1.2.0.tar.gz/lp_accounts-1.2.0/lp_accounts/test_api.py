...
class AccountTestCase(BaseRestUnitTest):
    ...
    @staticmethod
    def get_user_type_linkedin_data():
        return {
            'username': 'launchmonkey_github@launchpeer.com',
            'access_token': 'thisisanaccesstoken',
            'first_name': 'Launch',
...
class AccountCreateTestCase(AccountTestCase):
    ...
    @parameterized.expand(REQUIRED_FIELDS_LINKEDIN)
    @override_settings(LP_ACCOUNTS_LINKEDIN_APP_ID='testappid', LP_ACCOUNTS_LINKEDIN_APP_SECRET='testclientsecret')
    def test_required_fields_linkedin_account_create(self, _, field):
        self.authenticate(user=None)
        data = self.modify_data(data=AccountTestCase.get_user_type_github_data(), exclude=[field])
        response = self.create(data=data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    @override_settings(LP_ACCOUNTS_LINKEDIN_APP_ID=None, LP_ACCOUNTS_LINKEDIN_APP_SECRET=None)
    def test_linkedin_login_config_check(self):
        self.authenticate(user=None)
        response = self.create(data=self.get_user_type_linkedin_data())
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
@override_settings(LP_ACCOUNTS_LINKEDIN_APP_ID='testappid', LP_ACCOUNTS_LINKEDIN_APP_SECRET='testclientsecret')
class LinkedInAccountLoginTestCase(AccountTestCase):
    url = 'lp_accounts_login'
    ...
    def setUp(self):
        super(LinkedInAccountLoginTestCase, self).setUp()
        data = self.get_user_type_default_data()
        data['type'] = User.TYPE_LINKEDIN
        user = User.objects.create(**data)
        user.save()
        self.data = {
            'username': data['username'],
