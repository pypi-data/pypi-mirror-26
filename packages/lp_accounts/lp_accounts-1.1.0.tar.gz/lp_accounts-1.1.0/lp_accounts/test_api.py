...
class AccountTestCase(TestCase):
    ...
    def reset(self, **kwargs):
        return self.client.post(
            reverse('lp_accounts_password_reset'),
            format='json',
            **kwargs
        )
    ...
    def update_password(self, **kwargs):
        return self.client.post(
            reverse('lp_accounts_password_update'),
            format='json',
            **kwargs
        )
...
class AccountCreateTestCase(AccountTestCase):
    PERMISSIONS = [
        ...
        ('Authenticated Admin User',
         AccountTestCase.get_admin_user(), status.HTTP_201_CREATED)
    ]
    REQUIRED_FIELDS = [
        ('username', 'username'),
        ('email', 'email'),
    ...
    def test_default_account_create_permissions(self, _, user, expected):
        ...
        response = self.create(
            data=AccountTestCase.get_user_type_default_data())
    ...
    def test_google_account_create_permissions(self, _, user, expected):
        self.authenticate(user)
        response = self.create(
            data=AccountTestCase.get_user_type_google_data())
    ...
    def test_facebook_account_create_permissions(self, _, user, expected):
        self.authenticate(user)
        response = self.create(
            data=AccountTestCase.get_user_type_facebook_data())
        self.assertEqual(response.status_code, expected)
    ...
    def test_required_fields_default_account_create(self, _, field):
        ...
        data = self.modify_data(
            data=AccountTestCase.get_user_type_default_data(), exclude=[field])
    ...
    def test_required_fields_google_account_create(self, _, field):
        self.authenticate(user=None)
        data = self.modify_data(
            data=AccountTestCase.get_user_type_google_data(), exclude=[field])
        response = self.create(data=data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_required_fields_facebook_account_create(self, _, field):
        self.authenticate(user=None)
        data = self.modify_data(
            data=AccountTestCase.get_user_type_facebook_data(), exclude=[field])
        response = self.create(data=data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_required_fields_facebook_account_create(self, _, field):
        self.authenticate(user=None)
        data = self.modify_data(
            data=AccountTestCase.get_user_type_github_data(), exclude=[field])
        response = self.create(data=data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_validates_type(self):
        self.authenticate(user=None)
        data = self.modify_data(
            data=AccountTestCase.get_user_type_default_data(), injections={'type': 100})
        response = self.create(data=data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_password_is_secure(self):
        self.authenticate(user=None)
        data = self.modify_data(data=AccountTestCase.get_user_type_default_data(
        ), injections={'password': 'password'})
        response = self.create(data=data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
