...
class Wcc:
    ...
    def AddArticles(self, articles,batch_id=0,thread_name="N"):
        ...
        print("addArticle: m:%-4d i:%-4d x:%-4d e:%-2d --- i:%-4d x:%-4d e:%-2d s:%6.1f b:%s t:%s w:%4s"
        ...
        for asmconf in asmconfigArray:
            ...
            while(True):
                ...
                try:
                    ...
                    print(tags)
                    print(traceback.print_exc())
                    print("func调用失败,func is:"+str(func))
                if www_from == "test":
                    print("test模式,忽略检查")
                    continue
                if len(pagebook)>0:
    ...
    def validate(self,www_from):
        if www_from == "test":
            ...
            return True
        resjson = Wcc.getjson("http://api.wikicivi.com/wcclist/wccinfo/"+www_from)
        #print(resjson)
        try:
            if resjson["status"] != 0:
    ...
    def MakeArtMsg(art_info):
        try:
            ...
            if art_info["rid_from"] > 0 and art_info['sid'] == 0:
                print("Error: 有祖父的文章 sid 不能为0")
                print(art_info)
                return None

            if art_info["pid_from"] > 0 and art_info['sid'] == 0:
                print("Error: 有父亲的文章 sid 不能为0")
                print(art_info)
                return None

            if art_info["pid_from"] > 0 and art_info['tid_from'] - art_info['pid_from'] != art_info["sid"]:
                ...
                if Wcc.downloadFile(file_urlmd5_localpath, file_url) == False:
                    ...
                    if mime_type == "image/gif":
                        ...
                        if(gif2mp4(file_urlmd5_localpath)==False):
                            ...
                            if Wcc.existsFile('wikicivi-files',file_gifmp4_osspath) == True:
                                print("hasossFile:"+file_gifmp4_osspath)
                            else:
                                if Wcc.uploadFile(file_gifmp4_localpath, file_gifmp4_osspath,'wikicivi-files') == False:
