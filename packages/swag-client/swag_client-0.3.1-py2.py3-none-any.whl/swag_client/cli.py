...
def fix_service(ctx, path, name):
    ...
    for a in accounts:
        try:
            if swag.get_service(name, search_filter="[?id=='{id}']".format(id=a['id'])):
                status = [x['status'] for x in a['status']]
...
dynamodb.add_command(fix_service)
s3.add_command(list)
