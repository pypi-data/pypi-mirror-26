...
def create_account(account_uuid,
                   ...
                   submit_batch=True):
    config = Config()
    sawtooth_data_uri = config.settings['endpoints']['hydrosledger']['bank']
    ...
    if response is not None:
        ...
        if response.status_code == 400:
            ...
            account_model_create_error = response_dict['account_model_create_error']
            print('[x] Account instance not created due to error in model instance creation:')
            print('\t' + '•' + account_model_create_error)
...
def edit_account(account_uuid,
    ...
    if response is not None:
        ...
        if response.status_code == 400:
            ...
            account_model_edit_error = response_dict['account_model_edit_error']
            print('[x] Account instance not edited due to error during model instance editing:')
            print('\t' + '•' + account_model_edit_error)
        elif response.status_code == 500:
...
def submit_journal_entry(account_uuid,
    ...
    payload = dict(account_uuid=account_uuid,
                   ...
                   journal_entry_datetime=str(payload_datestamp),
                   tz=tz,
                   dependencies=dependencies,
                   nonce=nonce,
                   submit_batch=submit_batch,
    ...
    if response is not None:
        ...
        if response.ok:
            ...
            print('[x] Journal Entry was not processed due to transaction validation error')
        elif response.status_code == 422:
            journal_entry_model_create_error = response_dict['journal_entry_model_create_error']
            print('[x] Journal entry model not created due to error in model instance creation:')
            print('\t' + '•' + journal_entry_model_create_error)
        elif response.status_code == 500:
...
def submit_transaction(side_a_debit_account_uuid,
    ...
    config = Config()
    sawtooth_data_uri = config.settings['endpoints']['hydrosledger']['bank']
    sawtooth_data_uri += 'transaction/submit/'
    payload = dict(side_a_debit_account_uuid=side_a_debit_account_uuid,
                   ...
                   transaction_datetime=str(payload_datestamp),
                   tz=tz,
                   finx_key=config.settings['identity']['finx_key'])
    try:
        response = requests.get(sawtooth_data_uri, params=payload)
    ...
    if response is not None:
        response_dict = json.loads(response.text)
        if response.ok:
            journal_entry_param_set_list = json.loads(response_dict['batch_data'])
            print('[✓] Transaction "{}" processed'.format(journal_entry_param_set_list[0]['transaction_uuid']))
            for journal_entry_param_set in journal_entry_param_set_list:
                account_uuid = journal_entry_param_set['account_uuid']
                journal_entry_uuid = journal_entry_param_set['journal_entry_uuid']
            ...
            if 'journal_entry_model_create_error' in response_dict:
                journal_entry_model_create_error = response_dict['journal_entry_model_create_error']
                print('[x] Journal entry model instance not created due to error during model instance creation:')
                print('\t' + journal_entry_model_create_error)
            if 'transaction_model_create_error' in response_dict:
                transaction_model_create_error = response_dict['transaction_model_create_error']
                print('[x] Transaction model instance not created due to error during model instance creation:')
                print('\t' + '•' + transaction_model_create_error)
        elif response.status_code == 500:
    ...
    if response is not None:
        if response.ok:
            ...
            if 'journal_entry_model_create_error' in response_dict:
                ...
                print('[x] Journal entry model not created due to error in model instance creation:')
                print('\t' + journal_entry_model_create_error)
            if 'account_model_edit_error' in response_dict:
                account_model_edit_error = response_dict['account_model_edit_error']
                print('[x] Account instance not edited due to error during model instance editing:')
                print('\t' + '•' + account_model_edit_error)
        elif response.status_code == 500:
