...
class Mongo(object):
    ...
    def get_list_news(self, list_id_news):
        selected_collection = self.connection['News']
        result = selected_collection.find( { "_id" : { "$in": list_id_news } }, {"_id":1, "img_path":1, "category_title":1, "news_source":1, "url":1, "date":1, "article":1, "tags":1, "category_id":1, "place":1} )
        return result

    def add_in_lifo(self, id_user, id_news, action, timestamp):
        selected_collection = self.connection['LifoNews']
        result = None
        if action == 1:
            ...
            if result>0:
                selected_collection.update({"_id":int(id_user), "list_news.id":int(id_news)},{"$set":{"list_news.$.add":1,"list_news.$.id":int(id_news),"list_news.$.timestamp":timestamp}},upsert=True)
            else:
                selected_collection.update({"_id":int(id_user)},{"$push":{"list_news":{"id":int(id_news),"add":1,"del":-1,"timestamp":timestamp}}},upsert=True)
        else:
            ...
            if result > 0:
                ...
                selected_collection.update({"_id": int(id_user)}, {
                    "$push": {"list_news": {"id": int(id_news), "add": 1, "del": 1, "timestamp": timestamp}}},
                                           upsert=True)

    def remove_in_lifo(self):
        selected_collection = self.connection['LifoNews']
        selected_collection.update({}, {"$pull":{"list_news": {"del":1}}}, multi=True)


    def get_in_lifo(self, user_id, num_page, page_size):
        selected_collection = self.connection['LifoNews']
        result = selected_collection.find({"_id": int(user_id)},{"list_news":1, "_id":0})
