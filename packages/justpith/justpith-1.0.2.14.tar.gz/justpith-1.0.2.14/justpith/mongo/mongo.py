...
class Mongo(object):
    ...
    def add_in_lifo(self, id_news, action, timestamp):
        selected_collection = self.connection['LifoNews']
        result = None
        if action == 1:
            result = selected_collection.update_one({"_id": int(id_news)}, {"$set":{"add":1, "timestamp": timestamp}}, upsert=True )
        else:
            result = selected_collection.update_one({"_id": int(id_news)}, {"$set": {"del":1, "timestamp": timestamp}}, upsert=True )

    def remove_in_lifo(self):
        selected_collection = self.connection['LifoNews']
        selected_collection.remove({"del":1})
