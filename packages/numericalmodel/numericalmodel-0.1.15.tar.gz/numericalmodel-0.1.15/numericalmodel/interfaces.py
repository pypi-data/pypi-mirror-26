...
class InterfaceValue(utils.ReprObject):
    ...
    def value(self):
        ...
        try:
            ...
            try:
                ...
                raise IndexError("{}: no values recorded yet".format(self.name))
    ...
    def value(self,newvalue):
        ...
        if np.any(ind): # time already there?
            try:
                ...
                # logging.debug("time {t} already there, "
            ...
            # logging.debug("time {t} not yet there, "
    ...
    def next_time(self):
        ...
        try:
            assert next_time >= self.time, \
                "next_time ({}) has to be later than current time ({})".format(
                next_time,self.time)
        except IndexError:
            pass
        return next_time
    ...
    def merge_accumulated(self):
        ...
        if len(self.accumulated_values) > 0:
            logging.debug("merging {} accumulated values...".format(
                len(self.accumulated_values)))
            try:
                self.times = \
...
class Observation(InterfaceValue):
    """
    Class for Observations
    """
    @property
    def _default_id(self):
        return "unnamed_observation"
...
class SetOfObservations(SetOfInterfaceValues):
    """
    Class for a set of observations
    """
    def __init__(self,elements = []):
        utils.SetOfObjects.__init__( self,
            elements = elements,  # these variables
            element_type = Observation, # only Observations belong here
            )
