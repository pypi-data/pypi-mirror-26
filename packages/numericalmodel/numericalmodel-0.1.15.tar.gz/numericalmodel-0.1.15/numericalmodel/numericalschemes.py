...
class NumericalScheme(utils.ReprObject):
    """
    ...
    def integrate(self, time = None, until = None):
        ...
        logging.debug("current maximum timestep is {}".format(
            current_max_timestep))
        if until is None: until = time + current_max_timestep
        logging.debug("integrating until time {}".format(until))
        time_now = time
        while time_now < until:
            logging.debug(("current time {} is smaller than until time {}"
            ...
            if time_left > current_max_timestep: # full max_timestep fits
                ...
                timestep = time_left
            logging.debug(("integrate one step from time {} with "
                "timestep {}").format(time_now,timestep))
            # integrate one step
            self.integrate_step( time = time_now, timestep = timestep )
            time_now += timestep
        logging.debug(("reached until time {}").format(time_now))
    ...
    def integrate_step(self, time = None, timestep = None):
        ...
        # logging.debug("{} tendency: {}".format(var.id,tend))
        # logging.debug("var() = {}".format(var()))
        # logging.debug("var({}) = {}".format(time,var(time)))
        # logging.debug("var.time = {}".format(var.time))
        # logging.debug("var(var.time) = {}".format(var(var.time)))
        new = var(time) + tend
...
class RungeKutta4(NumericalScheme):
    ...
    def step(self, time = None, timestep = None, tendency = True):
        ...
        k1 = timestep * d(time = time,      variablevalue = cur )
        k2 = timestep * d(time = half_time, variablevalue = cur + k1 / 2 )
        k3 = timestep * d(time = half_time, variablevalue = cur + k2 / 2 )
        k4 = timestep * d(time = next_time, variablevalue = cur + k3 )
...
class SetOfNumericalSchemes(utils.SetOfObjects):
    ...
    ### Methods ###
    ###############
    def assert_plan_valid(self, plan):
        """ 
        Check a given :any:`plan` for validity and completeness and raise an
        error if 

        Args:
            plan (list): the scheme plan

        Returns:
            bool : ``True`` if the plan is valid

        Raises:
            ValueError : If anything is wrong with the plan
        """
        logging.debug("### Testing the plan ###")
        simulation = {ivid:[0] for ivid in self.keys()}
        for stage in plan: # go over each stage
            ivid, steps = stage
            try:
                self[ivid]
            except KeyError:
                raise ValueError("The plan mentions {} but there is no " 
                    "equation for {}!".format(ivid,ivid))
            if not steps:
                raise ValueError("The plan for {} is empty!".format(ivid))
            if max(steps) != 1 or steps[-1] != 1:
                raise ValueError("The plan for {} does not end with a " 
                "unified step of 1. Try something like {}".format(
                ivid,steps+[1]))
            # we now want to integrate 'ivid' to 'steps'
            logging.debug(
                "We currently have simulated this: {}".format(simulation))
            logging.debug(
                "We now want to integrate {} to {}".format(ivid,steps))
            needed_steps_uni = self[ivid].needed_timesteps(1)
            logging.debug( "{} needs the unified timesteps {}".format(
                ivid, needed_steps_uni))
            for step in steps: # check data availability of each step
                logging.debug(
                    "We now want to integrate {} by {}".format(ivid,step))
                needed_steps = step * needed_steps_uni
                logging.debug( "For a step of {} {} needs the " 
                    "timesteps {}".format(step,ivid,needed_steps))
                # check the equation input
                for in_ivid in self[ivid].equation.input.keys():
                    logging.debug("{} depends on {}".format(ivid,
                        in_ivid))
                    if in_ivid == ivid: # dependencies on itself 
                        logging.debug("{} depends on itself. No need " 
                            "to check the availability of itself.".format(
                            ivid))
                    elif in_ivid in self: # we model this ivid
                        logging.debug("For {} we currently have the " 
                            "steps {}".format(in_ivid,simulation[in_ivid]))
                        for needed_step in needed_steps:
                            if needed_step in simulation[in_ivid]:
                                logging.debug("We have the step {} for " 
                                    "{}".format(needed_step,in_ivid))
                            else:
                                msg = ("{} integration: step {} of "
                                    "{} is not available. Try to change the "
                                    "related fallback_plan element to "
                                    "something like {}".format(
                                        ivid,needed_step,in_ivid,
                                        [in_ivid,[x for x in needed_steps_uni \
                                            if x != 0]],
                                        ))
                                logging.debug(msg)
                                raise ValueError(msg)
                    else: # we don't model this ivid
                        logging.debug("We do not simulate {} so we " 
                            "assume it is always available".format(in_ivid))
                        pass # assume unmodeled ivids to be always available
                simulation[ivid].append(step) # we integrated 'ivid' to step
            logging.debug("We successfully integrated {} to {}".format(
                ivid,steps))
        logging.debug("Final test simulation: {}".format(simulation))
        logging.debug("### Plan test was successfull ###")
        return True
    ...
    def integrate(self, start_time, final_time):
        ...
        plan = self.plan # get the plan
        while current_time < final_time:
            logging.debug("current time {} is smaller than "
                "final time {}".format(current_time, final_time))
            # timestep of most dependent equation
            biggest_timestep = self[plan[-1][0]].max_timestep
            logging.debug("timestep of last scheme: {}".format(
                biggest_timestep))
        ...
        logging.info("end of integration")
