...
class SaveThread(QThread):   
    ...
    def generate_video(self,jpgs,fps,size):
        ...
        if music_file  and ffmpeg_file:
            ...
            os.system('{ffmpeg} -i {video} -i {audio} -vcodec copy {output}'.format(video=video_name,audio=audio_name,output=self.filename,ffmpeg=ffmpeg_file))
...
class GifMaker(QWidget):
    ...
    def editGif(self,filename):
        if self.videoclip is None:
            self.videoclip = VideoFileClip(filename)
        else:
            self.videoclip.__init__(filename)
        cf=self.videoclip
