...
class SaveThread(QThread):   
    ...
    def generate_gif(self,jpgs,fps,size):
        # t=stack(jpgs,0)
        # print(t.shape,'gif shape')
        # w=sio.vwrite(self.filename,t,outputdict={'-r':str(fps),'-s':'{w}x{h}'.format(w=size[1],h=size[0])},inputdict={'-r':str(fps)})
    ...
    def generate_video(self,jpgs,fps,size):
        inputdict={'-r':str(fps)}
        outputdict={'-r':str(fps),'-s':'{w}x{h}'.format(w=size[1],h=size[0])}
        music_file=self.parent.text_music_label.text().strip()
        n=music_file.find('|')
        duration=len(jpgs)/fps
        if n!=-1:
            start_text=music_file[n+1:]
        else:
            start_text=None

        if start_text:
            music_file=music_file[:n]
            try:
        ...
        if start_time != 0:
            ...
            if path.isfile(outfile):
                os.remove(outfile)
            os.system('ffmpeg -i "{infile}" -ss {start} -t  {duration} -acodec copy {outfile}'.format(infile=music_file,outfile=outfile,start=start_time,duration=duration))
            music_file=outfile
        else:
            outfile=None
        print('music',music_file)
        if path.isfile(music_file):
            outputdict={'-r':str(fps),'-i':music_file,'-t':str(duration), '-s':'{w}x{h}'.format(w=size[1],h=size[0])}
        print(outputdict)
        s=sio.FFmpegWriter(self.filename,inputdict=inputdict,outputdict=outputdict)
        for i in jpgs:
            s.writeFrame(i)
        s.close()
        if outfile:
    ...
    def generate_image(self,size):
        ...
        if outimg.shape[-1]==4 and path.splitext(self.filename)[1].lower() in ['.jpg','.jpeg']:
            outimg=outimg[:,:,:3]
        sio.vwrite(self.filename,outimg,outputdict={'-s':'{w}x{h}'.format(w=size[1],h=size[0])})
...
class GifMaker(QWidget):
    ...
    def after_run(self):#用于图形界面启动后导入模块以及检查更新
        ...
        from os import path
        global imageio,CutImageWidget,re,os,path,sio,misc
    ...
    def video2gif(self,filename):
        ...
        if fps.find('/')!=-1:
            ...
            fps=float(fps)
        k=VideoCutter().get_info(duration,fps)
        print(k)
        if k is None:return
        else:
            start,end,fps=k
            if end<start or fps<=0:return
        videoreader=sio.vreader(filename,outputdict={'-ss':str(start),'-t':str(end-start),'-r':str(fps)})
        self.videoThread=VideoReadThread()
