...
without_cv2=True #默认不采用cv2
without_audio=False #默认采用添加音频
temp_nn=14 #标记 只增加不减小
global_ndarry_max=2*1024*1024*1024 #设置ndarray占用的最大内存
del tempfile
...
class SaveThread(QThread):   
    ...
    def get_music_file(self,duration):
        ...
        return outfile
    def generate_video_cv2(self,jpgs,fps,size):
        writer=cv2.VideoWriter(self.filename,cv2.VideoWriter.fourcc(*'MPEG'),fps,(size[1],size[0]))
        for i in jpgs:
            i=resizeimg(ndarryfile(i),size)
            i=i[:,:,(2,1,0)]
            writer.write(i)
        writer.release()
        return None
    def generate_video(self,jpgs,fps,size):
        if without_cv2:
            ...
            self.generate_video_cv2(jpgs,fps,size)
...
class GifPreview(QDialog):   #预览gif
    ...
    def update_image(self):
        ...
        x=ndarryfile(self.jpgs[self.nn])
    ...
    def preview(self,parent):
        ...
        if isinstance(parent,tuple):
            ...
            self.fps=float(parent.text_fps_label.text())
            self.jpgs=parent.getNdImages()
            shape=int(parent.text_h_label.text()),int(parent.text_w_label.text())
...
class GifMaker(QWidget):
    ...
    def after_run(self):#用于图形界面启动后导入模块以及检查更新
        ...
        # import re,os
        # from os import path
        # global imageio,CutImageWidget,re,os,path
        # if not without_cv2:
        #     import cv2
        #     global cv2
    ...
    def test(self):  #测试按钮
        self.__rotate(270)
def main():
    global ffmpeg_file,without_cv2,without_audio,global_ndarry_max
    gif_file=None
    ...
    for i,v in enumerate(sys.argv):
        ...
        if v=='--without_audio':
            without_audio=True
        if v=='--ndarray_max':
            global_ndarry_max=int(sys.argv[i+1])
        if v[-3:].lower()=='gif' and path.isfile(v):
            gif_file=v 
            break
    else:
        ...
        return
    app = QApplication(sys.argv)
    x=imageio.get_reader(v,'ffmpeg')
    meta=x.get_meta_data()
    fps=meta['fps']
    size=meta['size']
    jpgs=[i for i in x]
    preview = GifPreview('SongZ GIF').preview((jpgs,fps,(size[1],size[0])))
