...
def resizeimg(img,size):
	...
	pimg.resize((size[1],size[0]))
	x=np.fromstring(pimg.tobytes(),dtype='uint8')
...
class ImageLabel(QLabel):
    ...
    def setndimage(self,name): #Filename 可以是文件名 也可以是 ndarry
        ...
        print(self.father.ndarry_n,self.father.ndarry_max)
...
class GifMaker(QWidget):
    ...
    def __rotate(self,angle=90):
        ...
        op=self.after_open
        if angle==90:
            l=[op((ndarryfile(i)[:,::-1].transpose((1,0,2)),)) for i in ndimages]
        elif angle==180:
