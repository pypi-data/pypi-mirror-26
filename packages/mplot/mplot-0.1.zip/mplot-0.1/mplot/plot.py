...
class chart():
    def __init__(self,x,y,legend = None,color = None, alpha = 1,filledge = None,hatch = None,secondaxis = False,stacked = False,edgecolor = None,fill = True,thickness = 1,markthickness = None):
        '''
        x can be 0,1,2 or Cat, Dog, Rabbit
        '''
        self.name = 'chart'
        self.func = makechart
        self.rawx = x
...
def getcategories(objects):
    ...
    i = 1
    for obj in objects:
        if obj.name not in ['chart','boxplot']:
            continue
        if obj.rawx == []:
            continue
        for x in obj.rawx:
...
def makegridlines_sub(gridlinestyle,key,default):
    if not gridlinestyle.has_key(key):
        return default
