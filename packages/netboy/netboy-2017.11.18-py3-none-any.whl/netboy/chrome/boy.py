...
class ChromeBoy:
    ...
    def new_page(self, **kwargs):
        ...
        if result is None or not result.get('value') or result_except_flag:
            ...
            try:
                ...
                print(e, type(e), 'error when getting result from response again: {}'.format(url))
                r = {}
                r['state'] = 'error'
    ...
    def chrome(self):
        browser_socket = websocket.create_connection(self._browser_url,  # + '/9d220c5c-7eba-4927-a95c-dfd56c689779',
                                                     timeout=self._socket_timeout)
        return ChromeInterface(browser_socket, self._host, self._port), browser_socket
...
class ConcurrentBoy:
    ...
    def _run_simple(self, data, info):
        ...
        for chunked_data in grouper_it(data, 4):
            info['max_workers'] = 2
            result = self._run(chunked_data, info, 'process')
            results.extend(result)
