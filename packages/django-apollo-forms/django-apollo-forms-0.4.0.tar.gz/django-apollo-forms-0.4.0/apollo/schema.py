...
class CreateForm(graphene.Mutation):
    ...
    def mutate(root, info, **args):
        ...
        form, form_created = Form.objects.update_or_create(name=name, defaults=dict(
            ...
            submission_contacts=[c.strip() for c in data['submission_contacts'].split(',')] if 'submission_contacts' in data else None,
            layout=layout
        ))
...
class Query(graphene.AbstractType):
    ...
    def resolve_form_field_template(self, info):
        id = info.get('id')
    ...
    def resolve_form_field(self, info):
        id = info.get('id')
    ...
    def resolve_all_form_submissions(self, info):
        return FormSubmission.objects.filter(form_id=info.get('form_id')).select_related('form').all()

    def resolve_form_submission(self, info):
        id = info.get('id')
