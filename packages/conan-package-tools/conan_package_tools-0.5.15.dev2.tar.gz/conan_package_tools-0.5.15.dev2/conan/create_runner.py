...
class TestPackageRunner(object):
    def __init__(self, profile_text, username, channel, mingw_installer_reference=None, runner=None,
        ...
        self.conan_home = self.client_cache.conan_folder
        self.data_home = self.client_cache.store
...
class DockerTestPackageRunner(TestPackageRunner):
    def __init__(self, profile_text, username, channel, mingw_ref=None, runner=None,
        ...
        self.sudo = "sudo" if platform.system() == "Linux" else ""
    ...
    def run(self, pull_image=True):
        ...
        if pull_image:
            ...
            command = "%s docker run --name conan_runner %s /bin/sh -c " \
                      ...
                      "--upgrade" % (self.sudo, self.docker_image, package_tools_version)
            ...
            self._runner("%s docker commit conan_runner %s" % (self.sudo, self.docker_image))
            self._runner("%s docker rm conan_runner" % self.sudo)
        ...
        command = "%s docker run --rm -v %s:/home/conan/project -v " \
                  "%s:/home/conan/.conan -it %s %s /bin/sh -c \"" \
                  "rm -rf /home/conan/.cache/pip %% " \
                  "rm -f /home/conan/.conan/conan.conf && cd project && " \
                  "rm -r /home/conan/.conan/profiles/default && " \
                  "conan profile new default --detect && " \
                  "run_create_in_docker\"" % (self.sudo, os.getcwd(), self.conan_home,
    ...
    def pull_image(self):
        ...
        self._runner("%s docker pull %s" % (self.sudo, self.docker_image))
