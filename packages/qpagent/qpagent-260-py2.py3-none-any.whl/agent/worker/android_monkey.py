...
class AndroidMonkey(BaseWorker):
    ...
    def start_worker(self):
        ...
        self.uninstall_app(self.device_id, "me.ele")
        self.install_app(self.device_id, self.wkdir + '/eleme_android.apk')
        log_file_name_with_location = self.generate_log_file_name_with_location(self.device_id)
        ...
        if mail_content == '':
            ...
            ftp_client.upload(self.log_file_dir, str(self.task_id))
            mail_content = 'http://10.12.38.246:8000/report/' + str(self.task_id) + '/' + str(self.task_id) + '.txt'

        server_api.add_results_for_cases(self.run_id, self.case_id, mail_content, app)
        self.complete()
    ...
    def generate_log_file_name_with_location(self, device_id):
        # 获取当前 Log 存储路径
    ...
    def deal_with_log(self, log_file_name_with_location, monkey_duration):
        # analyze with log:
