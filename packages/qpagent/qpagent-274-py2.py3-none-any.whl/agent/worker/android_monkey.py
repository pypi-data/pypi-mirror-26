...
class AndroidMonkey(BaseWorker):
    ...
    def start_worker(self):
        ...
        self.install_app(self.device_id, self.android_monkey_task_dir + '/eleme_android.apk')
    ...
    def start_monkey(self, device_id):
        ...
        cmd_monkey = 'adb -s {} shell monkey -p {} --throttle {} --pct-touch {} --pct-motion {} --pct-syskeys {} --ignore-crashes --ignore-timeouts -v -v 5000 1>{}.txt 2>{}.txt'.format(
            device_id, self.grand_id, self.throttle, self.pct_touch, self.pct_motion, self.pct_syskeys,
            self.android_monkey_log, self.android_monkey_error)
