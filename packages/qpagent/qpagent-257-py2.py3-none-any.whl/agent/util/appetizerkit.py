...
class AppToolKit(object):
    ...
    def get_devices(self):
        try:
            devices = self.app.devices.list()
        except Exception:
            return []
        return devices

    def install(self, devices, apk_path):
        self.app.devices.control(devices, 'install', apk_path)

    def uninstall(self, devices, apk_path):
        self.app.devices.control(devices, 'uninstall', apk_path)

    def start_record(self, trace_file, device_serial_number):
    ...
    def stop_record(self, record_task):
        record_task.stop()

    def start_replay(self, trace_file, serialnos):
        appetizer = Appetizer(toolkit=os.path.dirname(__file__) + '/darwin/appetizer')
        replay_task = appetizer.trace.replay(os.path.dirname(__file__) + "/trace/" + trace_file, serialnos)
        replay_task.wait()


class iOSToolKit(object):
    def get_devices(self):
        try:
            devices = []
