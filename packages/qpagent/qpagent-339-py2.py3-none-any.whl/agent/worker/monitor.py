...
class Monitor(BaseWorker):
    ...
    def start_worker(self):
        ...
        logger.info(u'当前目录：>>>>>>>>>>' + os.getcwd())
        logger.info(
            u'开始插桩：>>>>>>>>>>' + self.android_monitor_task_dir + '/eleme_android.apk' + '|' + self.android_monitor_task_dir + '/instrumented.apk')

        insights.process(self.android_monitor_task_dir + '/eleme_android.apk',
                               self.android_monitor_task_dir + '/instrumented.apk')

        logger.info(
            u'开始安装：>>>>>>>>>>' + self.android_monitor_task_dir + '/instrumented.apk' + '|' + str(self.device_id))

        insights.install(self.android_monitor_task_dir + '/instrumented.apk', {self.device_id})

        logger.info(
            u'开始清理log：>>>>>>>>>>' + self.android_monitor_task_dir + '/instrumented.apk' + '|' + str(self.device_id))

        insights.clearlog(self.android_monitor_task_dir + '/instrumented.apk', {self.device_id})
        ...
        logger.info(
            u'开始分析：>>>>>>>>>>' + self.android_monitor_task_dir + '/instrumented.apk' + '|' + str(self.device_id))

        key = insights.analyze(self.android_monitor_task_dir + '/instrumented.apk', {self.device_id})
        ...
        if self.run_id:
            server_api.add_results_for_cases(self.run_id, self.case_id, mail_content, TestRunStatus.failed.value,
                                             self.app)
        server_api.send_mail(self.notice_account, '性能测试报告', '饿了么android_性能测试', self.app,
