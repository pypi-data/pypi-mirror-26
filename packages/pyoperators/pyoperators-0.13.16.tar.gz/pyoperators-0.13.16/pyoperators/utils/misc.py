...
__all__ = ['all_eq',
           ...
           'cast',
           'complex_dtype',
           'complex_intrinsic_dtype', # deprecated
           'deprecated',
           'first',
           'first_is_not',
           'float_dtype',
           'float_intrinsic_dtype', # deprecated
           ...
           'isscalar', # deprecated
           'isscalarlike',
           'izip_broadcast', # deprecated
           'last',
           'last_is_not',
           'least_greater_multiple',
           'merge_none',
           'native_complex_dtype',
           'native_float_dtype',
           'ndarraywrap',
           'one',
           'omp_num_threads',
...
def complex_dtype(dtype):
    """
    Return the complex data type associated with a numeric data type.

    If the input is a float or complex data type, the number of bytes used to
    encode the values is preserved if possible. The native endianness is
    always returned.
    Unlike native_complex_dtype, this function is intended to be used in the
    numpy ecosystem, where non-native complex data types are handled.
    ...
    if dtype.kind == 'c':
        return np.dtype('c{0}'.format(dtype.itemsize))
    if dtype.kind not in 'biuf':
        raise TypeError('Non numerical data type.')
    if dtype.kind != 'f' or dtype.itemsize == 2:
        return np.dtype(complex)
    return np.dtype('c{0}'.format(dtype.itemsize * 2))


def float_dtype(dtype):
    """
    Return the float data type associated with a numeric data type.

    If the input is a float or complex data type, the number of bytes used to
    encode the values is preserved. The native endianness is always returned.
    Unlike native_float_dtype, this function is intended to be used in the numpy
    ecosystem, where non-native float data types are handled.
    ...
    dtype = np.dtype(dtype)
    if dtype.kind == 'f':
        return np.dtype('f{0}'.format(dtype.itemsize))
    if dtype.kind not in 'biuc':
        raise TypeError('Non numerical data type.')
    if dtype.kind != 'c':
        return np.dtype(float)
    return np.dtype('f{0}'.format(dtype.itemsize // 2))


def float_or_complex_dtype(dtype):
    """
    Return the float or complex data type associated with a numeric data type.

    If the input is a float or complex data type, the number of bytes used to
    encode the values is preserved. The native endianness is always returned.
    This function is intended to be used in the numpy ecosystem where non-
    native float and complex data types are handled.
    ...
    if dtype.kind in 'biu':
        return np.dtype(float)
    if dtype.kind in 'fc':
        return np.dtype('{0}{1}'.format(dtype.kind, dtype.itemsize))
    raise TypeError('Non numerical data type.')
...
def native_complex_dtype(dtype):
    """
    Return the native complex dtype (complex64 or complex128) associated
    ...
    Example
    -------
    >>> native_complex_dtype(int)
    dtype('complex128')
    >>> native_complex_dtype(np.float32)
    dtype('complex64')
    >>> native_complex_dtype('>f16')
    dtype('complex128')
    >>> native_complex_dtype(np.float128)
    dtype('complex128')
    >>> native_complex_dtype(np.complex256)
    dtype('complex128')
...
@deprecated("use 'native_complex_dtype' instead.")
def complex_intrinsic_dtype(dtype):
    return native_complex_dtype(dtype)


def native_float_dtype(dtype):
    """
    Return the native floating dtype (float32 or float64) associated with a
    numeric dtype.
    ...
    Example
    -------
    >>> native_float_dtype(int)
    dtype('float64')
    >>> native_float_dtype(np.float32)
    dtype('float32')
    >>> native_float_dtype('>f8')
    dtype('float64')
    >>> native_float_dtype(np.float128)
    dtype('float64')
    >>> native_float_dtype(np.complex256)
    dtype('float64')
...
@deprecated("use 'native_float_dtype' instead.")
def float_intrinsic_dtype(dtype):
    return native_float_dtype(dtype)
