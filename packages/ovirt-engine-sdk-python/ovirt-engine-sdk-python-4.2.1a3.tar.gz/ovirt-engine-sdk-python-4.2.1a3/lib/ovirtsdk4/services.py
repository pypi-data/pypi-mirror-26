...
class HostService(MeasurableService):
    ...
    def install(
        ...
        `image`:: When installing an {product-name} node a image ISO file is required.

        `async`:: Indicates if the installation should be performed asynchronously.

        `deploy_hosted_engine`:: When set to `true` it means this host should also deploy the self-hosted engine components. A missing value
        ...
        `undeploy_hosted_engine`:: When set to `true` it means this host should un-deploy the self-hosted engine components and this host will
    ...
    def upgrade(
        ...
        `image`:: The image parameter specifies path to image, which is used for upgrade. This parameter is used only to
        upgrade Vintage Node hosts and it is not relevant for other hosts types.

        `reboot`:: Indicates if the host should be rebooted after upgrade.
