...
class PasswordInputSettings(TLObject):
    ...
    def __bytes__(self):
        assert (self.new_salt and self.new_password_hash and self.hint) or (not self.new_salt and not self.new_password_hash and not self.hint), 'new_salt, new_password_hash, hint parameters must all be False-y (like None) or all me True-y'
        return b''.join((
            b'\xebm\x91\x86',
