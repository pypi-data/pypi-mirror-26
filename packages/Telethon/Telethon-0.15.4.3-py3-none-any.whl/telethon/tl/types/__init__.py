...
class BotInlineResult(TLObject):
    ...
    def __bytes__(self):
        assert (self.content_url and self.content_type) or (not self.content_url and not self.content_type), 'content_url, content_type parameters must all be False-y (like None) or all me True-y'
        assert (self.w and self.h) or (not self.w and not self.h), 'w, h parameters must all be False-y (like None) or all me True-y'
        return b''.join((
            b'\xb9\xae\xeb\x9b',
...
class Channel(TLObject):
    ...
    def __bytes__(self):
        assert (self.restricted and self.restriction_reason) or (not self.restricted and not self.restriction_reason), 'restricted, restriction_reason parameters must all be False-y (like None) or all me True-y'
        return b''.join((
            b'\x1cK\xb4\x0c',
...
class ChannelFull(TLObject):
    ...
    def __bytes__(self):
        assert (self.kicked_count and self.banned_count) or (not self.kicked_count and not self.banned_count), 'kicked_count, banned_count parameters must all be False-y (like None) or all me True-y'
        assert (self.migrated_from_chat_id and self.migrated_from_max_id) or (not self.migrated_from_chat_id and not self.migrated_from_max_id), 'migrated_from_chat_id, migrated_from_max_id parameters must all be False-y (like None) or all me True-y'
        return b''.join((
            b'\x81T\xafv',
            struct.pack('<I', (8 if self.can_view_participants else 0) | (64 if self.can_set_username else 0) | (128 if self.can_set_stickers else 0) | (1024 if self.hidden_prehistory else 0) | (1 if self.participants_count else 0) | (2 if self.admins_count else 0) | (4 if self.kicked_count else 0) | (4 if self.banned_count else 0) | (16 if self.migrated_from_chat_id else 0) | (16 if self.migrated_from_max_id else 0) | (32 if self.pinned_msg_id else 0) | (256 if self.stickerset else 0) | (512 if self.available_min_id else 0)),
            struct.pack('<i', self.id),
            ...
            b'' if not self.available_min_id else (struct.pack('<i', self.available_min_id)),
        ))
...
class Config(TLObject):
    ...
    def __bytes__(self):
        assert (self.suggested_lang_code and self.lang_pack_version) or (not self.suggested_lang_code and not self.lang_pack_version), 'suggested_lang_code, lang_pack_version parameters must all be False-y (like None) or all me True-y'
        return b''.join((
            b'd\t\x84\x9c',
...
class RecentMeUrlChat(TLObject):
    ...
    def __init__(self, url, chat_id):
        ...
        Constructor for RecentMeUrl: Instance of either RecentMeUrlUser, RecentMeUrlChat, RecentMeUrlStickerSet, RecentMeUrlChatInvite, RecentMeUrlUnknown.
        """
        super().__init__()
...
class RecentMeUrlChatInvite(TLObject):
    ...
    def to_dict(self, recursive=True):
        return {
            'url': self.url,
            'chat_invite': (None if self.chat_invite is None else self.chat_invite.to_dict()) if recursive else self.chat_invite,
        }
...
class User(TLObject):
    ...
    def __bytes__(self):
        assert (self.bot and self.bot_info_version) or (not self.bot and not self.bot_info_version), 'bot, bot_info_version parameters must all be False-y (like None) or all me True-y'
        assert (self.restricted and self.restriction_reason) or (not self.restricted and not self.restriction_reason), 'restricted, restriction_reason parameters must all be False-y (like None) or all me True-y'
        return b''.join((
            b'\xc3\xf4\x13.',
...
class WebPage(TLObject):
    ...
    def __bytes__(self):
        assert (self.embed_url and self.embed_type) or (not self.embed_url and not self.embed_type), 'embed_url, embed_type parameters must all be False-y (like None) or all me True-y'
        assert (self.embed_width and self.embed_height) or (not self.embed_width and not self.embed_height), 'embed_width, embed_height parameters must all be False-y (like None) or all me True-y'
        return b''.join((
            b'\xbc\xb4\x07_',
