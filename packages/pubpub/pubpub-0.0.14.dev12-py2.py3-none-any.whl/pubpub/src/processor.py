...
class Processor():
  def __init__(self, **opts):
    ...
    self.working_directory = get_working_directory(
        opts.get('working_directory'), opts.get('base_dir', self.build_dir))
...
\usepackage{lipsum}
  ...
  def to_latex(self):
    ...
    latex_filename = self.build_dir + 'merged.tex'
  ...
  def process_notebook_html(self, filename, i=0, **opts):
    ...
    self.copy_assets(body,
                     os.path.dirname(filename), os.path.dirname(html_filename))
  ...
  def run_pdflatex(self, filename, output_filename):
    ...
    args = ['pdflatex', '-output-directory=%s' % output_dir, filename]
  ...
  def copy_assets(self, md_filename, src_file, dest_dir):
  ...
  def cleanup(self):
    ...
    if not self.build_dir is not None and self.build_dir is not '' and self.build_dir is not '/':
