...
class Logging:
    ...
    def error(self, *args):
        print('ERROR:' + self._filename + ':', *args)
        return args
...
# from https://stackoverflow.com/questions/2352181/how-to-use-a-dot-to-access-members-of-dictionary/23689767#23689767 # noqa
class DotDict(dict):
...
# defaultdict that doesn't set the default value when it is read
#
...
def unpack(pos):
    return lambda x: x[pos]


def get_output_dir(folder='/output', file=None):
    try:
...
def save_csv_file(filename, histories, headers=None, output_dir=None,
                  csv_sep=';'):
    if headers:
