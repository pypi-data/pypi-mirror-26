...
def wordfill(words, fontname, cloudsize=(400, 400), uppercase=False, fontsize=84, minfontsize=7, colours=multi, background='black', step=1, fontstep=1, rotated=0.5, mask=None):
    """
    Generate the word cloud image
    ...
    while True:
        ...
        while True:
            # get font at current size and measure
            font = ImageFont.truetype(fontname, fontsize_cur)
            size = ImageDraw.Draw(im).textsize(word, font=font)
            searchsize = (size[1], size[0]) if rotate else size
            # find possible spots
            if step > 1:
                spots = ii_search(ii, searchsize[0], searchsize[1], step=step)
            else:
                spots = ii_search_nostep(ii, searchsize[0], searchsize[1])
            if len(spots) > 0:
                # spots available so create text patch and place randomly
                im1 = Image.new("RGB", (size[0] + 10, size[1] + 10), background)
                alpha = Image.new("L", im1.size, 0)
                ...
                # no spots, decrease font size and try again
                fontsize_cur -= fontstep
            # return when we hit the min font size
                if fontsize_cur <= minfontsize:
                    return im
    # return when we run out of words
    return im