...
def load_all_bedrocks_from_github():
    ...
    # Load each Bedrock and create it in TM1 if it doesn't exist yet
    for bedrock in all_bedrocks:
        url = 'https://raw.githubusercontent.com/MariusWirtz/bedrock/master/json/{}'.format(bedrock)
        process_as_json = requests.get(url).text
        yield Process.from_json(process_as_json)
