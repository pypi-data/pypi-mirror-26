...
class NodelistTest(unittest.TestCase):
    def setUp(self):
        ...
        Initialize Tests
        """
        nodelist_file = open(os.path.dirname(os.path.realpath(__file__)) + os.sep + "testfiles/yanic_example_town_nodes.json", "r")
        self.nodelist = parse_nodes_json(nodelist_file.read())
        nodelist_file.close()

        nodegraph_file = open(os.path.dirname(os.path.realpath(__file__)) + os.sep + "testfiles/yanic_example_town_graph.json", "r")
