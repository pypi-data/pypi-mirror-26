        ...
        if not json_lines:
            for node in state['dag']['nodes']:
                ...
                if not p:
                    ...
                    id = p['proxydetails'].get('task_id','n/a')
                click.secho('node: {} id: {} state: {}'.format(node['name'], id, click.style(node['state'], fg = colors.get(node['state']))))
        else:
            click.secho(json.dumps(state))
    if topic == 'log':
        for i in r.iter_lines():
            if not json_lines:
