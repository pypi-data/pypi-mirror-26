...
def progress_bar(progress, total=1):
    """
    ...
    """
    barLength = 10  # length of the progress bar
    status = ""
    ...
    if progress/total >= 1:
        progress = 1
        status = "Done...\r\n"
    block = int(round(barLength*(progress/total)))
    text = "\rPercent: [{0}] {1}% {2}".format("#"*block + "-"*(barLength-block), progress/total*100, status)
