...
def verify_request(decorated_method, port=9999):
    ...
    error(report)
    error(report[0])
    error(report[0].get('body'))
    actual_request = PactRequest(
        path=report[0].get('path'),
        query=report[0].get('query'),
        method=report[0].get('method'),
        body=report[0].get('body'),
        headers=report[0].get('headers')
    )
    ...
    debug(actual_request.body)
    debug(actual_request.stocazzo())
    debug('')
    debug(mock_response)
    debug('### RUNNER ###')
    ...
    if type(matching_result) is Right:
        out = {
            ...
            'request': actual_request.stocazzo(),
