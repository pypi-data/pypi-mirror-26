...
def log_providers_test_results(test_results):
    if type(test_results) is Left:
        ...
        debug(str(len(test_results.value)) + ' available test results.')
        for test_result in test_results.value:
            if type(test_result) is Left:
                error(test_result.value)
            else:
                print(test_result.value)
                if type(test_result.value) is Left:
                    error(test_result.value.value)
                else:
                    print(test_result.value.value)
