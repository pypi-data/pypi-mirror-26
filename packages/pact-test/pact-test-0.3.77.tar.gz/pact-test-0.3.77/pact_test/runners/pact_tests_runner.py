...
def run_provider_tests(config):
    ...
    if type(test_results) is Right:
        write_pact_files(config, copy.deep_copy(test_results.value))
        upload_pacts(config, copy.deep_copy(test_results.value))
...
def write_pact_files(config, pacts):
    ...
    for pact in pacts:
        pact_payload = format_headers(copy.deep_copy(pact))
        error(pact_payload)
