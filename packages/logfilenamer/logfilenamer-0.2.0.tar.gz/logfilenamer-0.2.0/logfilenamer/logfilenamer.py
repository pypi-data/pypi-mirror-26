...
def get_id_and_name(header, converted_date, cursor):
    ...
    execute_string = """
        ...
        WHERE
            ...
            DATEADD(second, -1, TrackTreatment.Create_DtTm) <= '{1}' AND
            DATEADD(second, 1, TrackTreatment.Edit_DtTm) >= '{1}' AND
...
def logfilenamer(filepath, cursors, skip_if_exists=True):
    ...
    if header.field_label == '':
        ...
        patient = get_id_and_name(header, converted_date, cursors[centre])
