...
def get_mosaiq_extract(header, converted_date, cursor, time_alignment_buffer,
                    skip_on_not_found):
    ...
    mosaiq_extract = MosaiqExtract(*sql_result[0])


    return mosaiq_extract
...
def single_logfilenamer(filepath, cursors, skip_if_exists,
    ...
    if header.field_label == '':
        ...
        mosaiq_extract = get_mosaiq_extract(
            header, converted_date, cursors[centre], time_alignment_buffer,
            skip_on_not_found)

        if not mosaiq_extract and skip_on_not_found:
            return None

        mosaiq_extract_dict = attr.asdict(mosaiq_extract)

        merged_metadata = {**mosaiq_extract_dict, **header_dict}
...
def main():
    ...
    parser.add_argument('--skip_on_not_found', action='store_true')
    ...
    skip_on_not_found = args.skip_on_not_found
