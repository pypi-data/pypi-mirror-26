...
def single_logfilenamer(filepath, cursors, skip_if_exists, time_alignement_buffer):
...
def logfilenamer(glob_strings, skip_if_exists=True, time_alignement_buffer=1):
    with sql_connection(SQL_USERS, SQL_SERVERS) as cursors:
        for glob_string in glob_strings:
            filepaths = glob(glob_string)
            for filepath in filepaths:
                single_logfilenamer(
                    filepath, cursors, skip_if_exists, time_alignement_buffer)
...
def main():
    ...
    parser.add_argument('--time_alignement_buffer', default='1')

    args = parser.parse_args()

    glob_strings = args.glob_strings
    skip_if_exists = bool(args.skip_if_exists)
    time_alignement_buffer = int(args.time_alignement_buffer)

    logfilenamer(glob_strings, skip_if_exists, time_alignement_buffer)
