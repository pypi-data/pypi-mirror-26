...
class DoubleSixGirls(Spider):
    ...
    def sub_parse(self, response):
        ...
        for url in item_urls:
            ...
            if product_no in self.crawled_product:
                ...
                review_url = 'http://widgets1.cre.ma/66girls.co.kr/products/reviews?app=0&iframe=1&iframe_id=crema-product-reviews-1&parent_url=%s' % self.netloc + url
                yield Request(
                    url=review_url,
        ...
        if len(response.css('li.product_review__container')) and response.meta['page'] <= 5:
            for product_review in response.css('li.product_review__container'):
                ...
                for option_value in parsed.find_all('div', class_='review-option'):
                    ...
                    if option == '키':
                        try:
                            writer['height'] = int(value.text.strip().split('~')[-1])
                        except:
                            self.logger.info('키 에러: %s' % value.text)
    ...
    def detail_parse(self, response):
        ...
        if response.meta['is_exist']:
            self.logger.info('Exist: %s' % response.url)
            ...
            self.logger.info('Not exist: %s' % response.url)
            product = {
