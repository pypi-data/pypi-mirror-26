...
def _api_load(url, file_name='output.json', key_var="PCT_API_READ_KEY", silent=False):
    ...
    return_request, resp_headers = _api_try(url, headers, None, silent=silent)
    ...
    try:
        ...
        if not silent:
            print("No page count / One page specified.  Writing Single Page...")
    ...
    for page in range(1, page_count+1):
        single_response, response_headers = _api_try(url, headers, page, silent=silent)
...
def _api_try(url, headers, page=None, silent=False):
    ...
    while tries < API_RETRY:
        ...
        response, resp_headers = _api_call(url, headers, page=page, silent=silent)
...
def _api_call(url, headers, page=None, silent=False):
    ...
    if page is not None and "?" not in url:
        url += "?Page=" + str(page)
