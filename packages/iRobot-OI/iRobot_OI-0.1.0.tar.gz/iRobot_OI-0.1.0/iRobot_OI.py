...
def init(sp='/dev/tty.ElementSerial-ElementSe', debug=False):
    """
    This command initializes the interface. It must be called before everything else.
    """

    global oi, DEBUG
    oi = serial.Serial(sp, baudrate=57600)
    DEBUG = debug
...
class utils:
    ...
    def sendOpcode(opc):
        if DEBUG:
            print("Sending " + str(opc) + " as " + str(bytes([opc])))
        oi.write(bytes([opc]))

    @staticmethod
    def sendBytes(*args):
        if DEBUG:
            print("Sending " + str(args) + " as " + str(bytearray(args)))
...
class actuator:
    ...
    def drive(velocity, radius):
        ...
        velUnsigned = struct.unpack('>H', struct.pack('>h', velocity))[0]
        radUnsigned = struct.unpack('>H', struct.pack('>h', radius))[0]
        velHi = velUnsigned >> 8
        velLo = velUnsigned & 0xff
        radHi = radUnsigned >> 8
        radLo = radUnsigned & 0xff
    ...
    def driveDirect(rightVelocity, leftVelocity):
        ...
        rvUnsigned = struct.unpack('>H', struct.pack('>h', rightVelocity))[0]
        lvUnsigned = struct.unpack('>H', struct.pack('>h', leftVelocity))[0]
        rvHi = rvUnsigned >> 8
        rvLo = rvUnsigned & 0xff
        lvHi = lvUnsigned >> 8
        lvLo = lvUnsigned & 0xff
...
class wait:
    ...
    def waitDistance(distance):
        ...
        distUnsigned = struct.unpack('>H', struct.pack('>h', distance))[0]
        distHi = distUnsigned >> 8
        distLo = distUnsigned & 0xff
        utils.sendBytes(distHi, distLo)

    @staticmethod
    def waitAngle(angle):
        """
        This command causes Create to wait until it has rotated through specified angle in degrees.
        ...
        angUnsigned = struct.unpack('>H', struct.pack('>h', angle))[0]
        angHi = angUnsigned >> 8
        angLo = angUnsigned & 0xff
