...
class API:
    ...
    def modify_group(self, id, name=None, description=None, add_subs=None, rem_subs=None):
        """Change the name or description of a group
            id - The id of the group to modify
        """

        if add_subs is not None and not isinstance(add_subs, set):
            raise APIError(self.localizer.to_local('modify_group.not_set', 'add_subs'))
        if rem_subs is not None and not isinstance(rem_subs, set):
            raise APIError(self.localizer.to_local('modify_group.not_set', 'rem_subs'))

        args = {
            "id": id,
            "name": name,
            "description": description,
            "add_subs": list(add_subs) if add_subs is not None else None,
            "rem_subs": list(rem_subs) if rem_subs is not None else None
        }

        return self._make_request("modify_group", args)

    def get_groups(self, id=None, name=None):
