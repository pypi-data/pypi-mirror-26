...
class vmconnector_restapi_with_ns():
    ...
    def reload_network_namespace(self):
        try:
            result, content = subprocess.getstatusoutput("ip netns | grep qrouter")
        except:
            result = -500
    ...
    def do_script(self, api_info, port_str=None):
        ...
        try:
            ...
            # build api
            #api = "ip netns exec %s curl --insecure -b %s -H \"%s\" -vX %s %s -d \'%s\'" % (self.net_ns, cookie, header, request_type, url, params)
            
            if self.host.find("192.168.254") >= 0: #if self.action == "UPDATE":
                api = "curl --insecure -k "
            else:
                api = "ip netns exec %s curl --insecure -k " %(self.net_ns)
    ...
    def copy_ssh_key(self, path='/root/.ssh/id_rsa.pub'):
        ...
        if self.host.find("192.168.254") >= 0: #if self.action == "UPDATE":
            ...
            mkdir_cmd = "ip netns exec %s ssh %s %s@%s mkdir .ssh" % (self.net_ns, options, self.user, self.host)
            result1, content1 = self.exec_pexpect(mkdir_cmd)
