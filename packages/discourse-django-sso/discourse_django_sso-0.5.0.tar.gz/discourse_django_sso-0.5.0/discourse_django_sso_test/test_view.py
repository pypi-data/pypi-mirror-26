...
def test_client_url_generaion(admin_client: Client):
    response = admin_client.get(reverse('client'), data={})
    assert response.status_code == 302
    expected_response = (
...
def test_required_args_for_session_gen(admin_client: Client):
    """
    Check that sig and sso are required parameters.
    :param admin_client:
    :return:
    """
    response = admin_client.get(reverse('createSession'),
                                data={})
    assert response.status_code == 400
    response = admin_client.get(reverse('createSession'),
                                data={"sso": "does not matter"})
    assert response.status_code == 400


def test_error_sso_not_match_sig(admin_client: Client):
    """
...
def test_session_generation(admin_client: Client):
    response = admin_client.get(reverse('createSession'), data={
