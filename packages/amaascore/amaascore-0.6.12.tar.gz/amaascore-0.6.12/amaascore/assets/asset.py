...
class Asset(AMaaSModel):
    ...
    def hashcode(self):
        ...
        self._remove_amaas_model_attributes(my_dict)
        for attr in ['client_additional']:
            value = getattr(self, attr)
            if value:
                dict_value = pickle.loads(value)
                if dict_value and isinstance(dict_value, dict):
                    my_dict[attr] = dict_value
        return compute_hash(my_dict)
    
    def _remove_amaas_model_attributes(self, model_dict):
        [model_dict.pop(attr, None) for attr in self.amaas_model_attributes()]
        models = []
        for value in model_dict.values():
            if isinstance(value, dict):
                self._remove_amaas_model_attributes(value)
            elif isinstance(value, AMaaSModel):
                models.append(value)

        for model in models:
            [model.__dict__.pop(attr, None) for attr in self.amaas_model_attributes()]
