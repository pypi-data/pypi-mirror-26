...
otherwise ``False``. Controls whether `Opbeat <https://opbeat.com/>`__
logging handlers should be setup application. The following environment
variables are required for Opbeat logging to be setup automatically:
``OPBEAT_ORGANIZATION_ID``, ``OPBEAT_APP_ID``, ``OPBEAT_SECRET_TOKEN``.
If all conditions are met, the following will be setup:
