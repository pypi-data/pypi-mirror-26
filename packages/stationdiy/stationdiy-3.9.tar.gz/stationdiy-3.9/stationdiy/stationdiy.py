...
class StationDiY():
    ...
    def sendMQTT(self):
        ...
        def on_connect(mqttc, userdata, flags, rc):
            mqttc.publish(topic='StationDiy/',  payload='{"user_hash":"%s", "action":"%s","username":"%s","password":"%s","device":"%s","sensor":"%s","data":"%s", "actioner":"%s", "data_actioner":"%s", "longitud":"%s", "latitud": "%s","description":"Lorem Ipsium Dev2...", "max_value":"%s", "min_value":"%s", "type":"%s"}'%(self.user_hash, self.action, self.username, self.password, self.device, self.sensor, self.data, self.actioner, self.data_actioner, self.longitud, self.latitud, self.max_value, self.min_value, self.type_data), qos=0)
