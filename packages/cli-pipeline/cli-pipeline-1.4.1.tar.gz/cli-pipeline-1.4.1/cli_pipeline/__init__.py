...
#        print("")
    ...
    def train_server_start(self,
        ...
        cmd = 'docker run -itd -p 6007:6007 -e TF_CONFIG=%s -e PIPELINE_MODEL_TRAIN_ARGS=%s --name=%s-%s-%s-%s -m %s %s/%s/%s-%s-%s:%s' % (tf_config_local_run, train_args, image_registry_namespace, model_type, model_name, model_tag, memory_limit, image_registry_url, image_registry_repo, image_registry_namespace, model_type, model_name, model_tag)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
