...
#        print("")
    ...
    def train_server_start(self,
        ...
        cmd = 'docker run -itd -p 6334:6334 --name=%s-%s-%s-%s -m %s %s/%s/%s-%s-%s:%s' % (build_registry_namespace, model_type, model_name, model_tag, memory_limit, build_registry_url, build_registry_repo, build_registry_namespace, model_type, model_name, model_tag)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
