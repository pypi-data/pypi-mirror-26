...
class PipelineCli(object):
    ...
    _default_image_registry_base_tag = 'cpu-1.3.0'
    ...
    def version(self):
        ...
        print('default build path: %s => %s' % (PipelineCli._default_build_context_path, build_context_path))
        print('')
    ...
    def _create_predict_server_Dockerfile(self,
        ...
        rendered_Dockerfile = '%s/generated-%s-%s-%s-%s-Dockerfile' % (build_context_path, image_registry_namespace, model_type, model_name, model_tag)
    ...
    def _create_predict_cluster_Kubernetes_yaml(self,
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path or './')).get_template(filename).render(context)
        rendered_filename = './generated-%s-%s-%s-%s-cluster-deploy.yaml' % (image_registry_namespace, model_type, model_name, model_tag)
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path or './')).get_template(filename).render(context)    
        rendered_filename = './generated-%s-%s-%s-%s-cluster-svc.yaml' % (image_registry_namespace, model_type, model_name, model_tag)
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'." % (filename, rendered_filename)) 
            rendered_filenames += [rendered_filename]
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path or './')).get_template(filename).render(context)
        rendered_filename = './generated-%s-%s-%s-%s-router-split-deploy.yaml' % (image_registry_namespace, model_type, model_name, model_tag)
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'." % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path or './')).get_template(filename).render(context)
        rendered_filename = './generated-%s-%s-%s-%s-router-split-ingress.yaml' % (image_registry_namespace, model_type, model_name, model_tag)
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'." % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path or './')).get_template(filename).render(context)
        rendered_filename = './generated-%s-%s-%s-%s-router-split-svc.yaml' % (image_registry_namespace, model_type, model_name, model_tag)
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'." % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
    ...
    def experiment_add(self,
        ...
        # TODO:  Generate RouteRule
        cmd = "kubectl apply -f <(istioctl kube-inject -f generated-experiment-%s-%s-%s-%s-%s-%s-%s.yaml)" % (image_registry_namespace, model_type, model_name, model_tag)
...
#        print("")
    ...
    def _create_train_server_Dockerfile(self,
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path or './')).get_template(filename).render(context)
        rendered_filename = '%s/generated-%s-%s-%s-%s-Dockerfile' % (build_context_path, image_registry_namespace, model_type, model_name, model_tag)
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'." % (filename, rendered_filename))
    ...
    def train_server_start(self,
        ...
        cmd = 'docker run -itd -p 6334:6334 --name=%s-%s-%s-%s -m %s %s/%s/%s-%s-%s:%s' % (image_registry_namespace, model_type, model_name, model_tag, memory_limit, image_registry_url, image_registry_repo, image_registry_namespace, model_type, model_name, model_tag)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
    ...
    def _create_train_cluster_Kubernetes_yaml(self,
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path or './')).get_template(filename).render(context)
        rendered_filename = './generated-%s-cluster-%s-%s-%s.yaml' % (image_registry_namespace, model_type, model_name, model_tag)
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
        print("'%s' => '%s'." % (filename, rendered_filename))
