...
#    _default_output_path = './pipeline_output'
#    _default_input_path = './pipeline_input'
    ...
    def version(self):
        ...
        print('')
        print('capabilities_enabled: %s' % ['train_server_*', 'predict_server_*', 'predict_test_http'])
        print('capabilities_available: %s' % ['train_cluster_*', 'predict_cluster_*', 'predict_test_stream', 'optimize_*', 'experiment_*'])
        print('')
    ...
    def predict_server_build(self,
        ...
        if input_host_path:
            input_host_path = os.path.expandvars(input_host_path)
            input_host_path = os.path.expanduser(input_host_path)
            input_host_path = os.path.abspath(input_host_path)

        if output_host_path:
            output_host_path = os.path.expandvars(output_host_path)
            output_host_path = os.path.expanduser(output_host_path)
    ...
    def _create_predict_cluster_Kubernetes_yaml(self,
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path or './')).get_template(filename).render(context)
        rendered_filename = './.pipeline-generated-%s-%s-%s-%s-%s-router-split-deploy.yaml' % (image_registry_namespace, model_runtime, model_type, model_name, model_tag)
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'." % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path or './')).get_template(filename).render(context)
        rendered_filename = './.pipeline-generated-%s-%s-%s-%s-%s-router-split-ingress.yaml' % (image_registry_namespace, model_runtime, model_type, model_name, model_tag)
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'." % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path or './')).get_template(filename).render(context)
        rendered_filename = './.pipeline-generated-%s-%s-%s-%s-%s-router-split-svc.yaml' % (image_registry_namespace, model_runtime, model_type, model_name, model_tag)
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'." % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
    ...
    def predict_server_pull(self,
        ...
        cmd = 'docker pull %s/%s/%s-%s-%s-%:%s' % (image_registry_url, image_registry_repo, image_registry_namespace, model_runtime, model_type, model_name, model_tag)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
...
#            output_host_path = os.path.abspath(output_host_path)
        ...
        cmd = 'docker run -itd -p 6969:6969 -p 9090:9090 -p 3000:3000 -e PIPELINE_INPUT_PATH=%s -e PIPELINE_OUTPUT_PATH=%s -e PIPELINE_INPUT_STREAM_URL=%s -e PIPELINE_INPUT_STREAM_TOPIC=%s -e PIPELINE_OUTPUT_STREAM_URL=%s -e PIPELINE_OUTPUT_STREAM_TOPIC=%s --name=%s-%s-%s-%s-%s -m %s %s/%s/%s-%s-%s-%s:%s' % (input_path, output_path, input_stream_url, input_stream_topic, output_stream_url, output_stream_topic, image_registry_namespace, model_runtime, model_type, model_name, model_tag, memory_limit, image_registry_url, image_registry_repo, image_registry_namespace, model_runtime, model_type, model_name, model_tag)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
    ...
    def _traffic_split(self,
        ...
        cmd = "kubectl apply -f <(istioctl kube-inject -f .pipeline-generated-split-%s-%s-%s-%s-%s.yaml)" % (model_runtime, model_type, model_name, model_tag, split_percentage)
...
#        print("")
    ...
    def _create_train_server_Dockerfile(self,
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path or './')).get_template(filename).render(context)
        rendered_filename = '%s/.pipeline-generated-%s-%s-%s-%s-%s-Dockerfile' % (build_context_path, image_registry_namespace, model_runtime, model_type, model_name, model_tag)
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'." % (filename, rendered_filename))
    ...
    def train_server_start(self,
        ...
        cmd = 'docker run -itd -p 6007:6007 -e PIPELINE_INPUT_PATH=%s -e PIPELINE_OUTPUT_PATH=%s -e PIPELINE_INPUT_STREAM_URL=%s -e PIPELINE_INPUT_STREAM_TOPIC=%s -e PIPELINE_OUTPUT_STREAM_URL=%s -e PIPELINE_OUTPUT_STREAM_TOPIC=%s -e TF_CONFIG=%s -e PIPELINE_MODEL_TRAIN_ARGS=%s -v %s:%s -v %s:%s --name=%s-%s-%s-%s-%s -m %s %s/%s/%s-%s-%s-%s:%s' % (input_path, output_path, input_stream_url, input_stream_topic, output_stream_url, output_stream_topic, tf_config_local_run, train_args, input_host_path, input_path, output_host_path, output_path, image_registry_namespace, model_runtime, model_type, model_name, model_tag, memory_limit, image_registry_url, image_registry_repo, image_registry_namespace, model_runtime, model_type, model_name, model_tag)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
    ...
    def _create_train_cluster_Kubernetes_yaml(self,
        ...
        predict_clustered_template = os.path.join(pipeline_templates_path, PipelineCli._kube_train_cluster_template_registry['train-cluster'][0][0])
        path, filename = os.path.split(predict_clustered_template)
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path or './')).get_template(filename).render(context)
        rendered_filename = './.pipeline-generated-%s-cluster-%s-%s-%s-%s.yaml' % (image_registry_namespace, model_runtime, model_type, model_name, model_tag)
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
        print("'%s' => '%s'." % (filename, rendered_filename))
    ...
    def train_cluster_start(self,
        ...
        generated_yaml = self._create_train_cluster_Kubernetes_yaml(model_runtime=model_runtime,
                                                                    model_type=model_type,
                                                                    model_name=model_name,
                                                                    model_tag=model_tag,
