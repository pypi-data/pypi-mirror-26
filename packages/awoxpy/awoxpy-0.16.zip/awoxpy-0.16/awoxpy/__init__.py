...
class awoxAroma:
    ...
    def get_colour(self):
        if type(self.red) == bytes:
            tempred = int.from_bytes([self.red], byteorder='big')
        else:
            tempred = self.red

        if type(self.green) == bytes:
            tempgreen = int.from_bytes([self.green], byteorder='big')
        else:
            tempgreen = self.green

        if type(self.blue) == bytes:
            tempblue = int.from_bytes([self.blue], byteorder='big')
        else:
            tempblue = self.blue
    ...
    def get_colour_brightness(self):
        if type(self.colourBrightness) == bytes:
