...
class DKCloudCommandRunner(object):
        ...
        try:
            ...
            for folder,files in rl['only_remote'].items():
                for f in files:
                    file = os.path.join(folder,f['filename'])
                    path = file[len(recipe_name)+1:]
                    if not os.path.isfile(path) and delete_remote:
                        changes[path] = {}
    ...
    @staticmethod
    @check_api_param_decorator
    def list_order(dk_api, kitchen, order_count=5, order_run_count=3, recipe=None):
        rc = dk_api.list_order(kitchen, order_count, order_run_count, recipe=recipe)
        if not rc.ok():
