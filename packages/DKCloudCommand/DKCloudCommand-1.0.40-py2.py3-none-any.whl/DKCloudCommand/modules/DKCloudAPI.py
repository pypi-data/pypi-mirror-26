...
class DKCloudAPI(object):
    ...
    def list_order(self, kitchen, order_count=5, order_run_count=3, recipe=None, save_to_file=None):
        """
        ...
        base_url_string = '%s/v2/order/status/%s?count=%d&scount=%d'
        if recipe:
            url = '%s/v2/order/status/%s?count=%d&scount=%d&r=%s' % (self.get_url_for_direct_rest_call(), kitchen, order_count, order_run_count, recipe)
        else:
            url = '%s/v2/order/status/%s?count=%d&scount=%d' % (self.get_url_for_direct_rest_call(), kitchen, order_count, order_run_count)
        try:
            response = requests.get(url, headers=self._get_common_headers())
            rdict = self._get_json(response)
            pass
