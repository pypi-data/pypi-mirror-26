...
@click.option('--order_count', '-oc', type=int, required=False, default=3, help='Number of orders to display')
@click.option('--order_run_count', '-orc', type=int, required=False, default=2, help='Number of order runs to display, for each order')
@click.option('--recipe', '-r', type=str, required=False, default=None, help='Filter results for this recipe only')
@click.pass_obj
def order_list(backend, kitchen, order_count, order_run_count, recipe):
    """
    ...
    check_and_print(
            DKCloudCommandRunner.list_order(backend.dki, use_kitchen, order_count, order_run_count, recipe=recipe))
