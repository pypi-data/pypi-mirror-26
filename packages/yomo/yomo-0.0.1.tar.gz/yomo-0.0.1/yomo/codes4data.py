...
def merge(fnames, savefname='memory', combine_variables=(), ignore_variables=('time', 'time_bounds'), 
    ...
    with Dataset(savefname, 'a') as dataset:
        ...
        if give_stats:
            dataset.createDimension('stats', 2)
            var = dataset.createVariable('stats', str, ('stats',))
            var[:] = np.array(['standard_deviation', 'number_of_elements'], dtype='object') 
        ...
        for fname in fnames[1:]:
            ...
            with assemble(fname) as dataset0:
                ...
                for key0 in keys[-1]:
                    if key0 in ignore_variables or key0 + ' from ' + prefix in ignore_variables:
                        ...
                        if c: # to be combined into a matrix
                            ...
                            varname = key0 + '_' + prefix if np.any([key0 in keys1 for keys1 in keys[:-1]]) else key0
                        ...
                        if varname in dataset.variables.keys(): 
                            ...
                            var = dataset[varname]
                            if give_stats:
                                varstats = dataset['stats_'+varname]
                        else:
                            ...
                            var = dataset.createVariable(varname, dataset0[key0].datatype, 
                                     vardimension,
                            ...
                            if give_stats:
                                varstats = dataset.createVariable('stats_'+varname, dataset0[key0].datatype, 
                ...
                for v, var in enumerate(variables):
                    if give_stats:
                        varstats = dataset['stats_'+var.name]
                    ...
                    if isinstance(cols[v], int) and not (len(var.dimensions) == 1 and cols[v] == 0):
                        ...
                        if give_stats:
                            varstats[0, :, cols[v]] = standard_deviation0[:,v]
                            varstats[1, :, cols[v]] = number_of_elements0[:,v]
                    else:
                        var[:] = datacombined0[:,v]                        
                        if give_stats:
                            varstats[0, :] = standard_deviation0[:,v]
                            varstats[1, :] = number_of_elements0[:,v]
...
def _readict(fnames, dataset, mdi_and_lsd = {'-9999.9': None, '-9999.99': None}, sep=',', **subsets):   
    ...
    with ExitStack() as stack:
        ...
        for fobj in fobjs:
            for i, line in enumerate(fobj):
                dataset.headers = dataset.headers + line
                if i==0:
        ...
        # Developer's notes: The values in the first row does not necessarily reflect the precision of
        # the entire data array for ict files with -9999s. A better approach is needed.
        ...
        least_significant_digits.insert(1, max((least_significant_digits[0],bound_offsets_least_significant_digit)) if bound_offsets else max(least_significant_digits[boundsindex[0]-1], least_significant_digits[boundsindex[1]-1])) # -1 accounts for the inserts
        ...
        for n, array in enumerate(zip(*arrays)):
            ...
            if n == 0: # time
                ...
                if n > 2: # dependent variables              
                    ...
                    var.missing_value = [missing_data_indicators[n-3], ULOD_FLAG, LLOD_FLAG] # missing_value as a vector; updated 2017/02/24; was var.missing_value = mdi[n-2]                     
...
def _readstardat(fnames, dataset, **subsets):   
    ...
    if all([str(datatype).startswith('C0_') for datatype in datatypes]): # all are C0 files
        ...
        for operation in operations[2]: # operations independent of spectrometer
            pass # !!! to be developed. For track files.
...
def _setglobalattributes(dataset, fnames):
    ...
    if 'source' not in dataset.ncattrs():
        ...
        dataset.source = dataset.source + '\n'
    dataset.history = dataset.history + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ' Assembled using ' + inspect.stack()[2][3] + ' and ' + inspect.stack()[1][3]
