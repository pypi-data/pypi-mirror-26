# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: rst/generic/DictionaryCollection.proto

import sys
_b=sys.version_info[0]<3 and (lambda x:x) or (lambda x:x.encode('latin1'))
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
from google.protobuf import descriptor_pb2
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()


from rst.generic import Dictionary_pb2 as rst_dot_generic_dot_Dictionary__pb2


DESCRIPTOR = _descriptor.FileDescriptor(
  name='rst/generic/DictionaryCollection.proto',
  package='rst.generic',
  syntax='proto2',
  serialized_pb=_b('\n&rst/generic/DictionaryCollection.proto\x12\x0brst.generic\x1a\x1crst/generic/Dictionary.proto\"@\n\x14\x44ictionaryCollection\x12(\n\x07\x65lement\x18\x01 \x03(\x0b\x32\x17.rst.generic.DictionaryB\x1a\x42\x18\x44ictionaryCollectionType')
  ,
  dependencies=[rst_dot_generic_dot_Dictionary__pb2.DESCRIPTOR,])




_DICTIONARYCOLLECTION = _descriptor.Descriptor(
  name='DictionaryCollection',
  full_name='rst.generic.DictionaryCollection',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='element', full_name='rst.generic.DictionaryCollection.element', index=0,
      number=1, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  syntax='proto2',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=85,
  serialized_end=149,
)

_DICTIONARYCOLLECTION.fields_by_name['element'].message_type = rst_dot_generic_dot_Dictionary__pb2._DICTIONARY
DESCRIPTOR.message_types_by_name['DictionaryCollection'] = _DICTIONARYCOLLECTION
_sym_db.RegisterFileDescriptor(DESCRIPTOR)

DictionaryCollection = _reflection.GeneratedProtocolMessageType('DictionaryCollection', (_message.Message,), dict(
  DESCRIPTOR = _DICTIONARYCOLLECTION,
  __module__ = 'rst.generic.DictionaryCollection_pb2'
  # @@protoc_insertion_point(class_scope:rst.generic.DictionaryCollection)
  ))
_sym_db.RegisterMessage(DictionaryCollection)


DESCRIPTOR.has_options = True
DESCRIPTOR._options = _descriptor._ParseOptions(descriptor_pb2.FileOptions(), _b('B\030DictionaryCollectionType'))
# @@protoc_insertion_point(module_scope)
