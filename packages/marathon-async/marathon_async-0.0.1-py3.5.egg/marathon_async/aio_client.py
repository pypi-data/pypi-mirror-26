...
import marathon
from marathon.models import MarathonApp, MarathonDeployment, MarathonGroup, MarathonInfo, MarathonTask, \
    MarathonEndpoint, \
    MarathonQueueItem
...
class MarathonAsyncClient(object):
    ...
    async def _do_request(self, method, path, params=None, data=None):
        ...
        if response.status >= 500:
            ...
            marathon.log.error('Got HTTP {code}: {body}'.format(
                code=response.status, body=response_text))
    ...
    async def _do_sse_request(self, path, params=None):
        ...
        for server in list(self.servers):
            url = ''.join([server.rstrip('/'), path])
            try:
                response = await self.sse_session.get(
                    url,
                    params=params,
    ...
    async def list_apps(self, cmd=None, embed_tasks=False, embed_counts=False,
                        embed_deployments=False, embed_readiness=False,
                        embed_last_task_failure=False, embed_failures=False,
                        embed_task_stats=False, app_id=None, label=None, **kwargs) -> list:
        """List all apps.
        ...
        apps = await self._parse_response(
            response, MarathonApp, is_list=True, resource_name='apps')
        for k, v in kwargs.items():
            apps = [o for o in apps if getattr(o, k) == v]
        return apps

    async def get_app(self, app_id, embed_tasks=False, embed_counts=False,
                      embed_deployments=False, embed_readiness=False,
                      embed_last_task_failure=False, embed_failures=False,
    ...
    async def list_groups(self, **kwargs) -> list:
        """List all groups.
        ...
        groups = await self._parse_response(
    ...
    async def delete_group(self, group_id, force=False) -> dict:
    ...
    async def list_tasks(self, app_id=None, **kwargs):
        ...
        tasks = await self._parse_response(
    ...
    async def kill_tasks(self, app_id, scale=False, wipe=False,
    ...
    async def kill_task(self, app_id, task_id, scale=False, wipe=False):
        ...
        """
        params = {'scale': scale, 'wipe': wipe}
        response = await self._do_request('DELETE', '/v2/apps/{app_id}/tasks/{task_id}'
    ...
    async def get_version(self, app_id, version):
        ...
        :rtype: :class:`marathon_async.models.app.MarathonApp`
        """
        response = await self._do_request('GET', '/v2/apps/{app_id}/versions/{version}'
