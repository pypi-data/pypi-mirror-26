...
class TestSC2Agent(unittest.TestCase):
    ...
    def test_exploration_is_called(self):
        agent = MockAgent(token=self.token, exploration_rate=1.0)
        agent.setup(None, None)
    ...
    def test_exploitation_is_called(self):
        agent = MockAgent(token=self.token, exploration_rate=0.0)
        agent.setup(None, None)
    ...
    def test_train_is_called_each_X_episodes(self):
        for frequency in range(1, 3):
            ...
            for passes in range(5):
                ...
                self.assertCountEqual(agent.train_called_args['train_x'], agent.samples.get_train_x())
                self.assertCountEqual(agent.train_called_args['train_y'], agent.samples.get_train_y())

    def test_train_is_not_called_if_no_samples(self):
        agent = MockAgent(token=self.token, training_frequency=3)
        agent.setup(None, None)
        agent._model = DummyModel()
...
class MockAgent(SC2Agent):
    def __init__(self, token, training_frequency=1, exploration_rate=0.5):
        super().__init__(token=token, training_frequency=training_frequency, exploration_rate=exploration_rate)
