...
def bam_extract_region_cover(args):
    ...
    with sam:
            ...
            for rec in it:
                if is_mating_target(rec):
                    if rec.qname in wait_recs:   # mating for this read name is resolved
                        ...
                        if emit_inside:
                            out.write(rec)   # TODO this part seems too slow!
                            out.write(mrec)
                    else:
                        wait_recs[rec.qname] = rec   # add this read name to mate waiting list
        ...
        for next_ref_id, cands in groupby(cands, lambda r: r.next_reference_id):
            ...
            logging.info('Search mate from %s ...', next_ref_name)
            for rec in tqdm(list(cands)):
                mrec = sam.mate(rec)
                if mrec:
                    if emit_inside:
                        out.write(rec)
        ...
        if wait_recs:
            ...
            logging.info('Mating of all the reads has been resolved.', len(wait_recs))
