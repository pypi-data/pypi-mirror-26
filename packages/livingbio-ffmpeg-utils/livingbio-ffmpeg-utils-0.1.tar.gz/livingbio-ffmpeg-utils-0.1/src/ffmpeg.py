...
def __merge_scences(frame_durations, merge_delta_time=0.7, min_delta_time=3):
    ...
    for frame_duration in frame_durations[1:]:
        ...
        if __is_effict_frame(frame_info) and __is_effict_frame(last_frame_info) and last_delta_time < merge_delta_time and delta_time < merge_delta_time:  # noqa
...
def _cut_scenes(video_path, threshold):
    scenes = []
...
def get_scene_info(filepath, threshold=0.3):
    # make sure all video is split completely
    scenes, total_duration = _cut_scenes(filepath, threshold)
