...
from quantipy.utils import _axes_decorator, _axes_decorator_3d, _torusdistance,\
    unique_rows, _kwtolegkw, _map_to_simtorus, unique_rows
...
class GenericModel:
        ...
        if self.OBC:
            ...
            finite_lattice = \
              Lattice(unitcell=self.lattice.unitcell,
                      ...
                      simulation_torus_matrix=(nbdist+1)*np.eye(self.dimension))
        ...
        if nbdist==0:
            ...
            for i, j in itertools.combinations(xrange(finite_lattice.n_sites),
                ...
                if np.abs(dist-latticedist[nbdist])<1e-8:
                    ...
                    bondtmp = np.hstack([np.array(finite_lattice._rel_indices[order[1]]['cell'])-\
                                         np.array(finite_lattice._rel_indices[order[0]]['cell']),
            ...
            for b in bonddata:
                ...
                self._couplings.append({'cellshift': cellshift,
                    'basis_sites': basissites, 'int_type':int_type,
                    'coupling':coupling, 'value':couplingvalue,
                    'directed':False, 'allowed_perms':None,
                    'trans_repeat':True})
    ...
    def set_long_range_interaction(self, function, max_nb_dist=None,
        ...
        for coord in finite_lattice.coordinates:
            for bcoord in finite_lattice.basiscoordinates:
                latticedist = np.append(latticedist, np.linalg.norm(coord-bcoord))
        latticedist = np.unique(np.round(latticedist,decimals=8))
    ...
    def _get_finite_lattice_bonds(self,lattice):
        ...
        for coupl in self._couplings:
            ...
            if coupl['trans_repeat'] is False:
                ...
                for bravaisshift in bravaisshifts:
                    ...
                    for i in range(len(coupl['basis_sites'])):
                        index = {'cell':list(cellshifts[i]+bravaisshift),
                                'basis':coupl['basis_sites'][i]}
                        pos = np.argwhere(index==np.array(lattice._rel_indices))
                        if pos.size:
                            bond[i] = pos[0,0]
                        else:
                ...
                #print int(param_pos)
                self._finite_lattice_bonds[int(param_pos)]['bonds'].extend(bonds)
    ...
    def pointgroup(self):
        """
        Returns the point group of the model
        """
    ...
    def _get_allowed_symmetries(self):
        ...
        for idx,sym in enumerate(self.lattice.symmetries_torus['rotations']):
            ...
            for bond in self._couplings:
                ...
                for checkbond in self._couplings:
                    if (checkbond['int_type']==bond['int_type'] and
                        ...
                        #map bonds to unitcell to compare to rotated coordinates
                        ...
                        checkbasiscoords = self.lattice.basiscoordinates[checkbond['basis_sites']]
                        checkbondcoords = checkcellcoords + checkbasiscoords

                        checkbasiscoords = _map_to_simtorus(checkbondcoords,
                                                            self.lattice.unitcell)
                        checkcellshift = np.linalg.solve(self.lattice.unitcell.T,
                                                         checkbondcoords.T).T
                        ...
                        for perm in allowedperms:
                            ...
                            basisok = np.all(np.abs(newbasiscoords[np.array(perm)]
            ...
            if symok:
                if ((self.maxpg == None) or
                    ...
                    rot_names.append(self.lattice.symmetries_torus['names'][idx])                    

        self.symmetries = {'rotations': np.array(reduced_rotations,
                                                 dtype='intc', order='C'),
        ...
        if self.dimension == 2:
            self._pointgroup = quantipy.spglib.spglib2d.get_pointgroup(
                self.symmetries["rotations"]) # Pointgroup of the lattice
        elif self.dimension == 3:
            self._pointgroup = quantipy.spglib.spglib.get_pointgroup(
                self.symmetries["rotations"]) # Pointgroup of the lattice
        
        #Recompute little groups
        self.little_groups = self.lattice._get_little_groups(self.symmetries)
    ...
    def symmetries(self):
        """
        Compute permutations and phase factors for all allowed 
        irreps of the model.
        """
        ...
        return symmetries._get_irreps(self.lattice, self.symmetries,
                                      twod_to_oned=True)
    ...
    def _plot(self, closed_loops=False, interactions=None, rep_inf=4,
        ...
        if self.lattice._is_infinite:
            plotlattice = Lattice(unitcell=self.lattice.unitcell,
                                  ...
                                  simulation_torus_matrix =
                                  rep_inf*np.eye(self.dimension))
        else:
        ...
        for n, coupl in enumerate(self._finite_lattice_bonds):
            ...
            if coupl['bonds'].shape[1]==1:
                ...
                if self.dimension==1:
                    ...
                    axplot = ax.plot(coords[:,0,0],coords[:,0,1],coords[:,0,2],\
                                     '*', c=color, ms=14)
    ...
    def write(self, filename, sublattice_structure=None, write_comments=True,
        ...
        if self.lattice._is_infinite:
            raise ValueError("Cannot write the model for " \
                             + "an infinite lattice yet!")
        else:
            writelattice = self.lattice
      
        #Compute symmetry stuff
        if write_symmetry_info:
            irreps = self.symmetries()
        else:
            ...
            if sublattice_structure is not None:
                ...
                for perm_idx, perm in enumerate(perms):
                    ...
                    for j in range(len(perm)):
                        newperm[j] = sublattice_permutation_inverse[perm[sublattice_permutation[j]]]
                    ...
                    fixed_sublattice_permutations[sublattice_idx].append(newperm)
                    symmetry_permutation_inv[sublattice_idx].append(perm_idx)

                        
                # Glue symmetries together in sublattice ordered way
                # and determine this permutation
                perms = np.array([item for sublist in
                                  fixed_sublattice_permutations
                                  for item in sublist])
                symmetry_permutation_inv = [item for sublist in
                                            symmetry_permutation_inv
                                            for item in sublist]
        ...
        with open(filename,'a') as modelfile:
            ...
            if write_symmetry_info:
                ...
                for i, perm in enumerate(perms):
                    modelfile.write("[S%i] "%i + " ".join(map(str,perm)) + "\n")
