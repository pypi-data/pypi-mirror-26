...
class ResultGetKeysIqProtocolEntity(ResultIqProtocolEntity):
    ...
    def fromProtocolTreeNode(node):
        ...
        for userNode in userNodes:
            ...
            # PATCH: throw error in case of number not registered
            if not preKeyNode:
                raise Exception('Receiver {0} is not registered in Whatsapp'.format(
                    userNode.attributes['jid'].replace('@s.whatsapp.net', ''))
                )
