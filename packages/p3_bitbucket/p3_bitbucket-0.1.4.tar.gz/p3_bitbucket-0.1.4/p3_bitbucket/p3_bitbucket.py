...
class BitbucketUtils:
    ...
    def create_repo(self,  public=False):
        """
        ...
        public_str = "true" if public == True else "false"
        resp = requests.post(
            'https://api.bitbucket.org/2.0/repositories/{}/{}'.format(
                self.uuid, self.app_name),
            auth=(self.bit_user, self.bit_pass),
            data={"scm": "git", "is_private": public_str,
                  "fork_policy": "no_public_forks"}
        )
...
def create_repo(args):
    b_utils.create_repo(args.public)
...
def execute_from_command_line():
    ...
    parser_foo.add_argument('-p', '--public', type=bool, default=False,
                            help="set to public, default is false")
