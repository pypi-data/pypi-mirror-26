...
class centroid():
    ...
    bin_width : float
        300; in seconds; bin_width of the full light curve
    min_time : float
        -
    dt : float
...
#        self.run()
    ...
    def load_object(self):
        ...
        if self.dic is None:
            ...
            self.dic = ngtsio.get(self.fieldname, self.ngts_version, keys, obj_id = self.obj_id, time_hjd = self.time_hjd, bls_rank = self.bls_rank, fnames=self.fnames, silent = True, set_nan = True)
            if self.dic is None:
...
#            self.fig_stacked_image = None
    ...
    def save_png(self):
        if self.outfname_plot is None:
            ...
            outfilename2 = self.outfname_plot + '_2'
...
#    C = centroid('NG0613-3633', '010712', 'TEST18', do_plot='pipeline', output='pipeline') 
#    C.run()
