...
class MessageSender(BotFinder):
    ...
    def on_join(self, connection,
        ...
        if event.source.startswith(self.user.get_name()):
            if not self.user.get_name() == event.target and \
                ...
                self.logger.log(u"Joined Channel " + event.target,
            ...
            if not self.channel_joined:
                ...
                log_message = \
                    u"Sending XDCC Request to " + self.current_pack.get_bot()
                log_message += \
    ...
    def on_namreply(self, connection,
                    event):
        u"""
    ...
    def on_endofnames(self, connection,
                      event):
        u"""
    ...
    def on_currenttopic(self, connection,
                        event):
        u"""
    ...
    def on_topicinfo(self, connection,
                     event):
        u"""
        ...
        :return:           None
        """
        self.logger.log(u"Topic Info: " + unicode(event.arguments),
                        LOG.CHANNEL_TOPIC)

    def on_quit(self, connection,
                event):
        u"""
    ...
    def on_mode(self, connection,
        ...
        :return:           None
        """
        self.logger.log(event.arguments[0] + u" mode changed",
                        LOG.CHANNEL_MODE_CHANGE)

    def on_action(self, connection,
                  event):
        u"""
    ...
    def on_nick(self, connection,
        ...
        :return:           None
        """
        self.logger.log(event.source + u" nick changed",
