...
class BaseIrclient(irc.client.SimpleIRCClient, ConnectionStates, Variables):
    ...
    def connect(self):
        ...
        :return: None
        """
        self.logger.log(u"Connecting to server:  " +
                        self.server.get_address(), LOG.CONNECTION_ATTEMPT)
        self.logger.log(u"Using Port:            " +
                        unicode(self.server.get_port()), LOG.CONNECTION_ATTEMPT)
        self.logger.log(u"As User:               " +
                        self.user.get_name(), LOG.CONNECTION_ATTEMPT)

        super(BaseIrclient, self).connect(
            self.server.get_address(),
            int(self.server.get_port()),
            self.user.get_name()
        )
        self.logger.log(
            u"Established Connection to Server",
    ...
    def start(self):
        ...
        try:
            ...
            self.logger.log(u"Failed to connect to Server",
            ...
            try:
                ...
                pass
            if unicode(type(e)) != \
                    u"<class 'xdcc_dl.xdcc.layers.irc.BaseIrcClient." \
                    u"Disconnect'>":
