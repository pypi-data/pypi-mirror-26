...
class Query(Table, Monad):
    ...
    def allBaseExprs(self):
        return self.allDescendants().filter(lambda x: type(x) is BaseExpr)
    ...
    def exists(self, out='col', col=False, _func='notnull_'):
        ...
        if out == 'col':
            ...
            # as a bool any
            return self.lj().fmap(lambda x: x.firstCol().__getattr__(_func)()).any()
        elif out == 'exists':
            # as an exists expr
