...
class ErrorsToCorrectBarcodesMap:
    ...
    def correct_barcode(self, barcode):
        return self._map[barcode]

    @staticmethod
    def _prepare_single_base_error_hash_table(iterable):
        """

        :param Iterable iterable: iterable of string barcodes
        :return:
        """
        error_map = {}
        for barcode in iterable:
    ...
    def single_hamming_errors_from_whitelist(cls, whitelist_file):
        ...
        :param str whitelist_file:
        :return CorrectBarcodeErrorsHash:
        """
    ...
    def correct_bam(self, bam_file, output_bam_file):
        ...
        with pysam.AlignmentFile(bam_file, 'rb') as fin, \
            ...
            for alignment in fin:
                try:
                    tag = self.correct_barcode(alignment.get_tag('CR'))
