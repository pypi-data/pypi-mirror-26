...
def trivial_whitelist():
    ...
    em = barcode.ErrorsToCorrectBarcodesMap._prepare_single_base_error_hash_table(barcode_iterable)
    return barcode.ErrorsToCorrectBarcodesMap(em)
...
def truncated_whitelist_from_10x():
    ...
    ec = barcode.ErrorsToCorrectBarcodesMap.single_hamming_errors_from_whitelist(
...
def test_correct_barcode_finds_and_corrects_1_base_errors(trivial_whitelist):
    assert trivial_whitelist.correct_barcode('TAAAAAAA') == 'AAAAAAAA'
    assert trivial_whitelist.correct_barcode('AAAACAAA') == 'AAAAAAAA'
    assert trivial_whitelist.correct_barcode('AAAGAAAA') == 'AAAAAAAA'
    assert trivial_whitelist.correct_barcode('AAAAAAAA') == 'AAAAAAAA'


def test_correct_barcode_raises_keyerror_when_barcode_not_correct_length(trivial_whitelist):
    with pytest.raises(KeyError):
        trivial_whitelist.correct_barcode('AAA')
    with pytest.raises(KeyError):
        trivial_whitelist.correct_barcode('AAAAAAAAA')
    with pytest.raises(KeyError):
        trivial_whitelist.correct_barcode('AAAAAAAAAA')


def test_correct_barcode_raises_keyerror_when_barcode_has_more_than_one_error(trivial_whitelist):
    with pytest.raises(KeyError):
        trivial_whitelist.correct_barcode('AAAAAATT')
    with pytest.raises(KeyError):
        trivial_whitelist.correct_barcode('TTAAAAAA')
