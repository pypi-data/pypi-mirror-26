...
class PrintableImage:
    def __init__(self, path_or_pil_or_uint_rgb_arr, full_width=False, use_h_res_augmentation_method=True, auto_normalize=True, auto_fix=True):
        ...
        if isinstance(path_or_pil_or_uint_rgb_arr, basestring):
            ...
            self.im = Image.fromarray(np.clip(auto_norm(path_or_pil_or_uint_rgb_arr, auto_normalize, auto_fix), 0, 255).astype(np.uint8))
        else:
    ...
    def show(self):
        try:
            ...
            print 'Can\'t show an image! Please run from iTerm 2 terminal to see cool images!'
            return
...
def show(path_or_pil_or_uint_rgb_arr, auto_normalize=True, auto_fix=True):
    PrintableImage(path_or_pil_or_uint_rgb_arr, auto_normalize=auto_normalize, auto_fix=auto_fix).show()


def auto_norm(im, auto_normalize, auto_fix):
