...
class MlRepo(Repo):
    def __init__(self, config, repo_root, read_only=False):
        self.__data_volume_config = DataVolumeConfig(repo_root)
    ...
    def create_object_store(self):
        ...
        if bucket_name is None:
            return BackendGCSObjectStore(self._connection_by_name('backend'), self._config, handle_api)

        return GCSObjectStore(self._connection_by_name('gcs'))
    ...
    def remote_commit(self, message):
        ...
        handle_api(self._config, requests.post, url, msg)
