...
class GCSUpload(GCSService):
    ...
    def _get_request(self, bucket_name, object_name, body):
        ...
        url = self.signed_url_service.get_signed_urls('PUT', [object_name], **headers)[0]

        media = MediaInMemoryUpload(body, mimetype=self.DEFAULT_MIMETYPE, chunksize=self.CHUNKSIZE)
        http = httplib2.Http()
