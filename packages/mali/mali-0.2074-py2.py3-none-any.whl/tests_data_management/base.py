...
class BaseTest(BaseCliTest):
    ...
    @classmethod
    def populate_data(cls, filenames):
    ...
    def add_file(self, volume_id, filen_names):
        self.log_stage('In volume %s, adding files: %s ...', volume_id, filen_names)

        args = ['data', 'add', volume_id]
        args.extend(self._multiple_options_to_args('--files', filen_names))
    ...
    def _run_command(self, command_args, should_assert=True):
        ...
        self.log_stage(
            ...
            result.exit_code, result.output, datetime.datetime.utcnow() - cmd_start_time)
