...
@click.pass_context
def clone_data(ctx, volumeid, dest, query, delete, batchsize):
    ...
    multi_process_control = MultiProcessControl()
...
@click.pass_context
def add_to_metadata(ctx, volumeid, files, data, datapoint, datafile, property, propertyint, propertyfloat, update):
    now = time.time()
