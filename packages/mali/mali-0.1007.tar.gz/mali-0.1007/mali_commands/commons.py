...
def handle_api(ctx, http_method, method_url, data=None):
    ...
    for retries in range(3):
        ...
        if r.status_code == 401:
            id_token = update_token(ctx.config_prefix, ctx.client_id, ctx.refresh_token, ctx.auth0)
            continue
...
def pixy_flow(ctx):
    ...
    redirect_uri = app.redirect_uri if app is not None else '{host}.firebaseapp.com/console/auth/success'.format(
        host=ctx.host)
