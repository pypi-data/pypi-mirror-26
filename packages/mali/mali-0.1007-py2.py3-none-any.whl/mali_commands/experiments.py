...
@click.pass_context
def update_graph(ctx, projectid, experimentid, graphid, scope, graphname, graphtype,
                 graphxname, graphx, graphyname, graphy):
    ...
    update_graph_path = 'projects/{project_id}/experiments/{experiment_id}/graph/{graph_id}' \
        .format(project_id=projectid, experiment_id=experimentid, graph_id=graphid)

    if projectid is None:
        raise click.BadOptionUsage('Please also specify --projectId option.')
    if experimentid is None:
        raise click.BadOptionUsage('Please also specify --experimentId option.')
    x = _get_metrics_json(graphx)
    y = _get_metrics_json(graphy)
    if len(x) != len(y):
        raise click.BadOptionUsage('PX and Y data arrays must be of the same size')
    data = {'name': graphname, 'x_name': graphxname, 'y_name': graphyname,
            'x': x, 'y': y, 'graph': graphtype,
            'scope': scope}
