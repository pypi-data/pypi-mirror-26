...
class TestExperiments(BaseCliTest):
    ...
    def testGraphUpdateUpdateGraph_withValidJSONAndShortHands_callHandleApiAndExit(self, handle_api_mock):
        ...
        graph_id = self.some_random_shit(size=10)
        graph_name = self.some_random_shit(size=10)
        graph_x_name = self.some_random_shit(size=10)
        graph_y_name = self.some_random_shit(size=10)
        graph_size = 10
        graph_x = list(range(0, graph_size))
        graph_y = list(range(0, graph_size))
        graph_type = 'line'
        scope = 'test'
        data = {'name': graph_name, 'x_name': graph_x_name, 'y_name': graph_y_name,
                'x': graph_x, 'y': graph_y, 'graph': graph_type, 'scope': scope}
        request = dict(data)
        ...
        handle_api_mock.expects_call().with_matching_args(
            arg.any(),  # ctx
            requests.put,
            'projects/{project_id}/experiments/{experiment_id}/graph/{graph_id}'.format(project_id=self.project_id,
                                                                                        experiment_id=self.experiment_id,
                                                                                        graph_id=graph_id),
            data
        ).returns({})
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ['experiments', 'updateGraph', '-p', self.project_id,
                                     '-e', self.experiment_id, '-g', graph_id,
                                     '-sc', scope, '-gt', graph_type, '-gn', graph_name,
                                     '-gxn', graph_x_name, '-gyn', graph_y_name,
                                     '-gx', json.dumps(graph_x), '-gy', json.dumps(graph_y)],
                               )
        self.assertEqual(result.exit_code, 0)
    ...
    def testGraphUpdateUpdateGraph_withValidJSON_callHandleApiAndExit(self, handle_api_mock):
        ...
        graph_size = 10
        graph_x = list(range(0, graph_size))
        graph_y = list(range(graph_size, 2 * graph_size))
        graph_type = 'line'
        scope = 'test'
        data = {'name': graph_name, 'x_name': graph_x_name, 'y_name': graph_y_name,
                'x': graph_x, 'y': graph_y, 'graph': graph_type,
                'scope': scope}
        request = dict(data)
        request['project_id'] = self.project_id
        request['experiment_id'] = self.experiment_id
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ['experiments', 'updateGraph', '--projectId', self.project_id,
                                     '--experimentId', self.experiment_id, '--graphId', graph_id,
                                     '--scope', scope, '--graphType', graph_type, '--graphName', graph_name,
                                     '--graphXName', graph_x_name, '--graphYName', graph_y_name,
                                     '--graphX', json.dumps(graph_x), '--graphY', json.dumps(graph_y)],
    ...
    def testGraphUpdateUpdateGraph_withSizeMismach_withValidJSON_callHandleApiAndExit(self, handle_api_mock):
        ...
        graph_y_name = self.some_random_shit(size=10)
        graph_size = 10
        graph_x = list(range(0, graph_size + 1))
        graph_y = list(range(graph_size, 2 * graph_size))
        graph_type = 'line'
        scope = 'test'
        data = {'name': graph_name, 'x_name': graph_x_name, 'y_name': graph_y_name,
    ...
    def testGraphUpdateUpdateGraph_with_no_proj_withValidJSON_callHandleApiAndExit(self, handle_api_mock):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ['experiments', 'updateGraph',
                                     '--experimentId', self.experiment_id, '--graphId', graph_id,
                                     '--scope', scope, '--graphType', graph_type, '--graphName', graph_name,
                                     '--graphXName', graph_x_name, '--graphYName', graph_y_name,
                                     '--graphX', json.dumps(graph_x), '--graphY', json.dumps(graph_y)],
