...
class SQLQueryBuilder(LuceneTreeVisitorV2):
    ...
    def __handle_internal_function(self, node):
        if node.name.lower() == 'sample':
            sample = 1.0 - float(node.expr.value)

            random_function = self.__sql_helper.random_function_name()

            self.__where.setdefault(1, []).append(('AND', '({random_function}($seed)>{sample:.4g})'.format(random_function=random_function, sample=sample)))
            return True
    ...
    def visit_search_field(self, node, parents, context):
        ...
        for child in node.children:
            if isinstance(child, Word):
                self.__add_where(node.name, str(node.expr), context.get('op'))
            else:
    ...
    def _sub_visit(self, child, op, context=None):
...
if __name__ == '__main__':  # pragma: no cover
    print(tree_to_sql_parts(parse_query('field1_vkwjtotc:val1_ajzftfdm AND field2_izjpecbv:val2_crvcgnjq')))

    print(tree_to_sql_parts(parse_query('class:text')))
