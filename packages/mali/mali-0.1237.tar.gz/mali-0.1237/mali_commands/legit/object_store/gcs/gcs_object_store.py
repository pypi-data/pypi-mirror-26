...
from ...dulwich.objects import hex_to_filename, hex_to_sha, sha_to_hex, Blob
...
def do_upload(bucket_name, volume_id, object_name, body, signed_url_service=None):
    GCSUpload(signed_url_service).upload(bucket_name, volume_id, object_name, body)
...
class GCSUpload(GCSService):
    def upload(self, bucket_name, volume_id, object_name, body):
        logger.debug('upload %s/%s (%s)', volume_id, object_name, '{:,}'.format(len(body)))

        request = self._get_request(bucket_name, volume_id, object_name, body)
    ...
    def __get_content_type(self, body):
        import puremagic
        import mimetypes
    ...
    def _get_request(self, bucket_name, volume_id, object_name, body):
        content_type = self.__get_content_type(body)

        if not self.signed_url_service:
            service = self._get_authenticated_service()
            resumable = len(body) > self.CHUNKSIZE
            media = MediaInMemoryUpload(body, mimetype=content_type, chunksize=self.CHUNKSIZE, resumable=resumable)
    ...
    def add_object(self, obj):
        ...
        if self.__use_multiprocess:
            self.__upload_pool.apply_async(
                do_upload, args=(self.__bucket_name, self.__volume_id, path, data), callback=self.on_upload_result)
            t = datetime.datetime.utcnow()
            self.__max_waiting_semaphore.acquire()
        else:
            do_upload(self.__bucket_name, self.__volume_id, path, data, signed_url_service=self._signed_url_service)
