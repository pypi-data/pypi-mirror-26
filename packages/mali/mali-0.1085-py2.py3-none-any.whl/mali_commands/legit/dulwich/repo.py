...
import warnings
from mali_commands.utilities.file_utils import get_content_type
...
class Repo(BaseRepo):
    ...
    def stage(self, fs_paths, embedded):
        ...
        def staging_file_builder():
            class UploadFileRequest(object):
                def __init__(self, full_path, tree_path):
                    self.full_path = full_path
                    self.st = os.lstat(self.full_path)
                    self.tree_path = tree_path
                    if embedded:
                        self.blob = blob_from_path_and_stat(self.full_path, self.st)
                    else:
                        self.blob = blob_from_reference_path(self.full_path)
            ...
            for fs_path in fs_paths:
                ...
                try:
                    yield UploadFileRequest(full_path, tree_path)
                except OSError:
        ...
        files_to_add = list(staging_file_builder())
