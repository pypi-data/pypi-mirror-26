...
MALI_CONFIG_FILENAME = 'test-missinglink.cfg'
...
class BaseTest(BaseCliTest):
    ...
    def create_data_volume(self, display_name='TestVolume', org='me', data_path=''):
        print("Creating data volume with display name = '%s', org = '%s', data_path = '%s' ..." % (display_name, org, data_path))
    ...
    def add_file(self, volume_id, filenames):
        print('In volume %s, adding files: %s ...' % (volume_id, filenames))
    ...
    def add_metadata(self, volume_id, filenames, metadata):
        print('In volume %s, adding metadata %s to files: %s ...' % (volume_id, metadata, filenames))
    ...
    def commit(self, volume_id, message=''):
        print('In volume %s, committing: %s ...' % (volume_id, message))
    ...
    def _run_command(self, command_args, should_assert=True):
        ...
        print('Command run: %s' % ' '.join(['mali'] + args))
        print('Command status: %d, output: %s' % (result.exit_code, result.output))
