...
def get_latest_pip_version(package, keywords, throw_exception=False):
    try:
        ...
        if sys.version_info >= (2, 7, 9):
            context = None
            create_unverified_context = getattr(ssl, '_create_unverified_context', None)
            if create_unverified_context is not None:
        ...
        latest_version = ServerProxy(pypi_server, transport=transport).package_releases(package)[0]
        ...
        logging.exception('could not check for new %s version:\n%s', package, e)
        return None
