...
def do_upload(bucket_name, object_name, body, signed_url_service, content_type, head_url, put_url, headers):
    # logging.debug('upload %s to %s', object_name, put_url)
    res = GCSUpload(signed_url_service).upload(bucket_name, object_name, body, content_type, head_url, put_url, headers)
...
class GCSUpload(GCSService):
    def upload(self, bucket_name, object_name, body, content_type=None, head_url=None, put_url=None, headers={}):
        logger.debug('upload %s (%s)', object_name, '{:,}'.format(len(body)))

        request = self._get_request(bucket_name, object_name, body, content_type, head_url, put_url, headers)
    ...
    def _get_request(self, bucket_name, object_name, body, content_type=None, head_url=None, put_url=None, headers={}):

        if not self.signed_url_service and put_url is None:
            service = self._get_authenticated_service()
        ...
        if head_url is None or put_url is None:
            if content_type is None:
                content_type = get_content_type(body)

            urls = self.signed_url_service.get_signed_urls(['PUT', 'HEAD'], [object_name], content_type, **GCSObjectStore.get_content_headers())
            put_url = urls['PUT'][0]
            head_url = urls['HEAD'][0]

        http = httplib2.Http()

        # HttpRequest postproc is called only on STATUS < 400
        def head_404_to_204(resp):
            if resp.status == 404:
                resp.status = 204
        ...
        request_head.add_response_callback(head_404_to_204)
...
class GCSObjectStore(ConnectionMixin, BaseObjectStore):
    ...
    def __init__(self, connection):
        ...
        self.__upload_pool = Pool(multiprocessing.cpu_count() * 2)
        self.__max_waiting_semaphore = Semaphore(multiprocessing.cpu_count() * 2 * 4)
    ...
    def upload(self, obj, content_type=None, head_url=None, put_url=None, headers=None):
        ...
        if self.__use_multiprocess:
            ...
            self.__upload_pool.apply_async(do_upload, args=(self.__bucket_name, path, data, None, content_type, head_url, put_url, headers), callback=self.on_upload_result)

        else:
            do_upload(self.__bucket_name, path, data, self._signed_url_service, content_type, head_url, put_url, headers)
