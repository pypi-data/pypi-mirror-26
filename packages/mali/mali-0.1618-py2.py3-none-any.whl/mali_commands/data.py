...
def _run_add_batches(paths, repo):
    ...
    with tqdm(total=total_files, desc="Adding files", unit=' files', ncols=80) as bar:
...
@click.pass_context
def add_to_data_volume(ctx, volumeid, files, commit, message, processes):
...
def __download_entity(config_prefix, volume_id, dest, item):
    from googleapiclient.errors import HttpError

    try:
        with with_repo(config_prefix, volume_id, read_only=True) as repo:
            ...
            if os.path.exists(dest_file):
                click.echo('file %s already exists, overriding' % dest_file, err=True)
...
@click.option('--processes', default=-1, type=int, required=False)
@click.pass_context
def clone_data(ctx, volumeid, dest, query, delete, batchsize, processes):
    ...
    try:
        with with_repo(ctx, volumeid, read_only=True) as repo:
            multi_process_control = MultiProcessControl(processes)
            ...
            with tqdm(total=total_data_points, desc="Downloading", unit=' data point', ncols=80) as bar:
                with closing(multi_process_control):
                    while True:
                        for item in results:
                            multi_process_control.execute(__download_entity, args=(ctx.obj.config_prefix, volumeid, dest, item), callback=lambda _: bar.update())
...
@click.pass_context
def add_to_metadata(
        ctx, volumeid, files, data, datapoint, datafile, property, propertyint, propertyfloat, update):
    now = time.time()
    ...
    with with_repo(ctx, volumeid) as repo:
        ...
        with tqdm(total=total_files, desc="Adding Metadata", unit=' files', ncols=80) as bar:
