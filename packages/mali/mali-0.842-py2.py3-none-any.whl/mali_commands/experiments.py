...
@click.pass_context
def update_graph(ctx, project_id, experiment_id, graph_id, scope, graph_name, graph_type,
                 graph_x_name, graph_x, graph_y_name, graph_y):
    """Send experiment external graph to an experiment.

    Experiment is identified by providing  both `--projectId` and `--experimentId`
    options. submitting graph with the same id more than once for the same expermient and scope will override its
    previous value.
    Example:

    To send graph to the 5th experiments ex_prerec graph of the project "123" in the validation scope, run

    \b
        mali experiments updateGraph --projectId 123 --experimentId 5  -sc validation -gid ex_prerec -gx '[0.1,0.5,0.8]' -gy '[0.9,0.5,0.2]'

    """

    update_graph_path = 'projects/{project_id}/experiments/{experiment_id}/graph/{graph_id}' \
        .format(project_id=project_id, experiment_id=experiment_id, graph_id=graph_id)
