...
class Config(object):
    def __init__(self, config_prefix):
        ...
        if config_file_abs_path is not None:
            parser.read([config_file_abs_path])
            readonly_parser.read([config_file_abs_path])
    ...
    def save(self):
        ...
        config_file_abs_path = find_first_file(self.config_prefix, _missing_link_config) if not self.config_prefix else None
        config_file_abs_path = config_file_abs_path or default_config_file(self.config_prefix, _missing_link_config)

        create_dir(os.path.dirname(config_file_abs_path))
