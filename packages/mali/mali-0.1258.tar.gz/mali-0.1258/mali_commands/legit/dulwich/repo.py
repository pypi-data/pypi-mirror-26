...
class Repo(BaseRepo):
    ...
    def __init__(self, repo_root, data_path=None):
        ...
        object_store = self.create_object_store()
        refs = self.create_ref_container()
    ...
    def _upload_staged_files(self, files_to_add):
        self.object_store.add_objects(files_to_add)

    def _set_entries(self, index, entries):
        index.set_entries(entries)

    class UploadFileRequest(object):
        def __init__(self, full_path, tree_path, embedded):
            self.st = os.lstat(full_path)
            if not isinstance(full_path, bytes):
                full_path = full_path.encode(sys.getfilesystemencoding())
            self.full_path = full_path.decode(sys.getfilesystemencoding())
            self.tree_path = tree_path
            if embedded:
                self.blob = blob_from_path_and_stat(full_path, self.st)
            else:
                self.blob = blob_from_reference_path(full_path)
            self.content_type = get_content_type(self.blob.data)

        @property
        def index_entry(self):
            from mali_commands.legit.dulwich.index import index_entry_from_stat
            return index_entry_from_stat(self.st, self.blob.id, 0)

    def _build_stage_files(self, fs_paths, index, embedded):
        root_path_bytes = self.data_path.encode(sys.getfilesystemencoding())
        ...
        for fs_path in fs_paths:
            ...
            try:
                yield Repo.UploadFileRequest(full_path, tree_path, embedded)
            except OSError:
    ...
    def stage(self, fs_paths, embedded):
        ...
        files_to_add = list(self._build_stage_files(fs_paths, index, embedded))
        self._upload_staged_files(files_to_add)

        entries = {}
        for added_file in files_to_add:
            entries[added_file.tree_path] = added_file.index_entry

        self._set_entries(index, entries)
