...
class TestMetadata(BaseTest):
    ...
    def testBigQuerySample_only(self):
        ...
        where_result = '((FARM_FINGERPRINT(CONCAT(_sha, "$seed")) / 2 + 0.5)>{sample:.4g})'.format(sample=1.0 - sample)
        vars_result = None
    ...
    def testBigQuerySample(self):
        ...
        where_result = '({field_name}="{field_value}")AND((FARM_FINGERPRINT(CONCAT(_sha, "$seed")) / 2 + 0.5)>{sample:.4g})'.format(
    ...
    def testBigQuerySample_twice(self):
        ...
        where_result = '((FARM_FINGERPRINT(CONCAT(_sha, "$seed")) / 2 + 0.5)>{sample1:.4g})AND((FARM_FINGERPRINT(CONCAT(_sha, "$seed")) / 2 + 0.5)>{sample2:.4g})'.format(
