...
def add(repo_path=".", paths=None, embedded=True):
    ...
    """
    ignored = set()
    with open_repo_closing(repo_path) as r:
        ignore_manager = r.get_ignore_filter_manager()
        if not paths:
            paths = list(
                get_untracked_paths(os.getcwd(), r.data_path, r.open_index()))
        ...
        for p in paths:
            relpath = os.path.relpath(p, r.data_path)
        ...
        r.stage(relpaths, embedded=embedded)
