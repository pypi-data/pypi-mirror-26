    ...
    def _query(self, sql_vars, select_fields, where, max_results=None, start_index=None):
        ...
        def query_all_meta_data_without_staging(version):
            query = """
                ...
                FROM (                
                    SELECT * EXCEPT(_phr), ROW_NUMBER() OVER (PARTITION BY _sha ORDER BY _ts DESC) row_number
                    FROM (
                        SELECT CASE 
                        ...
                        FROM (
                          ...
                          WHERE _ts <= ( 
                            SELECT MAX(_ts) 
                            FROM {dataset_name}.{metadata_table_name} 
                            WHERE _commit_sha = '{version}' AND ({where}) 
                            LIMIT 1
                          ) 
                        )
                        WHERE ({where})
                    ...
                    LEFT JOIN (
                        ...
                        SELECT * EXCEPT(row_number)
                        FROM (
                          SELECT name as _sha, sha as _hash, url as _url, ROW_NUMBER() OVER (PARTITION BY name) row_number
                          FROM {dataset_name}.{index_table_name}
                          WHERE ts <= ( 
                            SELECT MAX(_ts) 
                            FROM {dataset_name}.{metadata_table_name}
                            ...
                            LIMIT 1
                          ) AND ENDS_WITH(name, '.metadata') = FALSE
                        )
                        WHERE row_number = 1
                    )
                    USING(_sha)
