    ...
    def _write_query_params(self, f, protocol_version):
        ...
        if self.keyspace is not None:
            if ProtocolVersion.uses_keyspace_flag(protocol_version):
                ...
                raise UnsupportedOperation(
                    "Keyspaces may only be set on queries with Cassandra protocol version "
                    "5 or higher.")
    ...
    def send_body(self, f, protocol_version):
        ...
        if self.keyspace is not None:
            if ProtocolVersion.uses_keyspace_flag(protocol_version):
                ...
                raise UnsupportedOperation(
                    "Keyspaces may not be set on protocol version "
    ...
    def send_body(self, f, protocol_version):
        ...
        if self.op_type == ReviseRequestMessage.RevisionType.PAGING_BACKPRESSURE:
            if self.next_pages <= 0:
                ...
                raise UnsupportedOperation(
                    "Continuous paging backpressure not supported in current protocol version")
            else:
