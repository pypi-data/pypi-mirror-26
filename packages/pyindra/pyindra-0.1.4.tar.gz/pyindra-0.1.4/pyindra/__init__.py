...
class Indra:
    ...
    def __init__(self, url=None, port=8916, dir=None, lang="en", corpus="googlenews300neg", model="w2v", precision=10,
        ...
        self.precision_formatter = "{{0:.{}f}}".format(precision)
    ...
    def vectors(self, terms):
        results = self._submit(POST_VECTORS.format(self._url), self._get_payload(terms=terms))[TERMS]
        new_results = dict()
        for term, vector in results.items():
            new_results[term] = None if vector is None else [float(self.precision_formatter.format(e)) for e in vector]
