...
from strpy.bobo.util import quietprint, seq
...
def cumulative_match_characteristic(similarityMatrix, gtMatrix):
    ...
    #n_pos = np.sum(gtMatrix) # total number of positive ground truth (some probes may have no match)
    n_pos = np.sum(np.max(gtMatrix, axis=1)) # total number of positive ground truth (some probes may have no match or multiple matches)

    #recall = [np.sum(np.max(gtMatrix[:,0:r], axis=1)) / n_pos for r in rank]
    #recall = [np.sum(np.sum(gtMatrix[:,0:r], axis=1)) / n_pos for r in rank]
    recall = [np.sum(np.max(gtMatrix[:,0:r], axis=1)) / n_pos for r in rank]
    return (rank, recall)
