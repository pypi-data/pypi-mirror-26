...
if sys.platform == 'linux' or sys.platform == 'linux2':
    try:
        matplotlib.use('Agg') 
        #plt.switch_backend('gtkagg')  # linux display
    except:
        pass  # linux headless
...
def savefig(filename=None, handle=None, pad_inches=0):
    if handle is not None:
        plt.figure(handle)
    if filename is None:
        filename = temppng()
    plt.savefig(filename, bbox_inches='tight', pad_inches=pad_inches)
    return filename
...
def imbbox(img, xmin, ymin, xmax, ymax, bboxcaption=None, figure=None, bboxcolor='green', facecolor='white', facealpha=0.5, textcolor='black', textfacecolor='white', do_updateplot=True, do_imshow=True, colormap='gray', fontsize=10, captionoffset=(0,0)):
    ...
    # (x,y) bounding box is right and down, swap to right and up for plot
    plt.axvspan(xmin, xmax, ymin=1.0-np.float32(float(ymax)/float(img.shape[0])), ymax=1-np.float32(float(ymin)/float(img.shape[0])), edgecolor=bboxcolor, facecolor=facecolor, linewidth=3, fill=True, alpha=facealpha, label=None, capstyle='round', joinstyle='bevel')  

    if bboxcaption is not None:
        plt.text(xmin+captionoffset[0], ymin+captionoffset[1], bboxcaption, color=textcolor, bbox=dict(facecolor=textfacecolor, edgecolor=textcolor, alpha=1, boxstyle='round'), fontsize=fontsize)
