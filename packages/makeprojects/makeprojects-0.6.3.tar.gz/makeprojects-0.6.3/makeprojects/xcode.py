...
def calcuuid(input_str):
	temphash = hashlib.md5(burger.converttowindowsslashes(str(input_str))).hexdigest()
...
class Defaults(object):
	...
	def defaults(self, solution):
		...
		if solution.platform == makeprojects.core.PlatformTypes.ios:
			if solution.projecttype == makeprojects.core.ProjectTypes.library:
				...
				if solution.projecttype == makeprojects.core.ProjectTypes.game:
		...
		elif solution.platform == makeprojects.core.PlatformTypes.macosx:
...
class PBXBuildFile(object):
	...
	def write(self, fp):
		...
		if self.filereference.type == makeprojects.core.FileTypes.frameworks:
			ref_type = 'Frameworks'
		else:
		# It's a source file
			ref_type = 'Sources'
...
class PBXFileReference(object):
	...
	def write(self, fp):
		...
		if self.type != makeprojects.core.FileTypes.library and \
			self.type != makeprojects.core.FileTypes.exe and \
			self.type != makeprojects.core.FileTypes.frameworks:
			fp.write(' fileEncoding = 4;')

		# Each file type is handled differently

		if self.type == makeprojects.core.FileTypes.library:
			fp.write(' explicitFileType = archive.ar; includeInIndex = 0; path = ' + basename + \
				'; sourceTree = BUILT_PRODUCTS_DIR;')
		elif self.type == makeprojects.core.FileTypes.exe:
		...
		elif self.type == makeprojects.core.FileTypes.xcconfig:
			fp.write(' lastKnownFileType = text.xcconfig; name = ' + basename + '; path = xcode/' + \
				basename + '; sourceTree = BURGER_SDKS;')
		elif self.type == makeprojects.core.FileTypes.cpp:
...
class PBXSourcesBuildPhase(object):
	...
	def append(self, item):
		if item.filereference.type == makeprojects.core.FileTypes.glsl:
...
def generate(solution):
	...
	codefiles, includedirectories = solution.getfilelist([makeprojects.core.FileTypes.icns, \
		makeprojects.core.FileTypes.h, makeprojects.core.FileTypes.cpp, makeprojects.core.FileTypes.frameworks, makeprojects.core.FileTypes.exe, \
		makeprojects.core.FileTypes.library, makeprojects.core.FileTypes.glsl])
	...
	if solution.xcode.configfilename != None:
		configfilereference = xcodeprojectfile.addfilereference(solution.xcode.configfilename, makeprojects.core.FileTypes.xcconfig)
	else:
	...
	for framework in solution.xcode.frameworks:
		item = makeprojects.core.SourceFile(framework, '', makeprojects.core.FileTypes.frameworks)
	...
	for item in codefiles:
		...
		if item.type != makeprojects.core.FileTypes.rc and \
			item.type != makeprojects.core.FileTypes.r and \
			item.type != makeprojects.core.FileTypes.hlsl:
	...
	if solution.projecttype == makeprojects.core.ProjectTypes.library:
		if solution.platform == makeprojects.core.PlatformTypes.ios:
			...
			libextension = 'osx.a'
		outputfilereference = xcodeprojectfile.addfilereference('lib' + solution.projectname + idecode + libextension, makeprojects.core.FileTypes.library)
	else:
		if solution.projecttype == makeprojects.core.ProjectTypes.game:
			outputfilereference = xcodeprojectfile.addfilereference(solution.projectname + '.app', makeprojects.core.FileTypes.exe)
		else:
			outputfilereference = xcodeprojectfile.addfilereference(solution.projectname, makeprojects.core.FileTypes.exe)
	...
	if ioslibrary is True:
		devfilereference = xcodeprojectfile.addfilereference('lib' + solution.projectname + idecode + 'dev.a', makeprojects.core.FileTypes.library)
		simfilereference = xcodeprojectfile.addfilereference('lib' + solution.projectname + idecode + 'sim.a', makeprojects.core.FileTypes.library)
		...
		for item in xcodeprojectfile.pbxfilereferences:
			if item.type == makeprojects.core.FileTypes.cpp or item.type == makeprojects.core.FileTypes.glsl:
			...
			elif item.type == makeprojects.core.FileTypes.frameworks:
	...
	for item in xcodeprojectfile.pbxfilereferences:
		...
		if item.type == makeprojects.core.FileTypes.exe:
			groupproducts.append(item)
		elif item.type == makeprojects.core.FileTypes.library:
			groupproducts.append(item)
		elif item.type == makeprojects.core.FileTypes.frameworks:
	...
	elif solution.projecttype == makeprojects.core.ProjectTypes.screensaver:
		outputtype = 'com.apple.product-type.bundle'
	elif solution.projecttype == makeprojects.core.ProjectTypes.game:
	...
	if solution.platform == makeprojects.core.PlatformTypes.macosx:
		if solution.projecttype == makeprojects.core.ProjectTypes.tool:
	...
	if solution.finalfolder != None:
		...
		elif solution.projecttype == makeprojects.core.ProjectTypes.library:
