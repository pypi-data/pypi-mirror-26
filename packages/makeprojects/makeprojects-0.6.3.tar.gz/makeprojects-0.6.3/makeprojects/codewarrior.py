...
class Defaults(object):
	...
	def defaults(self, solution, configuration='Debug'):
		...
		if solution.platform.iswindows():
			...
			if solution.projecttype != makeprojects.core.ProjectTypes.library or \
...
class SETTING(object):
	...
	def write(self, fileref, level=4):
		...
		if self.value != None:
			fileref.write('<VALUE>' + self.value + '</VALUE>')
...
class MWProject_X86(object):
	def __init__(self, projecttype, filename):
		if projecttype == makeprojects.core.ProjectTypes.library:
			x86type = 'Library'
			extension = '.lib'
		else:
			x86type = 'Application'
...
class FILEREF(object):
	...
	def write(self, fileref, level=4):
		...
		if self.configuration != None:
			fileref.write(tabs2 + '<TARGETNAME>' + self.configuration + '</TARGETNAME>\n')
		fileref.write(tabs2 + '<PATHTYPE>Name</PATHTYPE>\n')
		fileref.write(tabs2 + '<PATH>' + self.filename + '</PATH>\n')
		fileref.write(tabs2 + '<PATHFORMAT>' + self.format + '</PATHFORMAT>\n')
		fileref.write(tabs + '</FILEREF>\n')
...
class SUBTARGET(object):
	...
	def write(self, fileref, level=4):
		...
		fileref.write(tabs2 + '<TARGETNAME>' + self.target.name + '</TARGETNAME>\n')
		fileref.write(tabs + '</SUBTARGET>\n')
...
class TARGET(object):
	...
	def write(self, fileref, level=2):
		...
		fileref.write(tabs2 + '<NAME>' + self.name + '</NAME>\n')
...
class ORDEREDTARGET(object):
	...
	def write(self, fileref, level=2):
		tabs = TAB * level
		fileref.write(tabs + '<ORDEREDTARGET><NAME>' + self.target.name + '</NAME></ORDEREDTARGET>\n')
...
def generate(solution):
	...
	codefiles, includedirectories = solution.getfilelist([ \
		makeprojects.core.FileTypes.h, makeprojects.core.FileTypes.cpp, makeprojects.core.FileTypes.rc, \
		makeprojects.core.FileTypes.hlsl, makeprojects.core.FileTypes.glsl])
	...
	listh = makeprojects.core.pickfromfilelist(codefiles, makeprojects.core.FileTypes.h)
	listcpp = makeprojects.core.pickfromfilelist(codefiles, makeprojects.core.FileTypes.cpp)
	listwindowsresource = []
	if solution.platform.iswindows():
		listwindowsresource = makeprojects.core.pickfromfilelist(codefiles, \
			makeprojects.core.FileTypes.rc)
	...
	for configuration in solution.configurations:
		...
		if solution.platform.iswindows():
			...
			target.settinglist.append(MWProject_X86(solution.projecttype, \
				solution.projectname + idecode + 'w32' + makeprojects.core.getconfigurationcode(configuration)))
