...
def make_aggregate_graphs(data, filename, ignore_initial_rounds):
    ...
    for col in columns:
        ...
        plot = figure(title=title, width=1200, height=600, sizing_mode='stretch_both',
        ...
        if (col not in ['round', 'id', 'index', 'index_ttl'] and
            ...
            for i in range(num_graphs):
                plot = Histogram(data[data['id'] == i][col],
                                 sizing_mode='stretch_both', width=1200, height=600)
    ...
    for col in data.columns:
        if col not in ['round', 'id', 'index']:
            ...
            plot = figure(title=title, sizing_mode='stretch_both',
                          output_backend='webgl',
                          toolbar_location='below', tools=TOOLS, width=1200, height=600)
