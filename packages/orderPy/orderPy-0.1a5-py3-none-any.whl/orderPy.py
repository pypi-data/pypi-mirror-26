...
def gatherFromArgClineCall(clineMode):
    ...
    order(argPath, argOrderType, extensionList, clineMode)


def gatherFromEmptyClineCall(clineMode):
    clinePath = getPath()
    clineOrderType = getOrderType()
    clineExtPath = getExtPath()
    extensionList = readExtFromFile(clineExtPath)
    order(clinePath, clineOrderType, extensionList, clineMode)
...
def order(workPath, orderType, fileTypes, callMode = -2):
