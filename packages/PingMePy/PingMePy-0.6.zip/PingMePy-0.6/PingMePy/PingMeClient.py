...
def deprecated(message):
    ...
    def decorator(func1):
        ...
        def new_func(*args, **kwargs):
            ...
            warnings.warn("Call to deprecated function {}. {}".format(func1.__name__, message), category=DeprecationWarning, stacklevel=2)
...
class PingMeClient(object):
    ...
    def get_maintenance_windows(self, group_id):
        """
        Get all maintenance windows with end dates in the future. [1]
        :param group_id: The id of the group in Cloud Manager / Ops Manager, if not known,
                         use the groupName and call get_group_by_name to get the Id
        :return:
        [1]: https://docs.opsmanager.mongodb.com/current/reference/api/maintenance-windows/
        """
        self.__test_parameter_for_string(group_id, 'group_id')
    ...
    def get_maintenance_window(self, group_id, maintenance_window_id):
        """
        Get a Single Maintenance Window by its ID. [1]
        :param group_id: The id of the group in Cloud Manager / Ops Manager, if not known,
                         use the groupName and call get_group_by_name to get the Id
        :param maintenance_window_id: The Id of the maintenance window to retrieve.
        :return:
        [1]: https://docs.opsmanager.mongodb.com/current/reference/api/maintenance-windows/
        """
        self.__test_parameter_for_string(group_id, 'group_id')
        self.__test_parameter_for_string(maintenance_window_id, 'maintenance_window_id')
    ...
    def create_maintenance_window(self, group_id, maintenance_window):
        """
        Create a new Maintenance Window for the group. [1]
        :param group_id: The id of the group in Cloud Manager / Ops Manager, if not known,
                         use the groupName and call get_group_by_name to get the Id
        :param maintenance_window: The maintenance window to create.
        :return:
        [1]: https://docs.opsmanager.mongodb.com/current/reference/api/maintenance-windows/
        """
    ...
    def update_maintenance_window(self, group_id, maintenance_window):
        """
        Update a Maintenance Window for the group. [1]
        :param group_id: The id of the group in Cloud Manager / Ops Manager, if not known,
                         use the groupName and call get_group_by_name to get the Id
    ...
    def delete_maintenance_window(self, group_id, maintenance_window_id):
        """
        Delete a new Maintenance Window for the group [1]
        :param group_id: The id of the group in Cloud Manager / Ops Manager, if not known,
                         use the groupName and call get_group_by_name to get the Id
        :return:
        [1]: https://docs.opsmanager.mongodb.com/current/reference/api/maintenance-windows/
    ...
    # region Groups
    # Last modified 2017-07-16
    def get_groups(self, tags=None):
        """
        Get the list of groups that the current username / API Key have access to. [1]
        :param tags: To query by group tags, use the tag query parameter. If you specify multiple
                     tag values, the group must have all the specified values. You must have the
                     Global Read Only or Global Owner role to view tags.
        :return: The list of groups.
        [1]: https://docs.opsmanager.mongodb.com/current/reference/api/groups/
        """

        url = urljoin(self.url, 'groups')

        if tags is not None:
            self.__test_parameter_for_array(tags, 'tags')
            url = '{0}?{1}'.format(url, self.__to_url_parameters('tag', tags))
    ...
    # region Metrics
    @deprecated('See the new Measurements API https://docs.opsmanager.mongodb.com/current/reference/api/measurements/')
    ...
    @staticmethod
    def __test_parameter_for_array(param, param_name):
        if not PingMeClient.__is_array(param):
            raise Exception(str.format('{0} must be an array and not empty'), param_name)

    @staticmethod
    def __to_url_parameters(param_name, values):
        param_array = []
        for val in values:
            param_array.append('{0}={1}'.format(param_name, val))
