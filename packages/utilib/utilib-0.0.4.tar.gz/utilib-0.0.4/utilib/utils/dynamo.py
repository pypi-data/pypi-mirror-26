...
class DynamoDBHelper:
    ...
    async def async_del_table(self, table_name):
        async with aioboto3.resource(
                'dynamodb',
                region_name='cn-north-1',
                aws_secret_access_key=self.aws_secret_access_key,
            ...
            await asyncio.get_event_loop().create_task(
                resource.meta.client.delete_table(TableName=table_name)
            )
        return True

    async def async_put_item(self, table_name, item):
        """
        插入单条数据
    ...
    async def async_batch_write_item(self, table_name, items):
        """
        批量插入item
        """
        async with aioboto3.resource(
    ...
    async def async_del_item(self, table_name, key):
        """
        删除数据
        :param key: 格式正确的json
        """
