...
from intermake import command, m_print_helper

from neocommand.data import constants
from neocommand.data.core import CORE
from neocommand.database.entities import  Node, Edge
from neocommand.database.endpoints import IOrigin, IEndpoint, NeoCsvFolderEndpoint, DbEndpoint
from neocommand.helpers.neo_csv_helper import NEO_TYPE_COLLECTION, NeoCsvFilename
from neocommand.extensions.plugins.basic import database
...
@command()
def parcel_transfer( input:IOrigin, endpoint:IEndpoint ):
    """
...
def parcel_run_cypher( endpoint:DbEndpoint, file: READ_CYPHER_FILE ) -> None:
    ...
    if files_needed:
        for file_needed in MCMD.iterate( files_needed, "Transferring files to import folder" ):
            ...
            dest = path.join( CORE.settings.import_folder, file_needed )
    ...
    try:
        for scriptlet in MCMD.iterate( scriptlets, "Running commands" ):
            if scriptlet.strip():
                database.cypher( code = scriptlet )
    finally:
...
def parcel_db( db_name: str, input: NeoCsvFolderEndpoint, run : bool = True ) -> str:
    ...
    if not binary_dir or not path.isdir(binary_dir):
        raise m_print_helper.SetupError( "neo4j_binary_folder", binary_dir )
    ...
    result.append( "./neo4j-import --into {0} {1} --multiline-fields=true --skip-bad-relationships=true --skip-duplicate-nodes=true --bad-tolerance=100000000 --ignore-empty-strings=true".format( db_name, script_text ) )
...
def parcel_bulk( input: NeoCsvFolderEndpoint, endpoint : DbEndpoint, run : bool = True ) -> str:
    ...
    for file in files:
        headers, first_line = __get_csv_head( file.filename )
        
        attributes, name_column, start_column, end_column = __create_attribute_script( headers, __neo4j_type_to_csv_neo4j_conversion, None )
        
        # Generate the "create" part of the statement
        if file.is_edge:
            assert name_column is None  , "{} {}".format(file.filename, name_column)
            assert start_column is not None     , "{} {}".format(file.filename, start_column)
            assert end_column is not None     , "{} {}".format(file.filename, end_column)
            ...
            assert name_column is not None, "{} {}".format(file.filename, name_column)
            assert start_column is None, "{} {}".format(file.filename, start_column)
            assert end_column is None, "{} {}".format(file.filename, end_column)
        ...
        statement = string_helper.bulk_replace( statement,
                                               label = file.label,
                                               creation = method,
                                               file_name = file_prefix + file.alone + constants.EXT_B42CSV,
    ...
    if run:
        # noinspection PyTypeChecker
        parcel_run_cypher( endpoint, READ_CYPHER_FILE( file_name ) )
...
def parcel_add( input: NeoCsvFolderEndpoint, run : bool = True ) -> str:
    ...
    for file in files:
        with open( file.filename, "r" ) as in_file:
            ...
            for row in reader:
                attributes, name_column, start_column, end_column = __create_attribute_script( headers, __neo4j_type_to_direct_neo4j_conversion, row )
...
def __create_attribute_script( headers, expression: Callable[ [ str, int, Optional[ List[ object ] ] ], Optional[ str ] ], row_data: Optional[ List[ object ] ] ):
    """
    ...
    for i, header in enumerate( headers ):
        if header.startswith( constants.PRIMARY_KEY_DECORATED_NAME ):
            ...
            row_value = expression( header, i, row_data )
...
# noinspection PyUnusedLocal
def __neo4j_type_to_csv_neo4j_conversion( header: str, index: int, row: Optional[ List[ object ] ] ) -> Optional[ str ]:
    """
    Given a type name in a CSV header (suitable for use with the bulk import tool), gets the conversion format suitable for a Neo4j Cypher query

    :returns a format statement containing {0} where the data is to be placed.
    """
    attr_name, attr_type = header.split( ":" )
    value = "r.`{0}`".format( header )
    
    if attr_type == NEO_TYPE_COLLECTION.INT:
        result = "toInt({0})".format( value )
    elif attr_type == NEO_TYPE_COLLECTION.FLOAT:
        result = "toFloat({0})".format( value )
    elif attr_type == NEO_TYPE_COLLECTION.STR:
        result = value
    elif attr_type == NEO_TYPE_COLLECTION.BOOL:
        result = "(case {0} when \"True\" then true else false end)".format( value )
    elif attr_type == NEO_TYPE_COLLECTION.STR_ARRAY:
        result = value
    else:
        raise SwitchError( "attr_type", attr_type )
    
    return "z.`{0}` = {1}".format( attr_name, result )


def __neo4j_type_to_direct_neo4j_conversion( header: str, index: int, row: Optional[ List[ object ] ] ) -> Optional[ str ]:
    """
    Given a type name in a CSV header (suitable for use with the bulk import tool), gets the conversion format suitable for a Neo4j Cypher query
    ...
    if attr_type == NEO_TYPE_COLLECTION.INT:
        ...
        result = '"{0}"'.format( str(value).replace( '"', "'" ) )
    elif attr_type == NEO_TYPE_COLLECTION.BOOL:
        ...
        result = str(value).replace( '"', "'" ).split( ";" )
    else:
        raise SwitchError( "attr_type", attr_type )
