...
class NeoCsvFilename( IVisualisable ):
    ...
    def visualisable_info( self ) -> UiInfo:
        return UiInfo( name = file_helper.get_filename_without_extension(self.filename),
                       ...
                       colour = EColour.BLUE,
                       icon = Res.edge if self.is_edge else Res.node,
...
class NeoCsvHeader:
    ...
    def __str__( self ):
        if self.__special == ENeoCsvSpecialHeader.NONE:
            return "NeoCsvHeader({} : {}{})".format( self.__name, self.__type, "[]" if self._is_array else "" )
        else:
    ...
    def from_decorated_name( file: NeoCsvFilename, decorated_name: str ) -> "NeoCsvHeader":
        ...
        assert decorated_name.count( ":" ) == 1, decorated_name
    ...
    def encompass_value( self, value: object ) -> bool:
        ...
        if self.__type.NO_ARRAY is NEO_TYPE_COLLECTION.STR:
            ...
            if (value is None) or (type_ in [ int, bool ]):
                ...
                self.__type = self.__type.to( NEO_TYPE_COLLECTION.STR )
                return True
        elif self.__type.NO_ARRAY == bool:
...
class NeoCsvWriter:
    ...
    def translate( self, value ) -> str:
        ...
        if type_ is list or type_ is tuple:
            # Output a list
            return ";".join( self.translate( x ) for x in value )
        elif type_ is bool:
...
class NeoCsvMultiWriter:
    ...
    def write_node( self, node: Node ) -> None:
        ...
        node.properties[ ENeoCsvSpecialHeader.UID ] = node.uid
        
        file.write_row( node.properties )
        
        del node.properties[ ENeoCsvSpecialHeader.UID ]
    ...
    def write_edge( self, edge: Edge ) -> None:
        ...
        edge.properties[ ENeoCsvSpecialHeader.START ] = edge.start.uid
        edge.properties[ ENeoCsvSpecialHeader.END ] = edge.end.uid
        
        file.write_row( edge.properties )
        
        del edge.properties[ ENeoCsvSpecialHeader.START ]
        del edge.properties[ ENeoCsvSpecialHeader.END ]
