...
from neocommand.helpers.neo_csv_helper import NEO_TYPE_COLLECTION, NeoCsvFilename, NeoCsvHeader
...
def __neo4j_type_to_csv_neo4j_conversion( nc_file_name : NeoCsvFilename, header_text: str, index: int, row: Optional[ List[ object]] ) -> Optional[ str]:
    ...
    return "z.`{0}` = {1}".format( header_text, result )


def __neo4j_type_to_direct_neo4j_conversion( nc_file_name : NeoCsvFilename, header_text: str, index: int, row: Optional[ List[ object]] ) -> Optional[ str]:
    """
    Given a type name in a CSV header (suitable for use with the bulk import tool), gets the conversion format suitable for a Neo4j Cypher query
