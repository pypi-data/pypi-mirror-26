...
def validate(name):
    def decorator(function):
        def wrapper(input):
            ...
            annos = yaml.load(f)
            for param in annos[name]["parameters"]:
                if(type(dic[param["name"]]).__name__ not in type_map.get(param["type"],[])):
                    raise TypeError(param["name"] + " has type " + type(dic[param["name"]]).__name__ + " but require " + param["type"])
