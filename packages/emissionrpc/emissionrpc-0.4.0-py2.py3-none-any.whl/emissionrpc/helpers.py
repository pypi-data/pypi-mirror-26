...
def check_ids(ids):
    ...
    if not (isinstance(ids, int) or isinstance(ids, list) or isinstance(ids, str)):
        raise EmissionBadParamater('ids shoul be a int, list or string')

    if isinstance(ids, list):
        for v in ids:
            if isinstance(v, float):
                raise EmissionBadParamater('ids shoul not be a float')
...
def check_method(method):
    ...
    if method not in TORRENT_ARGUMENTS.keys():
        raise EmissionBadParamater("'{0}' method does not exist".format(method))
...
def check_fields(method, args):
    ...
    def check_name(name, arguments):
        if name not in arguments.keys():
            raise EmissionBadParamater("'{0}' is not a valid argument".format(name))
...
def check_arguments(method, args):
    ...
    def check_value(name, value, arguments):
        if not isinstance(value, arguments[name][0]):
            raise EmissionBadParamater("'{0}' is not a valid value".format(value))
