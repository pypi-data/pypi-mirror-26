...
from rsplib.processing.consumer import RSPEngine, ASPEngine, StreamReasoner
...
def deploy(experiment, stream_running=True):
    engine = RSPEngine("http://"+experiment.engine()['host'], experiment.engine()['port']);
...
def execute(execution, stream_running=True, collect=False):
    ...
    engine = RSPEngine("http://"+execution.experiment.engine()['host'], execution.experiment.engine()['port']);
