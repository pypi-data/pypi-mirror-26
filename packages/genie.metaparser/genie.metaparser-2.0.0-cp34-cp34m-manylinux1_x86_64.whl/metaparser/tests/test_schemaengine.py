...
from metaparser.util.schemaengine import Fallback
from metaparser.util.schemaengine import (SchemaValueError, SchemaClassError, 
...
class TestPathClass(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        from metaparser.util.schemaengine import Path, Any
    ...
    def test_missing_from(self):
        ...
        missings = Path((1, Any(), Any())).missing_from(((1,2,3), 
                                                            (1,3,4)))
        self.assertEqual(missings, [(1,3,3), (1,2,4)])
...
class TestSchemaEngine(unittest.TestCase):
    ...
    def test_default_dict_nested_complex(self):
        ...
        self.assertEqual(data, {'a': {'b': {'c': 'default', 'd': 1}}})
    ...
    def test_infinite_fallback_loop(self):
        from metaparser.util.exceptions import SchemaFallbackLoopError
        schema = Schema({
            'a': Fallback(str, 'b'),
            'b': Fallback(str, 'a'),
        })
    ...
    def test_fallback_to_none(self):
        from metaparser.util.exceptions import SchemaFallbackError
        test_schema = {
            'a': {
                'b': {
                    'c' : {
    ...
    def test_and_callable(self):
        ...
        data = schema.validate('ott-xdm-100')
        self.assertTrue(isinstance(data, IPv4Address))
        self.assertEqual(data, IPv4Address(socket.gethostbyname('ott-xdm-100')))
