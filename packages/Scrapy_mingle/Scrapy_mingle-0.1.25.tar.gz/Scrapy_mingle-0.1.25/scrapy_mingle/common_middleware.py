...
class Common_Middleware(object):
    ...
    def update_proxy_cycle(self, delay=20):
        ...
        old_proxies = [proxy for proxy in self.proxy_pool['proxy']]
    ...
    def process_response(self, request, response, spider):
        ...
        if response.status != 200:
            logger.error(
                "response status {} proxy {} url {}".format(
                    response.status,
                    request.meta['proxy'],
                    request.url,
                ))
            if response.status in self.ban_code:
                # 在bancode里，权值快速下降
                self.proxy_node_table[this_proxy].weight = 1 if self.proxy_node_table[
                    this_proxy].weight > 5 else -1
            elif response.status in self.ignore_code:
                return response
            else:
                # 其他错误，权值下降
                self.proxy_node_table[this_proxy].weight -= max(
                    self.proxy_node_table[this_proxy].weight//2, 3)
            request.dont_filter = True
            self.use_next_proxy(request)
            return request
        else:
