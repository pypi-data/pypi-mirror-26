...
class DataCore(object):
    ...
    def build_single_terms_features(self, features=None):
        ...
        #list(map(lambda x: x.updateH(maxTF=maxTF, avgTF=avgTF, number_of_sentences=self.number_of_sentences, features=features), validTerms))
        list(map(lambda x: x.updateH(maxTF=maxTF, avgTF=avgTF, number_of_sentences=self.number_of_sentences, features=features), self.terms.values()))
    ...
    def getTerm(self, str_word, save_non_seen=True):
        ...
        isstopword = isstopword or unique_term in self.stopword_set
...
class composed_word(object):
    ...
    def updateH(self, features=None, isVirtual=False):
        sum_H  = 0.
        prod_H = 1. #god is not dead 1 1.0 8.1273016668805 0 4
        for (t, term_base) in enumerate(self.terms):
            ...
            if term_base.stopword:
                ...
                sum_H += term_base.H
                prod_H *= term_base.H
        # math.log(1.+self.tf, (2.+n)-len(self.terms)) \approx kw.tf 
        # self.H = prod_H / ( ( sum_H + 1 ) * self.tf * math.log(1.+self.tf, (2.+n)-len(self.terms)) )
        tf_used = self.tf
        if features != None and "KPF" not in features:
            tf_used = 1.
        if isVirtual:
...
class single_word(object):
    def __init__(self, unique, idx, graph):
        ...
        self.pagerank = 1.
    def updateH(self, maxTF, avgTF, number_of_sentences, features=None):
