...
class JsonApiPropertiesClassCreator(object):
    ...
    def __init__(self,
        ...
        :param parent_mixins: A list of class imports to add and to use as mixins for the parent class
                              e.g. [u'from classutils import Observable',]
        ...
        if folder_path is None:
            # Get the path to the folder containing the generate script
            frame = inspect.stack()[1]
            module = inspect.getmodule(frame[0])
            self.folder_path = os.path.dirname(module.__file__)
        else:
    ...
    def mixin_import_string(module,
                            name):
        return (u"from {module} import {name}"
    ...
    def write_source_code(self,
        ...
        if not filename.endswith(u'.py'):
            filename += u'.py'
        file_path = os.path.join(self.folder_path, self.GENERATED_CLASSES_FOLDER, filename)
...
def generate_api_files(definitions):
    ...
    # TODO: Fix this check
    #
    # definitions_file_date = os.path.getmtime(api_definition_file)
    #
    # generated_classes_folder = os.path.join(folder_path,
    #                                         JsonApiPropertiesClassCreator.GENERATED_CLASSES_FOLDER)
    #
    # date_last_file_was_generated = max([os.path.getmtime(os.path.join(generated_classes_folder, f))
    #                                     for f in os.listdir(generated_classes_folder)])
    # if date_last_file_was_generated < definitions_file_date:
    #     logging.debug(u'Definitions are older than the generated code. Not generating code for {api_definition_file}'
    #                   .format(api_definition_file=api_definition_file))
    #    return
    ...
    for class_params in api:
        JsonApiPropertiesClassCreator(folder_path=folder_path,
