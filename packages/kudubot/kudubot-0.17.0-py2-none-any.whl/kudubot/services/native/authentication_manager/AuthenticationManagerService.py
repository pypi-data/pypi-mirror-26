u"""
Copyright 2015-2017 Hermann Krumrey

This file is part of kudubot.

kudubot is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 3 of the License, or
(at your option) any later version.

kudubot is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
GNU General Public License for more details.

You should have received a copy of the GNU General Public License
along with kudubot.  If not, see <http://www.gnu.org/licenses/>.
"""

from __future__ import absolute_import
from typing import List, Dict
from kudubot.entities.Message import Message
from kudubot.services.HelperService import HelperService
from kudubot.services.AuthenticatedService import AuthenticatedService


class AuthenticationManagerService(HelperService, AuthenticatedService):
    u"""
    Lets admins manage admin rights of other users and blacklist them.
    """

    @staticmethod
    def define_identifier():
        u"""
        Defines the identifier for this service

        :return: The service's identifier
        """
        return u"authentication_manager"

    def define_command_name(self, language):
        u"""
        Defines the command prefix for the service.
        By default, '/' followed by the service identifier from
        self.define_identifier() is used.

        :param language: The language in which to define the command name
        :return: The command name in the specified language
        """
        return {
            u"en": u"/auth_manage",
            u"de": u"/auth_verwalten"
        }[language]

    @staticmethod
    def define_requirements():
        u"""
        Defines required Services for this Service

        :return: The list of required services
        """
        return []

    def determine_language(self, message):
        u"""
        Determines the language of a message

        :param message: The message to check for the language
        :return: The language of the message
        """
        body = message.message_body.lower()
        for language in self.supported_languages():
            if body.startswith(self.define_command_name(language)):
                return language
        return self.get_stored_language_preference(message.sender)

    def define_language_text(self):
        u"""
        Defines the dictionary used for translating strings using the
        self.reply_translated() or self.translate() methods

        The format is:

            term: {lang: value}

        :return: The dictionary for use in translating
        """
        return {
            u"@{LIST}": {u"en": u"list", u"de": u"auflisten"},
            u"@{ADMIN_MAKE}": {u"en": u"makeadmin", u"de": u"adminmachen"},
            u"@{BLACKLIST}": {u"en": u"blacklist", u"de": u"blacklisten"},
            u"@{LIST_TITLE}": {u"en": u"User List", u"de": u"Nutzerliste"},
            u"@{LIST_BODY}": {u"en": u"User List", u"de": u"Nutzerliste"},
            u"@{ADMIN_MAKE_TITLE}": {u"en": u"Admin", u"de": u"Administrator"},
            u"@{ADMIN_MAKE_BODY}": {u"en": u"The user was granted administrative "
                                         u"privileges.",
                                   u"de": u"Dem Nutzer wurden Adminrechte "
                                         u"zugeschrieben."},
            u"@{BLACKLIST_TITLE}": {u"en": u"Blacklisted", u"de": u"Blacklisted"},
            u"@{BLACKLIST_BODY}": {u"en": u"The user was blacklisted",
                                  u"de": u"Der Nutzer wurde geblacklisted"}
        }

    def define_help_message(self, language):
        u"""
        Defines the help message of the service in various languages

        :param language: The language to use
        :return: The help message in the language
        """
        return {
            u"en": u"Tool for administrators to manage blacklisted and "
                  u"administrative privileges.",
            u"de": u"Tool für Administratoren, um blackgelistete Nutzer und"
                  u"administrative Rechte zu verwalten"
        }[language]

    def define_syntax_description(self, language):
        u"""
        Defines the Syntax description of the Service in various languages

        :param language: The language in which to return the syntax message
        :return: The syntax message
        """
        return {
            u"en": u"/auth_manage list\n"
                  u"/auth_manage makeadmin <ID>\n"
                  u"/auth_manage blacklist <ID>",
            u"de": u"/auth_verwalten auflisten\n"
                  u"/auth_verwalten adminmachen <ID>\n"
                  u"/auth_verwalten blacklisten <ID>"
        }[language]

    # noinspection SqlDialectInspection,SqlDialectInspection
    # noinspection SqlNoDataSourceInspection
    def handle_message(self, message):
        u"""
        Handles an applicable message
        Checks the mode and handles accordingly

        :param message: The message to handle
        """
        args = message.message_body.lower().split(u" ")
        mode = args[1]

        if mode in [self.translate(u"@{LIST}", language)
                    for language in self.supported_languages()]:
            list_body = u"@{LIST_BODY}:\n\n"
            results = self.connection.db.execute(
                u"SELECT id, display_name, address "
                u"FROM address_book").fetchall()

            for result in results:
                user_id = int(result[0])
                list_body += unicode(user_id) + u": " + unicode(result[1])
                list_body += u"<" + unicode(result[2]) + u"> "

                if self.connection.authenticator.is_admin(user_id):
                    list_body += u"✅"
                elif self.connection.authenticator.is_blacklisted(user_id):
                    list_body += u"❌"

                list_body += u"\n"

            self.reply_translated(u"@{LIST_TITLE}", list_body.strip(), message)

        elif mode in [self.translate(u"@{ADMIN_MAKE}", language)
                      for language in self.supported_languages()]:
            user_id = int(args[2])
            self.connection.authenticator.make_admin(user_id)
            self.reply_translated(
                u"@{ADMIN_MAKE_TITLE}", u"@{ADMIN_MAKE_BODY}", message)

        elif mode in [self.translate(u"@{BLACKLIST}", language)
                      for language in self.supported_languages()]:
            user_id = int(args[2])
            self.connection.authenticator.blacklist(user_id)
            self.reply_translated(
                u"@{BLACKLIST_TITLE}", u"@{BLACKLIST_BODY}", message)

    def is_applicable_to(self, message):
        u"""
        Checks if a Message is applicable to this Service

        :param message: The message to check
        :return: True if the Message is applicable, False otherwise
        """
        language = self.determine_language(message)
        print self.define_command_name(language)
        if not self.starts_with_command_keyword(message, language):
            return False

        body = message.message_body.lower().split(u" ")

        if len(body) == 2:
            print u"O"
            if body[1] == self.translate(u"@{LIST}", language):
                return True

        elif len(body) == 3:
            print u"A"
            if body[1] in [
                self.translate(u"@{BLACKLIST}", language),
                self.translate(u"@{ADMIN_MAKE}", language)
            ]:
                try:
                    int(body[2])
                    return True
                except ValueError:
                    return False

        print u"I"
        return False
