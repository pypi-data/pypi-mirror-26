...
def hash_the_WARfile(basedir,warfile):
    da_fullfile = "{0}/tomcat/webapps/{1}".format(basedir, warfile)
    if os.path.exists(da_fullfile):
        pprint.info("   {0}  trouvé".format(warfile))
        da_hash = sha256_checksum(da_fullfile)
...
def check_iParapheur_version_is_valid(basedir):
    ...
    hash_the_WARfile(basedir, "iparapheur.war")
...
check_iParapheur_version_is_valid(defaut_iparapheur_root)
