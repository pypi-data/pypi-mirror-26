...
class Context(object):
    ...
    def __new__(cls, *args, **kwd):
        if Context.__instance is None:
            ...
            cls.token = None
            cls.mode = None
            cls.backtest_start_time = None   # 回测策略开始时间
            cls.backtest_now_is_vaild = False  # 这个用来判断是否回测可以用context.now
...
class BarTask(object):
    ...
    def state_analysis(self):
        ...
        # 要判断一下是不是订阅的代码都到了啊。
        if self.wait_symbols == len(self.symbols):
            return self.sign_waiting(context.now)

        # 如果超时时间到了， 任务就必须执行， 直接设is_ready = True
        if (context.now - self.wait_start_time).total_seconds() >= self.overtime:
            print ('context.now', context.now)
            print ('self.wait_start_time', self.wait_start_time)
