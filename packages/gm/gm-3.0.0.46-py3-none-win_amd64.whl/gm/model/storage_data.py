...
class SubBarData(object):
    ...
    def _inside_get_data(self, symbol, frequency, eob):
        ...
        if not self._data.get(sub_tag):
            return
        data_deque = self._data[sub_tag].items
        # 暂时这样观察下性能
        if len(data_deque) >= 2:
            data = [data_deque[-2], data_deque[-1]]
        elif len(data_deque) == 1:
            data = [data_deque[-1]]
        else:
            data = []
    ...
    def data_is_ready(self, sub_tag, eob):
        if not self._data.get(sub_tag):
            return False
        eod_list = [info['eob'] for info in self._data.get(sub_tag).items][-2:]
...
class BarInfoQueue(object):
    ...
    def data_init(self):
        # 回测init的时候now不能用
        if context.mode == MODE_BACKTEST and not context.backtest_now_is_vaild:
            init_time = context.backtest_start_time
        else:
            init_time = context.now.strftime("%Y-%m-%d %H:%M:%S")

        pre, symbol, frequency = self.sub_id.split(':')
        from gm.api import history_n
        datas = history_n(symbol, frequency, self.count, end_time=init_time)
        # 不同市场收市时间均设为15：15
        if frequency == '1d':
            for data in datas:
