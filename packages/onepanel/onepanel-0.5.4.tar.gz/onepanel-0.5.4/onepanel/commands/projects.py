...
class Project:
    ...
    def save(self, home):
        ...
        # exclude .onepanel-project from git
        exclude_file = os.path.join(home, '.git/info/exclude')
        with open(exclude_file,'a+') as f:
            f.write('.onepanel-project\n')
...
def create_project(ctx, account_uid, home):
    ...
    if Project.exists_remote(project_uid, remote_list):
        if Project.exists_local(home):
            ...
            project.save(home)
            GitWrapper().init(home, account_uid, project_uid)
...
@projects.command('get', help='Get <account_uid>/<uid> project from server.')
@click.argument('path', type=click.Path())
@click.argument('directory', type=click.Path(), required=False)
@click.pass_context
@login_required
def projects_get(ctx, path, directory):
    conn = ctx.obj['connection']
...
@projects.command('push', help='Push changes to onepanel repository')
@click.pass_context
@login_required
