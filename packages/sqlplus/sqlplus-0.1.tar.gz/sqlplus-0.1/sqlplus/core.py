...
class SQLPlus:
    ...
    def read(self, tname, cols=None, where=None, group=None, roll=None):
        """Generates a sequence of rows from a table.
        """
        # Of course you can make a rule for both but it only causes trouble
        if group and roll:
            raise ValueError('Use either roll or group, not both')
