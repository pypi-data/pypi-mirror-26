...
def check_service_availability(host,
    ...
    :return:            (int)           First port successfully connected to
    """
    ...
    try:
        ...
        logging.error(u'Failed to connect to port {p}: {e}'.format(p=port,
                                                                   e=err))

        # if we still have ports to check attempt next connection
        if last_port is not None and port is not last_port and alt_ports:
            logging.info(u'Attempting connection on alternate port ({port})'.format(port=alt_ports[0]))
            ...
            result = check_service_availability(**connect_kwargs)
