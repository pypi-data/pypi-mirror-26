...
import time
# import re
from python.converter.excel_util import Cell
...
"""
def vlookup(item, excel_rows, col_index_num, case_sensitive=False):
    # Get the range from excel and put it in a list
    excel_rows = [x for x in excel_rows if x is not None]
    column_dict = create_column_dict(excel_rows, col_index_num - 1, case_sensitive)
