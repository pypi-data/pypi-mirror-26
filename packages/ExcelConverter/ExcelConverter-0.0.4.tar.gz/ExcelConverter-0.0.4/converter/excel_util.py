...
import re
import string
from python.converter.excel_lib import *
    ...
    def compile(self):
        ...
        try:
            if is_compile_allowed(self, self):
                ...
                # if isinstance(self.value, str) and re.match('[A-Za-z]+', self.value):
                #     self._compiled_expression = self.value
                # else:
                #     self._compiled_expression = compile(self.python_expression, '<string>', 'eval')
...
def resolve_range(rng, flatten=False, sheet=''):
    ...
    if start_row and not end_row or not start_row and end_row:
        raise Exception("One row is missing (both must be (not) present, start row: " + start_row + ", end row: " + end_row)
