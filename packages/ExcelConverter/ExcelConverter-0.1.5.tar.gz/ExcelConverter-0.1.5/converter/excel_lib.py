...
@case_sensitive: Checks if the item is Uppercase if so set to Lowercase
"""
...
def vlookup_column(item, column_values, case_sensitive=False):
    if case_sensitive:
        ...
        return column_values[item] if item in column_values else None

"""
Method Doc
@Item: The value you want to find in a horizontal row
@Rows: All the excel rows in the given range
@Row_index_num: Specifies in which row to find the value
@case_sensitive: Checks if the item is Uppercase if so set to Lowercase
"""
...
def iferror(lookup_value, value_if_not_error):
    # list of errors that are used in excel
    error_list = ['#DIV/0!', '#N/A!', '#NAME?', '#NULL!', '#NUM!', '#REF!', '#VALUE!']

    for value in error_list:
        if value == lookup_value:
            return value
        else:
            return value_if_not_error
...
def counta(range):
    ...
    range = [x for x in range if x is not None]
