            ...
            if ast is not None:
                ...
                for dep in deps:
                    ...
                    if is_range(dep):
                        ...
                        rng_address = create_address(dep, sheet=cursheet)
...
#                        if rng.address() in cellmap:
                        if rng_address in cellmap:
                            ...
                            rng = None
                            try:
                                cells, nrows, ncols, rng_details = Cell.make_cells(self.excel, dep, sheet=cursheet)
                                # update the original range as the returned one is probably mor specific if it wasn't a list
                                rng = CellRange(rng_details if rng_details else rng_address, sheet=cursheet)
