...
class ExcelCompiler(object):
    ...
    def gen_graph_multi(self, ranges):
        ...
        try:
            ...
            seeds, nr, nc = Cell.make_cells(self.excel, ranges)
            seeds = list(flatten(seeds))
            print(("Ranges [%s] expanded into [%s] cells" % (ranges, len(seeds))))
            return self.gen_graph_intern(seeds)
        except Exception as ex:
            raise Exception("Error making cells, ranges: [%s], excel: [%s], seeds: [%s]" % (ranges, self.excel, seeds)) from ex
    ...
    def gen_graph_intern(self, seeds):
        ...
        while todo:
            ...
            try:
                ...
                raise Exception(
                    "Error compiling/cell2code cell: [%s], todo: [%s], seeds: [%s]" % (c1, todo, seeds)) from ex
                    ...
                    if is_range(dep):
                        ...
                        if rng.address() in cellmap:
                            ...
                            try:
                                ...
                                raise Exception(
                                    "Error making cells, cell: [%s], ast: [%s], dep: [%s], rng: [%s], todo: [%s], seeds: [%s]" %
                                    (c1, ast, dep, rng, todo, seeds)) from ex
    ...
    def evaluate(self, cell, is_addr=True):
        if is_addr:
            try:
                ...
                raise Exception("KeyError: %s, %s" % (ex, self.cellmap))
        ...
        try:
            ...
            if str(ex).startswith("Problem evalling"):
                raise ex
            else:
                raise Exception("Problem evalling: %s for %s, %s" % (ex, cell.address(), cell.python_expression))
        return cell.value
