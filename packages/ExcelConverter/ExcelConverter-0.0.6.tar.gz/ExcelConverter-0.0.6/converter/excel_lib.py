...
def hlookup(item, excel_rows, col_index_num, case_sensitive=False):
    excel_rows = [x for x in excel_rows if x is not None]
    row_dict = create_row_dict(excel_rows, col_index_num, case_sensitive)
    return hlookup(item, row_dict, case_sensitive)

def create_row_dict(excel_rows, col_index_num, case_senstive=False):
    if case_senstive:
...
@excel_rows: Each row specified in the lookup range
...
def vlookup(item, excel_cols, col_index_num, case_sensitive=False):
    # Get the range from excel and put it in a list
    excel_cols = [x for x in excel_cols if x is not None]
    column_dict = create_row_dict(excel_cols, col_index_num - 1, case_sensitive)
    return vlookup_column(item, column_dict, case_sensitive)
...
"""
def vlookup_column(item, column_dict, case_sensitive=False):
    if not case_sensitive:
