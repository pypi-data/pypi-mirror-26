...
class Cell(object):
    ...
    # TODOEd: Continue: split up the ranges in a extra method
    @staticmethod
    ...
    def make_cells(excel: object, rng: object, sheet=None) -> object:
        ...
        def convert_range(rng, sheet=None):
            ...
            # TODOEd: de ord() below werkt niet als het 2 karakters zijn.
            try:
                # Check if the shorthand range is specified (A:B) instead of (A1:B99)
                if (numcols < 0 and numrows):
                    numrows = min(len(fs), len(vs))
                    numcols = len(fs[0]) if numrows > 0 else numcols
                    for row_nr in range(numrows):
                        row = []
                        for cell_nr in range(numcols):
                            col = chr(ord(start) + cell_nr)
                            a = col + str(row_nr + 1)
                            f = fs[row_nr][cell_nr]
                            ...
                            if (ord(col) > ord(end)):
                                raise Exception("Illegal cell address: [%s]. It can't be higher then the end: [%s], "
                                                "cell: [%s], row_nr: [%s], cell_nr: [%s], ads: [%s], fs: [%s], vs: [%s],"
                                                "range: [%s]" % (a, end, cl, row_nr, cell_nr, ads, fs, vs, range))
                            print("Created cell: [%s]" % (cl))
                            row.append(cl)
                        cells.append(row)
                    rng = start + '1:' + col + str(row_nr)
                else:
