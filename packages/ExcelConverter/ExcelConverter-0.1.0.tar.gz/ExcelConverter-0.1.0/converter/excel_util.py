...
import re
import string
# from converter.excel_lib import *
...
class Cell(object):
    ...
    def __init__(self, address, sheet, formula, value):
        ...
        if self.value is not None:
            if re.findall("\d+\.\d+", value):
                self.value = [list(map(float, x)) for x in value]
                self.value += (value)
            elif re.findall(r"[0-9]+", value):
                self.value = [list(map(int, x)) for x in value]
                self.value += (value)
            else:
                self.value = [list(map(str, x)) for x in value]
                self.value += (value)
            print(self.value)
        """
    ...
    @staticmethod
    def make_cells(excel: object, ranges: object) -> object:
        """
        :rtype: object
        """
        # cells = []
        ...
        def convert_range(range, sheet=None):
            ...
            try:
                for it in (list(zip(*x)) for x in zip(ads, fs, vs)):
                    ...
                    for c in it:
                        ...
                        if f is not None and len(f) < 2:
                            raise Exception("Wrong formula encountered: [%s], a: [%s], v: [%s], ads: [%s], fs: [%s], "
                                            "vs: [%s], range: [%s], cell: [%s]" % (f, a, v, ads, fs, vs, range, cl))
                        row.append(cl)
                ...
                raise Exception(
                    "Error creating row cells for range [%s], seet: [%s], ads: [%s], num_rows: [%s], num_cols: [%s], formula: [%s],"
                    " values: [%s], cells: [%s]" % (range, sheet, ads, numrows, numcols, fs, vs, cells)) from ex

        def create_range_exception(range, cell, sheet):
            return create_exception(range, cell, sheet, "Error converting")

        def create_resolve_exception(range, cell, sheet):
            return create_exception(range, cell, sheet, "Error resolving")

        def create_exception(range, cell, sheet, msg):
            return Exception(msg + ", range: [%s], cell: [%s], sheet: [%s], cells: [%s], excel: [%s]"
                             % (range, cell, sheet, result, excel))

        for sheet, range in ranges.items():
            # starting points
            ...
            if isinstance(range, list):  # if a list of cells
                ...
                for cell in range:
                    if is_range(cell):
                        try:
                            ...
                            raise create_range_exception(range, cell, sheet) from ex
                    else:
                        try:
                            c = Cell.resolve_cell(excel, cell, sheet=sheet)
                            cells.append(c)
                        except Exception as ex:
                            raise create_resolve_exception(range, cell, sheet) from ex

                # cells = list(flatten(cells))
                result.extend(list(flatten(cells)))
                return result, -1, -1  # numrows and numcols are irrelevant here, so we return nr=nc=-1
                ...
                if is_range(range):
                    try:
                        ...
                        raise create_range_exception(range, None, sheet) from ex
                    ...
                    try:
                        ...
                        raise create_resolve_exception(range, None, sheet) from ex
