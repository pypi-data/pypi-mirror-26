...
class Cell(object):
    ...
    @staticmethod
    def make_cells(excel: object, range: object, sheet: object = None) -> object:
        """
        :rtype: object
        """
        cells = []
        ...
        def create_range_exception(cell, sheet):
            return create_exception(cell, sheet, "Error converting")

        def create_resolve_exception(cell, sheet):
            return create_exception(cell, sheet, "Error resolving")

        def create_exception(cell, sheet, msg):
            return Exception(msg + ", range: [%s], cell: [%s], sheet: [%s], cells: [%s], excel: [%s]"
                             % (range, cell, sheet, cells, excel))
        ...
        if isinstance(range, list):  # if a list of cells
            for cell in range:
                if is_range(cell):
                    try:
                        ...
                        raise create_range_exception(cell, sheet) from ex
                else:
                    try:
                        c = Cell.resolve_cell(excel, cell, sheet=sheet)
                        cells.append(c)
                    except Exception as ex:
                        raise create_resolve_exception(cell, sheet) from ex
            ...
            if is_range(range):
                try:
                    ...
                    raise create_range_exception(None, sheet) from ex
                ...
                try:
                    ...
                    raise create_resolve_exception(None, sheet) from ex
