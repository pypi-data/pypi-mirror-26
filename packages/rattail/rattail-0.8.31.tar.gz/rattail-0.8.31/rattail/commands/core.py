...
Commands:\n""".format(self))
    ...
    def run(self, *args):
        ...
        log.debug("running '{} {}' with args: {}".format(self.name, cmd.name, args.argv))
...
class Upgrade(Subcommand):
    ...
    def run(self, args):
        ...
        if args.execute:
            ...
            log.info("will now execute upgrade: {}".format(upgrade))
            if not args.dry_run:
                handler.mark_executing(upgrade)
                session.commit()
                handler.do_execute(upgrade, user, progress=self.progress)
            log.info("user '{}' executed upgrade: {}".format(user.username, upgrade))
