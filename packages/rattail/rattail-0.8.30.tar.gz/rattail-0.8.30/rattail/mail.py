...
class Email(object):
    ...
    def make_message(self, data={}, attachments=[], inlines=[], subject_prefix=UNSPECIFIED,
        ...
        if txt_template and html_template:
            ...
            txt_part = MIMEText(txt_template.render(**data))
            txt_part.set_charset('utf_8')

            html_part = MIMEText(html_template.render(**data), _subtype='html')
            html_part.set_charset('utf_8')
            if inlines:
            ...
            msg = MIMEText(txt_template.render(**data))
            msg.set_charset('utf_8')
            if attachments:
                msg = MIMEMultipart(_subtype='mixed', _subparts=[msg] + attachments)

        elif html_template:

            msg = MIMEText(html_template.render(**data), _subtype='html')
            msg.set_charset('utf_8')
            if inlines:
