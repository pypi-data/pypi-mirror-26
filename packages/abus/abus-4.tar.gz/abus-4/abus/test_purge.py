...
class PurgeTests(AbusTestBase):
   def test_purge_e2e(self):
      ...
      def contents_runs(expected_content_size, expected_runs_size):
         content= set(direntry.name for direntry in self.find_files(self.archivedir)) - {"index.sl3"}
         runs= set(n for n in content if n.endswith(".lst"))
         content.difference_update(runs)
         self.assertEqual(len(content), expected_content_size)
   ...
   def test_db_get_purgeable_backups(self):
      ...
      with database.connect(self.databasepath) as db:
         ...
         for checksum in all_checksums:
               db.remember_archivedir(checksum, checksum[-2:])
         del all_checksums
