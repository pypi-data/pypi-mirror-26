...
def lib_show(library, json_output):
    ...
    lib = get_api_result("/v1/lib/info/%d" % lib_id, cache_valid="1d")
    if json_output:
    ...
    for key in ("frameworks", "platforms"):
        if key not in lib or not lib[key]:
            continue
        blocks.append(("Compatible %s" % key, [lib[key]]))
...
def lib_register(url_or_file):
    if (url_or_file.startswith("http://") or
            url_or_file.startswith("https://")):
        result = get_api_result("/v1/lib/registerUrl", data=json.dumps({"config_url": url_or_file}))
    elif isfile(url_or_file):
        if (getsize(url_or_file) > 1024*1024):
            raise exception.InvalidLibFile(url_or_file)
        elif url_or_file.endswith(".tar.gz"):
            contentType = "application/gzip"
        elif url_or_file.endswith(".zip"):
            contentType = "application/zip"
        else:
            raise exception.InvalidLibConfURL(url_or_file)

        files = {'file': (url_or_file, open(url_or_file, 'rb'), contentType)}
        result = get_api_result("/v1/lib/registerFile", files=files)
    else:
        raise exception.InvalidLibConfURL(url_or_file)
