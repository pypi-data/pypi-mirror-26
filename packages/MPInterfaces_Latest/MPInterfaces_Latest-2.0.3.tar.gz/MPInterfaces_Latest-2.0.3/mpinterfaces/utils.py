...
def get_magmom_init(poscar,mag_init_file='mag_inits.yaml'):
    """
    ...
    try:
       ...
       logger.warn('mag_inits.yaml not found, so 0.5 is being used as initialization \
...
def load_config_vars(config_dict):
    ...
    if os.path.exists(PACKAGE_PATH):
       ...
       fireworks_path = PACKAGE_PATH.replace('/mpinterfaces','/fireworks/user_objects/queue_adapters')
...
def add_mem_submit_file(orig_file,pmem):
    runjob = open(orig_file,'a')
