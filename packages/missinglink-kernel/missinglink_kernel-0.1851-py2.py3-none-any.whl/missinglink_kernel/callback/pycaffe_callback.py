        ...
        class WrappedSolver(solver_class):
            ...
            def forward_test_net_all(self, weights_hash, blobs=None, start=None, end=None, index=0, **kwargs):
                ...
                for i in range(callback.test_iter[index]):
                    ...
                    iteration_metric_data = self.get_blobs(net)
                    for metric_name, metric_value in iteration_metric_data.items():
                        metric_data.setdefault(metric_name, 0)
                    ...
                    if found_layer_names:
                        ...
                        predictions = unpack_nested_list(predictions) if not np.array_equal(predictions, []) and len(predictions.shape) > 1 else predictions
                        probabilities = unpack_nested_list(probabilities) if not np.array_equal(probabilities, []) and len(probabilities.shape) > 1 else probabilities
                ...
                for key in metric_data.keys():
                    metric_data[key] /= callback.test_iter[index]
