...
class Vizualization:
    ...
    def request_clusterizer(self, method, params):
        ...
        if self.clusterizer.loadable(version=self.version):
            self.clusterizer.load_cluster(version=self.version)
        else:
            self.clusterizer.fit(xs)
    ...
    def save_clusterization(self, name_clusters='clusters'):
        ...
        pickle.dump(
                (self.version, self.clusterizer.get_name(), self.left_clicks),
                open(os.path.join(self.model_path, 'user_clusters', cache_filename), 'wb')
                )
    ...
    def load_clusterization(self, name):
        ...
        version_to_load, clusterizer_name, left_clicks_to_reproduce = pickle.load(
                open(os.path.join(self.model_path, 'user_clusters', cache_filename), 'rb'),
                )
