...
@click.option("-d", "--dry-run",
              is_flag=True,
              help="Print tickets which would be created, but don't do anything")
@catch_jira_errors
def bulk_create(filename, csv_dialect, filetype, limit, start_at,
                continue_file, select_filters, dry_run):
    """
    ...
    * anything regarding assignee and creator
    ...
    for name_regex in select_filters:
        ...
        createme = filter(lambda x: regex.search(x[1][fieldname]), createme)

    # make a list again.
    createme = list(createme)
    ...
    for name, results in (("created", successes), ("errors", errors)):
        if results:
            log_file = f"jira-{timestamp}.{name}.yaml"
