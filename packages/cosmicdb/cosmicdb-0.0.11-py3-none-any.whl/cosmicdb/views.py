...
class CosmicCreateView(CreateView):
    ...
    def form_valid(self, form):
        ...
        response = super(CosmicCreateView, self).form_valid(form)
        success_message = self.get_success_message(form.cleaned_data)
        if success_message:
            django_messages.success(self.request, success_message)
        return response

    def form_invalid(self, form):
        self.extra_context['page_error'] = True
        return super(CosmicCreateView, self).form_invalid(form)


class CosmicUpdateView(UpdateView):
    success_message = 'Saved'
    extra_context = {}
    ...
    def form_valid(self, form):
        self.extra_context['page_error'] = False
        response = super(CosmicUpdateView, self).form_valid(form)
        success_message = self.get_success_message(form.cleaned_data)
        if success_message:
            django_messages.success(self.request, success_message)
        return response
    ...
    def form_invalid(self, form):
        self.extra_context['page_error'] = True
        return super(CosmicUpdateView, self).form_invalid(form)
