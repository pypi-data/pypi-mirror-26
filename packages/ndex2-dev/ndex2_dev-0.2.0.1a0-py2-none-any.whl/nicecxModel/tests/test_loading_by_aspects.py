...
class TestLoadByAspects(unittest.TestCase):
    ...
    def test_create_from_pandas_with_headers2(self):
        ...
        with open('CTD_genes_pathways.txt', 'rU') as tsvfile:
            ...
            for k, v in niceCx.get_edges().items():
                print(k)
                print(v)
    ...
    def test_load_nodes(self):
        ...
        for i in range(1,10):
            niceCx.add_node(NodesElement(id=i, node_name='node%s' % str(i), node_represents=gene_list[i]))
    ...
    def test_load_edges(self):
        niceCx = ndex2.create_empty_nice_cx() #NiceCXNetwork()
        niceCx.add_node(NodesElement(id=1, node_name='node%s' % str(1), node_represents='ABC'))
        niceCx.add_node(NodesElement(id=2, node_name='node%s' % str(2), node_represents='DEF'))
        niceCx.add_edge(EdgesElement(id=1, edge_source=1, edge_target=2, edge_interaction='neighbor'))
    ...
    def test_pandas_loading(self):
        with open('MDA1.txt', 'rU') as tsvfile:
            ...
            for index, row in df.iterrows():
                niceCx.add_node(NodesElement(id=row['Bait'], node_name=row['Bait'], node_represents=row['Bait']))
                niceCx.add_node(NodesElement(id=row['Prey'], node_name=row['Prey'], node_represents=row['Prey']))

                niceCx.add_edge(EdgesElement(id=index, edge_source=row['Bait'], edge_target=row['Prey'], edge_interaction='interacts-with'))
    ...
    def test_manual_build(self):
        ...
        fox_node_id = niceCx.add_node(node_name='Fox')
        mouse_node_id = niceCx.add_node(node_name='Mouse')
        bird_node_id = niceCx.add_node(node_name='Bird')

        fox_bird_edge = niceCx.add_edge(edge_source=fox_node_id, edge_target=bird_node_id, edge_interaction='interacts-with')
        fox_mouse_edge = niceCx.add_edge(edge_source=fox_node_id, edge_target=mouse_node_id, edge_interaction='interacts-with')
