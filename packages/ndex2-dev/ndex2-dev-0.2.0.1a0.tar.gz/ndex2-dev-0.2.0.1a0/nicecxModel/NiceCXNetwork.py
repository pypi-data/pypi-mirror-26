...
class NiceCXNetwork(object):
    ...
    def add_node(self, node_element=None, id=None, node_name=None, node_represents=None, json_obj=None):
    ...
    def add_edge(self, edge_element=None, id=None, edge_source=None, edge_target=None, edge_interaction=None, json_obj=None):
    ...
    def add_node_attribute(self, node_attribute_element=None, i=None, subnetwork=None, property_of=None, name=None, values=None, type=None, json_obj=None):
    ...
    def add_edge_attribute(self, edge_attribute_element=None, i=None, subnetwork=None, property_of=None, name=None, values=None, type=None, json_obj=None):
    ...
    def create_from_pandas(self, df, source_field=None, target_field=None, source_node_attr=[], target_node_attr=[], edge_attr=[], edge_interaction=None):
        ...
        if source_field and target_field:
            for index, row in df.iterrows():
                ...
                # ADD NODES
                #=============
                self.add_node(id=row[source_field], node_name=row[source_field], node_represents=row[source_field])
                self.add_node(id=row[target_field], node_name=row[target_field], node_represents=row[target_field])

                #=============
                # ADD EDGES
                #=============
                self.add_edge(id=index, edge_source=row[source_field], edge_target=row[target_field], edge_interaction=row[edge_interaction])
            ...
            for index, row in df.iterrows():
                ...
                # ADD NODES
                #=============
                self.add_node(id=row[0], node_name=row[0], node_represents=row[0])
                self.add_node(id=row[1], node_name=row[1], node_represents=row[1])

                #=============
                # ADD EDGES
                #=============
                if len(row) > 2:
                    self.add_edge(id=index, edge_source=row[0], edge_target=row[1], edge_interaction=row[2])
                else:
                    self.add_edge(id=index, edge_source=row[0], edge_target=row[1], edge_interaction='interacts-with')
    ...
    def create_from_networkx(self, G):
        ...
        for n, d in G.nodes_iter(data=True):
            ...
            # ADD NODES
            #=============
            self.add_node(id=n, node_name=n, node_represents=n)
        ...
        for u, v, d in G.edges_iter(data=True):
            ...
            # ADD EDGES
            #=============
            self.add_edge(id=index, edge_source=u, edge_target=v, edge_interaction=d.get('interaction'))
        ...
        if server and uuid:
            ...
            if 'nodes' in available_aspects:
                ...
                for node_item in obj_items:
                    ...
                    self.add_node(json_obj=node_item)
                self.add_metadata_stub('nodes')
            ...
            if 'edges' in available_aspects:
                ...
                for edge_item in obj_items:
                    ...
                    self.add_edge(json_obj=edge_item)
                self.add_metadata_stub('edges')
            ...
            if e_a is not None:
                for e_a_item in e_a:
                    if type(e_a_item.getValues()) is list:
                        add_this_dict[e_a_item.get_name()] = ','.join(str(e) for e in e_a_item.getValues())
                        add_this_dict[e_a_item.get_name()] = '"' + add_this_dict[e_a_item.get_name()] + '"'
                    else:
                        add_this_dict[e_a_item.get_name()] = e_a_item.getValues()
            #================================
            ...
            if s_a is not None:
                for s_a_item in s_a:
                    if type(s_a_item.getValues()) is list:
                        add_this_dict['source_' + s_a_item.get_name()] = ','.join(str(e) for e in s_a_item.getValues())
                        add_this_dict['source_' + s_a_item.get_name()] = '"' + add_this_dict['source_' + s_a_item.get_name()] + '"'
                    else:
                        add_this_dict['source_' + s_a_item.get_name()] = s_a_item.getValues()
            ...
            if t_a is not None:
                for t_a_item in t_a:
                    if type(t_a_item.getValues()) is list:
                        add_this_dict['target_' + t_a_item.get_name()] = ','.join(str(e) for e in t_a_item.getValues())
                        add_this_dict['target_' + t_a_item.get_name()] = '"' + add_this_dict['target_' + t_a_item.get_name()] + '"'
                    else:
                        add_this_dict['target_' + t_a_item.get_name()] = t_a_item.getValues()
    ...
    def to_networkx(self):
        ...
        #============================
        for net_a in self.networkAttributes:
            G.graph[net_a.get_name()] = net_a.getValues()
        ...
        for k, v in node_items:
            ...
            if n_a:
                for na_item in n_a:
                    node_attrs[na_item.get_name()] = na_item.getValues()
                    #print(v)
                    my_name = v.get_name()
