...
def create_nice_cx_from_networkx(G):
    ...
    for n, d in G.nodes_iter(data=True):
        ...
        # ADD NODES
        #=============
        my_nicecx.add_node(id=n, node_name=n, node_represents=n)

        #======================
        # ADD NODE ATTRIBUTES
        #======================
        for k,v in d.items():
            my_nicecx.add_node_attribute(property_of=n, name=k, values=v)
    ...
    for u, v, d in G.edges_iter(data=True):
        ...
        # ADD EDGES
        #=============
        my_nicecx.add_edge(id=index, edge_source=u, edge_target=v, edge_interaction=d.get('interaction'))

        #==============================
        # ADD EDGE ATTRIBUTES
        #==============================
        for k,v in d.items():
            my_nicecx.add_edge_attribute(property_of=index, name=k, values=v)
    ...
    if cx:
        ...
        if 'nodes' in available_aspects:
            ...
            for node_item in obj_items:
                ...
                my_nicecx.add_node(json_obj=node_item)
            my_nicecx.add_metadata_stub('nodes')
        ...
        if 'edges' in available_aspects:
            ...
            for edge_item in obj_items:
                ...
                my_nicecx.add_edge(json_obj=edge_item)
            my_nicecx.add_metadata_stub('edges')
        ...
        if 'nodeAttributes' in available_aspects:
            ...
            for att in obj_items:
                ...
                my_nicecx.add_node_attribute(json_obj=att)
            my_nicecx.add_metadata_stub('nodeAttributes')
        ...
        if 'edgeAttributes' in available_aspects:
            ...
            for att in obj_items:
                ...
                my_nicecx.add_edge_attribute(json_obj=att)
            my_nicecx.add_metadata_stub('edgeAttributes')
...
def create_nice_cx_from_pandas(df, source_field=None, target_field=None, source_node_attr=[], target_node_attr=[], edge_attr=[], edge_interaction=None):
    ...
    if source_field and target_field:
        for index, row in df.iterrows():
            ...
            # ADD NODES
            #=============
            my_nicecx.add_node(id=row[source_field], node_name=row[source_field], node_represents=row[source_field])
            my_nicecx.add_node(id=row[target_field], node_name=row[target_field], node_represents=row[target_field])
            ...
            if edge_interaction:
                if row.get(edge_interaction):
                    my_nicecx.add_edge(id=index, edge_source=row[source_field], edge_target=row[target_field], edge_interaction=row[edge_interaction])
                else:
                    my_nicecx.add_edge(id=index, edge_source=row[source_field], edge_target=row[target_field], edge_interaction=edge_interaction)
            else:
                my_nicecx.add_edge(id=index, edge_source=row[source_field], edge_target=row[target_field], edge_interaction='neighbor-of')
            ...
            for sp in source_node_attr:
                ...
                if type(row[sp]) is float and math.isnan(row[sp]):
                    ...
                    row[sp] = 'Inf'
                    attr_type = ATTRIBUTE_DATA_TYPE.FLOAT
                my_nicecx.add_node_attribute(property_of=row[source_field], name=sp, values=row[sp], type=attr_type)
            ...
            for tp in target_node_attr:
                ...
                if type(row[tp]) is float and math.isnan(row[tp]):
                    ...
                    row[tp] = 'Inf'
                    attr_type = ATTRIBUTE_DATA_TYPE.FLOAT
                my_nicecx.add_node_attribute(property_of=row[target_field], name=tp, values=row[tp], type=attr_type)
            ...
            for ep in edge_attr:
                ...
                my_nicecx.add_edge_attribute(property_of=index, name=ep, values=row[ep], type=attr_type)
        ...
        for index, row in df.iterrows():
            ...
            # ADD NODES
            #=============
            my_nicecx.add_node(id=row[0], node_name=row[0], node_represents=row[0])
            my_nicecx.add_node(id=row[1], node_name=row[1], node_represents=row[1])

            #=============
            # ADD EDGES
            #=============
            if len(row) > 2:
                my_nicecx.add_edge(id=index, edge_source=row[0], edge_target=row[1], edge_interaction=row[2])
            else:
                my_nicecx.add_edge(id=index, edge_source=row[0], edge_target=row[1], edge_interaction='interacts-with')
...
def create_nice_cx_from_server(server=None, username=None, password=None, uuid=None):
    if server and uuid:
        ...
        #===================
        available_aspects = []
        for ae in (o for o in my_nicecx.stream_aspect(uuid, 'metaData', server, username, password)):
            available_aspects.append(ae.get(CX_CONSTANTS.METADATA_NAME))
            mde = MetaDataElement(json_obj=ae)
            my_nicecx.add_metadata(mde)
        ...
        #====================
        if 'networkAttributes' in available_aspects:
            objects = my_nicecx.stream_aspect(uuid, 'networkAttributes', server, username, password)
            obj_items = (o for o in objects)
            for network_item in obj_items:
                #add_this_network_attribute = NetworkAttributesElement(json_obj=network_item)
        ...
        #===================
        if 'nodes' in available_aspects:
            objects = my_nicecx.stream_aspect(uuid, 'nodes', server, username, password)
            obj_items = (o for o in objects)
            for node_item in obj_items:
                #add_this_node = NodesElement(json_obj=node_item)
        ...
        #===================
        if 'edges' in available_aspects:
            objects = my_nicecx.stream_aspect(uuid, 'edges', server, username, password)
            obj_items = (o for o in objects)
            for edge_item in obj_items:
                #add_this_edge = EdgesElement(json_obj=edge_item)
        ...
        #===================
        if 'nodeAttributes' in available_aspects:
            objects = my_nicecx.stream_aspect(uuid, 'nodeAttributes', server, username, password)
            obj_items = (o for o in objects)
            for att in obj_items:
                #add_this_node_att = NodeAttributesElement(json_obj=att)
        ...
        #===================
        if 'edgeAttributes' in available_aspects:
            objects = my_nicecx.stream_aspect(uuid, 'edgeAttributes', server, username, password)
            obj_items = (o for o in objects)
            for att in obj_items:
                #add_this_edge_att = EdgeAttributesElement(json_obj=att)
        ...
        #===================
        if 'citations' in available_aspects:
            objects = my_nicecx.stream_aspect(uuid, 'citations', server, username, password)
            obj_items = (o for o in objects)
            for cit in obj_items:
                add_this_citation = CitationElement(json_obj=cit)
        ...
        #===================
        if 'supports' in available_aspects:
            objects = my_nicecx.stream_aspect(uuid, 'supports', server, username, password)
            obj_items = (o for o in objects)
            for sup in obj_items:
                add_this_supports = SupportElement(json_obj=sup)
        ...
        #===================
        if 'edgeSupports' in available_aspects:
            objects = my_nicecx.stream_aspect(uuid, 'edgeSupports', server, username, password)
            obj_items = (o for o in objects)
            for add_this_edge_sup in obj_items:
                my_nicecx.add_edge_supports(add_this_edge_sup)
        ...
        #===================
        if 'nodeCitations' in available_aspects:
            objects = my_nicecx.stream_aspect(uuid, 'nodeCitations', server, username, password)
            obj_items = (o for o in objects)
            for node_cit in obj_items:
                my_nicecx.add_node_citations_from_cx(node_cit)
            my_nicecx.add_metadata_stub('nodeCitations')
        ...
        #===================
        if 'edgeCitations' in available_aspects:
            objects = my_nicecx.stream_aspect(uuid, 'edgeCitations', server, username, password)
            obj_items = (o for o in objects)
            for edge_cit in obj_items:
                my_nicecx.add_edge_citations_from_cx(edge_cit)
            my_nicecx.add_metadata_stub('edgeCitations')
        ...
        #===================
        for oa in opaque_aspects:
            objects = my_nicecx.stream_aspect(uuid, oa, server, username, password)
            obj_items = (o for o in objects)
            for oa_item in obj_items:
                aspect_element = AspectElement(oa_item, oa)
                my_nicecx.add_opaque_aspect(aspect_element)
