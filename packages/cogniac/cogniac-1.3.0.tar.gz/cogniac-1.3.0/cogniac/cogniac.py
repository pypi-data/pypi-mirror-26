...
class CogniacConnection(object):
    ...
    def __init__(self, username=None, password=None, tenant=None, timeout=20):
        """
        ...
        tenant (String):              Cogniac tenant_id OR 'tenant name' with which to assume credentials.
                                      This is only required if the user is a member of multiple tenants.
                                      If tenant is None, and the user is a member of multiple tenants
                                      then use the contents of the COG_TENANT environment variable
                                      will be used as the tenant.
                                      Note that the 'tenant name' field can be changed by tenant admins
                                      so it is more reliable to use a tenant_id here since tenant_id is immutable.
            ...
            try:
                ...
                if len(tenants) > 1:
                    ...
                    for tenant in tenants:
                        print "%20s (%s)    export COG_TENANT='%s'" % (tenant['tenant_name'], tenant['tenant_id'], tenant['tenant_id'])
                    print
    ...
    def get_all_subjects(self):
        ...
        """
        return CogniacSubject.get_all(self)
