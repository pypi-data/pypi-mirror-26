    ...
    def is_valid_ntf_folder(cls, path):
        ...
        for item_name in os.listdir(path):
            if not (item_name.startswith(".") or item_name in ("assets", "variables", "saved_model.pb")):
                raise cls.ValidationError("NTF model can only contain 'assets' and 'variables' folder")
            if item_name in ("assets", "variables", "saved_model.pb"):
                if not os.path.isdir(item_name):
                    raise cls.ValidationError("{item_name} must be a folder".format(item_name=item_name))
