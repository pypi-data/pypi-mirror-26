...
class TFBaseModel(BaseModel):
    ...
    def __init__(self, run_mode="local", hostport=None, model_name=None, timeout_seconds=5, saved_model_path=None, local_debug=False, **kwargs):
        ...
        if run_mode == "remote":
            ...
            self._prepare_local(saved_model_path, local_debug)
    ...
    def _prepare_local(self, saved_model_path, local_debug):
        saved_model_path = saved_model_path or modelhub.apis.get_model_path(self.model_name)
    ...
    def preprocess(self, input_item):
        """
        input_item -> user input, suggest format like this{
            "input1:value,
            "input2":value
        }
        return:
        ...
        """
        return self._pack_data(input_item)

    def postprocess(self, tf_output, input_item, preprocessed_item):
        """
        tf_output -> {
            "output1":[value],
            "output2":[value]
        }
        input_item -> one item of user input data
        preprocessed_item-> {
            "input1":[value],
            "input2":[value]
        }
