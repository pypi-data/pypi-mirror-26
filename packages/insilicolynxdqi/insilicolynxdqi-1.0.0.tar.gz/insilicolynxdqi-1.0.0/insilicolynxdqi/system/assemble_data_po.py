...
description = "The insilicolynxdqi.system folder assemble_data_po.py module"

class AssembleDataPo:
	# Class variables


	# __init__
	def __init__(self, settings, dataFile, prefix, columnHeaderSuffixText, option):
		try:
			# Inputs
			self.ColumnHeaderSuffixText = columnHeaderSuffixText	
	...
	def __str__(self):
		try:
			# Return output
			return "The insilicolynxdqi AssembleDataPo class"
	...
	def assembleData(self, dictData):
		try:
			...
			with fileToRead:
				...
				for row in reader:
					...
					if str(row[self.Name].split(self.NameSubstring)[0]) + str(self.NameSubstring) + str(row[self.GroupReference]) in nameOrder:
						# Compound already got some processed data associated to it - append new data to the stored list of lists within the assembledData dictionary
						cpd = assembledData[str(row[self.Name].split(self.NameSubstring)[0]) + str(self.NameSubstring) + str(row[self.GroupReference])]
						for index, item in enumerate(self.ColumnHeaderSuffixText):	
							if self.isFloat(row[self.Prefix + self.XAxisTitleSubString + self.Species + item]) == True and self.isFloat(row[self.Prefix + self.YAxisTitleSubString + self.Species + item]) == True:
								cpd.append([float(row[self.Prefix + self.XAxisTitleSubString + self.Species + item]), float(row[self.Prefix + self.YAxisTitleSubString + self.Species + item])])
						
						assembledData.update({str(row[self.Name].split(self.NameSubstring)[0]) + str(self.NameSubstring) + str(row[self.GroupReference]):cpd})
						...
						for index, item in enumerate(self.ColumnHeaderSuffixText):
							if self.isFloat(row[self.Prefix + self.XAxisTitleSubString + self.Species + item]) == True and self.isFloat(row[self.Prefix + self.YAxisTitleSubString + self.Species + item]) == True:
								# Cpd becomes a list of lists
								cpd.append([float(row[self.Prefix + self.XAxisTitleSubString + self.Species + item]), float(row[self.Prefix + self.YAxisTitleSubString + self.Species + item])])	
						
						assembledData.update({str(row[self.Name]) + str(self.NameSubstring) + str(row[self.GroupReference]):cpd})
			...
			return dictData, smilesExist, assembledData, nameOrder
