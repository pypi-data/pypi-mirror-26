...
description = "The insilicolynxdqi.calculations folder iv_2c_total.py module"
...
class Iv2CTotal:
	...
	def __str__(self):
		try:
			# Return output
			return "The insilicolynxdqi Iv2CTotal class"
				...
				for row in reader:
					...
					for index in range(0, 5):
						...
						for dose in settings["dose"]:
							...
							# Calculate levels
							amountBVector, amountCVector, amountBCVector = self.iv2CTotal(doseTimes, timeVector, dose, k2Rates[index], k3Rates[index], k4Rates[index], alpha, beta)			
							
							# Calculate AUC during a chosen time interval	
							aucAmountB, aucAmountC = self.iv2CTotalAUC(timesAUC, dose, k2Rates[index], k3Rates[index], k4Rates[index], alpha, beta, settings["numberOfSegments"])
							...
							maxAmountBValues.update({dose:maxAmountBValue})
							minAmountBValues.update({dose:minAmountBValue})
							if maxAmountBValue != 'nan' and minAmountBValue != 'nan':
								midAmountBValues.update({dose:float(math.pow(10, ((math.log10(float(maxAmountBValue)) + math.log10(float(minAmountBValue))) / 2)))})
							else:
								midAmountBValues.update({dose:'nan'})
							aucAmountBValues.update({dose:aucAmountB})
							
							maxConcentrationBValues.update({dose:maxConcentrationBValue})
							minConcentrationBValues.update({dose:minConcentrationBValue})
							if maxConcentrationBValue != 'nan' and minConcentrationBValue != 'nan':
								midConcentrationBValues.update({dose:float(math.pow(10, ((math.log10(float(maxConcentrationBValue)) + math.log10(float(minConcentrationBValue))) / 2)))})
							else:
								midConcentrationBValues.update({dose:'nan'})
							aucConcentrationBValues.update({dose:float(aucAmountB) / float(vcentralList[index])})
							
							maxAmountCValues.update({dose:maxAmountCValue})
							minAmountCValues.update({dose:minAmountCValue})				
							if maxAmountCValue != 'nan' and minAmountCValue != 'nan':
								midAmountCValues.update({dose:float(math.pow(10, ((math.log10(float(maxAmountCValue)) + math.log10(float(minAmountCValue))) / 2)))})
							else:
								midAmountCValues.update({dose:'nan'})
							aucAmountCValues.update({dose:aucAmountC})
							
							maxAmountBCValues.update({dose:maxAmountBCValue})
							minAmountBCValues.update({dose:minAmountBCValue})
							if maxAmountBCValue != 'nan' and minAmountBCValue != 'nan':
								midAmountBCValues.update({dose:float(math.pow(10, ((math.log10(float(maxAmountBCValue)) + math.log10(float(minAmountBCValue))) / 2)))})
							else:
								midAmountBCValues.update({dose:'nan'})
							aucAmountBCValues.update({dose:aucAmountB + aucAmountC})
						...
						if settings["compartment"] == "central":
							...
							# Perform analyse using "dose" ~ "amount_C_max" and "amount_C_min" data and return dictionary
							extrapolatedData = analysis.analyseDoseQuantityDictionaries(extrapolatedData, maxAmountCValues, minAmountCValues, settings["amountCPrefix"], columnHeaderSuffix[index])
	...
	# Iv 2C total levels
	def iv2CTotal(self, doseTimes, timeVector, dose, k2, k3, k4, alpha, beta):    				  
		try:
			#
	...
	# Iv 2C total levels AUC
	def iv2CTotalAUC(self, timesAUC, dose, k2, k3, k4, alpha, beta, numberOfSegments):
		try:
