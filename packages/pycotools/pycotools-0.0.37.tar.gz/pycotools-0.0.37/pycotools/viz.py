...
class Parse(object):
    ...
    def parse(self):
        ...
        """
        data = None
        LOG.debug(type(self.cls_instance))
        ...
        LOG.debug('log10 parse --> {}'.format(self.log10))

        LOG.debug('data --> {}'.format(data.head()))


        if self.log10:
            if not type(self.cls_instance) == tasks.ProfileLikelihood:
                LOG.debug('log10 and not pl')
                data = numpy.log10(data)
                return data
            else:
                LOG.debug('log10 and pl')
...
class Boxplots(PlotKwargs):
    ...
    **kwargs        see :ref:`kwargs`  options
    ...
    def __init__(self, cls, **kwargs):
        ...
        LOG.debug('lgo10 --. {}.'.format(self.log10))
        self.data = self.parse(self.cls, log10=self.log10, copasi_file=self.copasi_file)

        LOG.debug(self.data.head())
