...
import falcon
import json
import a2d_diary.encode_diary as encode
import a2d_diary.create_diary as create
import cv2
import traceback
import zipfile
import sys
#from static_middleware import StaticMiddleware
from a2d_diary.static_resources_middleware import StaticResourcesMiddleware
...
class TemplateResource(object):
    def on_get(self, req, resp):
        ...
        if len(tif_files) > 0:
            ...
            resp.body = json.dumps(
                    {"template": ""})
            # raise falcon.HTTPInternalServerError(title="Encoding template not found",
            #                                      description="We did not find a tiff file in /input/2_template_to_encode/")
