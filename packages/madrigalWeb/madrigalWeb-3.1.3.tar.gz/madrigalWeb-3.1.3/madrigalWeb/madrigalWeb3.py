"""The madrigalWeb module provides access to all Madrigal data via web services.
    Porting code to Python 3 by R. Ilma
"""
...
class MadrigalData:
    ...
    def __init__(self, url):
        ...
        if type(result) != str: result = result[0]

        self.cgiurl = urlBase + result[(len(result)-len(cgiName)-18):len(result)-len(cgiName)]
    ...
    def __getSiteDict(self):
        ...
        for line in lines:
            items = self.__byte2str(line.split(b','))
            if len(items) < 4:
                continue
            site = int(items[0])
    ...
    def __byte2str(self, items):

        """
            Decode a byte object (list) into a string (list)
        """

        return list(map(lambda ii : items[ii].decode('utf-8'), range(len(items))))
    ...
    def getAllInstruments(self):
        ...
        for line in page:
            if line.find(b'Error occurred') != -1:
    ...
    def isprint(self, filename, parms, filters, user_fullname, user_email, user_affiliation, outputFile=None):
        ...
        if page.find(b'Error occurred') != -1 and format == 'ascii':
    ...
    def compareVersions(self, ver1, ver2):
        ...
        if not type(ver1) == str:
            ver1 = ver1.decode("utf-8")
        if not type(ver2) == str:
            ver2 = ver2.decode("utf-8")
        ...
        while(True):
            ...
            result = (verNum1 > verNum2) - (verNum1 < verNum2)
            if result:
                return(result)
            index += 1
...
class MadrigalInstrument:
    ...
    def __init__(self, name, code, mnemonic, latitude, longitude, altitude, category='unknown'):
        ...
        """
        try:
            result = isinstance(name, basestring)
            if not result:
...
class MadrigalExperiment:
    ...
    def _getRealExperimentUrl(self, version):
        ...
        """
        version_list = [int(item) for item in version.split(b'.')]        
        retStr = ''
        ...
        if version_list[0] == 2:
            ...
            try:
                retStr += urlparse.quote_plus(self.name)
            except:
