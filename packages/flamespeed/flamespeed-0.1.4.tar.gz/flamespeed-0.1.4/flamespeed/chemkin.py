    ...
    def parse_reaction_rate_params(self, reaction):
        ...
        for rateCoeff in reaction.find('rateCoeff'):
            if rateCoeff.tag == 'Arrhenius':
                ...
                if rateCoeff.find('b') != None:
                    raise ValueError("Cannot reference b in Arrhenius reaction coefficient.")
...
class ReactionRate(ReactionCoefficients, ReactionParser):
    ...
    def __str__(self):
        ...
        if self.num_reactions == 0:
            ...
            if self.rev.count(True) == self.num_reactions:
                rev_system = True
            elif self.rev.count(True) != 0 and self.rev.count(True) != self.num_reactions:
                rev_system = 'Some'
            else:
                rev_system = False
    ...
    def read_XML(self, path):
        ...
        for reaction in rxns.find('reactionData').findall('reaction'):
            ...
            self.rev.append(False if reaction.attrib['reversible'] == 'no' else True)
            self.rtype = reaction.attrib['type']

            # Elementary, reversible reactions
            if self.rtype == 'Elementary':
                ...
                raise NotImplementedError('Only elementary reactions implemented')
    ...
    def set_temp(self, T):
        ...
        # Calculate backward reaction rate coefficients for reversible reactions
    ...
    def get_progress_rate(self, concs):
        ...
        for i in range(self.num_reactions):
            wf[i] = self.kf[i] * np.product(np.power(concs, self.reactant_coef[:,i]))

        # Get backward component
        wb = np.zeros(self.num_reactions)
        for i in range(self.num_reactions):
            if self.rev[i]:
                wb[i] = self.kb[i] * np.product(np.power(concs, self.product_coef[:,i]))
