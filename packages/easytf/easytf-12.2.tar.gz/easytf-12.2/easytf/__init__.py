             ...
             while self.epoch < self.minimum_epoch or self.prev_val_loss > self.val_loss:
                 for i in xrange(self.num_batch):
                    for j in xrange(int(self.batch_size)):
                        ...
                        if sc==1:
                            self.batch_wav=np.pad(self.batch_wav,[int(((1+(2*self.nfes))*self.hop_size)-tf.shape(self.batch_wav)[0]),0],'constant',constant_values=[0])
                        if ec==1:
                            self.batch_wav=np.pad(self.batch_wav,[0,int(((1+(2*self.nfes))*self.hop_size)-tf.shape(self.batch_wav)[0])],'constant',constant_values=[0])
