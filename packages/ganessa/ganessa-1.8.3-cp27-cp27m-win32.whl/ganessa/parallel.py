...
def mp_worker(inputqueue, outputqueue, init, term, wait, wpnum, wpcount, use):
    ...
    # rank init and term < 0 for sorting
    if term is not None:
        TMARK = 10 if wpcount < 10 else 100 if wpcount < 100 else 1000
        func, args = term
        rank = -wpnum - TMARK
...
def mp_calculate(func, args, rank, count, wpnum, use):
    try:
        ...
        stat = result if isinstance(result, basestring) else 'Ok!'
...
#   * results = mp.run(tasks [, init=None] [, term= None])
#   * results = mp.run_sequential(tasks [, init=None] [, term= None] [, wpnum= None])
#   * ncpu = mp.nmaxcpu
# ARGUMENT
...
#     When init task is defined, its results appear on top of the list (results[0:ncpu]).
#     term results are discarded.
...
class MultiProc(object):
    ...
    def run(self, tasks, init= None, term= None):
        ...
        processes = [Process(target=mp_worker, 
                             args=(task_queue, done_queue, tuple(init), term, 
        ...
        # sort results by initial task rank
        results.sort()
        logprint(mpf + snow())
        # place termination results at the end
        n = 0 if term is None else self.ncpu
        # returns results only, not rank
        return [res for (_rank, res) in results[n:] + results[0:n]]
    ...
    def run_sequential(self, tasks, init= None, term= None, wpnum=0):
        ...
        itasks = iter(tasks)
        if init is not None:
            itasks = chain([init], itasks)
        if term is not None:
            tasks = chain(itasks, [term])
        for func, args in itasks:
            if self.use_wpnum:
