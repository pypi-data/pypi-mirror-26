...
class FileRabbitConfiguration(BaseRabbitConfiguration):
    ...
    def __init__(self, config_object, result=False, status=False):
        if not isinstance(config_object, (configuration.DeployvConfig, BasicMessage)):
        ...
        values = (
            ...
            status and ('rmq_status_stat_queue', 'rmq_status_stat_topic') or
            ('rmq_task_queue', 'rmq_task_topic'))
        queue = self.config.get('rmq', values[0])
        route = self.config.get('rmq', values[1])
