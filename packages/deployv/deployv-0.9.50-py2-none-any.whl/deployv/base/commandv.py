...
class CommandV(Extensible):
    ...
    def deactivate_backup(self, db_config, backup, prefix, store_path, cformat, admin_password,
        ...
        jobs = self.__config.get('container_config').get('jobs')
        ...
        dump_path = 'database_dump.sql' if not jobs else 'database_dump'
        dump_path = os.path.join(dest_dir, dump_path)
        utils.clean_files(dump_path)
        postgres.dump(db_name, dump_path, jobs=jobs)
        backup_name = utils.generate_backup_name(db_name)
        files2backup = []
        files = os.listdir(dest_dir)
        for element in files:
            if any([no_fs and element == "filestore", jobs and element == 'database_dump.sql',
                    not jobs and element == 'database_dump']):
                continue
    ...
    def push_image(self):
        ...
        for line in self.instance_manager.cli.push(image.get('RepoTags')[0], stream=True):
            obj = json.loads(utils.decode(line))
            if obj.get('status'):
                logger.debug(obj)
                response = response + '\n' + utils.decode(line)
            elif obj.get('error'):
        ...
        res.get('attachments').append(
            {
                ...
                "file": utils.decode(base64.b64encode(utils.encode(response))),
                "type": 'text/plain'
            }
        )
