...
def __main():
    ...
    mode_set = __query_set_mode()
    mode_procedure = __query_procedure()
    mode_user = __query_install_mode( mode_procedure )
    
    if mode_set == SET_LOCALS:
        from_pypi.clear()
    
    if mode_user == IMODE_USER:
        from_pypi.extend( from_bitbucket )
        from_bitbucket = []
    
    if mode_user == IMODE_DEVELOPER:
        user_name = __query_user_name()
        dir_name = __query_target_directory( mode_procedure )
    else:
        user_name = None
        dir_name = None
    
    __query_summary( dir_name, from_bitbucket, from_pypi, user_name, mode_procedure, mode_user )
    
    __install_git( from_pypi, mode_procedure, python_interpreter )
    
    __install_bitbucket( dir_name, from_bitbucket, mode_procedure, python_interpreter, user_name )
...
def __install_bitbucket( dir_name, from_bitbucket, mode, python_interpreter, user_name ):
    ...
    for VARIABLE in from_bitbucket:
        ...
        if mode == MODE_INSTALL:
            pass
        elif mode == MODE_UPGRADE or mode == MODE_UNINSTALL:
...
def __query_colour_support():
    ...
    if colorterm is not None:
        __print_line( "\033[38;2;" + str( 0 ) + ";" + str( 255 ) + ";" + str( 128 ) + "m" + "NO ERROR" + BOX )
    else:
        __print_line( "\033[38;2;" + str( 255 ) + ";" + str( 0 ) + ";" + str( 0 ) + "m" + "24 BIT RED" + BOX )
        __print_line( "\033[31m" + "SYSTEM RED" + BOX )
        __print_line( "CHECK FAILED: YOU ARE NOT USING A TRUE COLOUR TERMINAL OR TRUE COLOUR IS MISCONFIGURED." )
        __print_line( "PLEASE SWITCH A TRUE COLOUR TERMINAL OR ENABLE 'COLORTERM'." )
        __print_line( "IF YOU ARE USING SSH THIS ASSERTION MAY BE IN ERROR'." )
        __print_middle()
...
def __query_install_mode( mode_procedure ):
    __print_top()
    
    if mode_procedure == MODE_INSTALL:
...
def __check_python_interpreter( python_interpreter ):
    ...
    try:
        ...
        __print_line( "CHECK FAILED - BAD INTERPRETER" )
        __print_bottom()
        raise BadConfigError()
    if int( output[0] ) != 3 or int( output[1] ) < 6:
        __print_line( "CHECK FAILED. THE «{}» COMMAND IS BOUND TO PYTHON VERSION «{}».«{}»".format( python_interpreter, output[0], output[1] ) )
        __print_line( "THIS APPLICATION REQUIRES VERSION 3.6." )
        __print_bottom()
        raise BadConfigError()
    
    __print_line( "«{}» IS PYTHON VERSION «{}».«{}»".format( python_interpreter, output[0], output[1] ) )
    __print_bottom()
...
def __print_line( text ):
    ptext = text.replace( "«", "" ).replace( "»", "" )
    ptext = re.sub( "\x1b\[[0-9;]*m", "", ptext )
    pad = 78 - len( ptext )
