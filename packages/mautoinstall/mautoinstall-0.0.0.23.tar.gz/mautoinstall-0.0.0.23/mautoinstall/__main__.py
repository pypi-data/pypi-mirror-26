...
def __query_summary( dir_name, from_bitbucket, from_pypi, user_name, mode, imode ):
    ...
    __print_line( "GIT-ONLY DEVELOPER MODE:              " + str( imode == IMODE_GIT_ONLY ) )
    __print_middle()
...
def __query_set_mode(  ):
    ...
    __print_top()
    __print_line( "WHAT SET DO YOU WISH TO MODIFY: «A»LL | «D»EPENDENCIES | «L»OCALS" )
...
def __print_line( text ):
    pad = 78 - len( text.replace( "«", "" ).replace( "»", "" ) )
