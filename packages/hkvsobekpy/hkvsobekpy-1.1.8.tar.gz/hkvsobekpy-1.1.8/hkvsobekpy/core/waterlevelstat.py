...
class __waterlevelstat_class(object):
    ...
    def PlotFiguur(self,out_folder='none'):
        """
        plot figuur
        ...
        S3_Tarray_sel_jaar = self.stats.stap3.Tarray_sel_jaar
        S3_WSarray_sel_jaar = self.stats.stap3.WSarray_sel_jaar
        TOI3 = self.stats.stap3.TOI
        S3_WSarray_TOI_jaar = self.stats.stap3.WSarray_TOI_jaar          
    ...
    def EnsembleRunner(self, out_folder, his_file, shp_file, shp_key='nodeID', parameter='auto',
        ...
        print ('read his-file')
        # create frame table
        his_reader.LeesMetadata(his_file)
        self._initTabellen(shp_key)
        print ('read shp-file')
        locations_df = gpd.read_file(shp_file)
        
        if parameter=='auto':
            parameter = his_reader.hisFile.variabeleInfo.variabelen[0]
        print ('create png-file')
        # iterate locationlist and create item in table
        pbar = tqdm(locations_df[shp_key])
        for location in pbar:
            pbar.set_description("location %s" % location.ljust(21))
            df_enkel = his_reader.EnkeleWaardenArray(location, parameter, startMMdd=(1, 1), endMMdd=(12, 31), jaarmax_as='date')
            if df_enkel.size > his_reader.hisFile.tijdstapInfo.N:
                #set_trace()
                self.__errors__.gebeurtenissenError()
            self.AfleidingParameters(df_enkel, his_reader.hisFile.tijdstapInfo.N, vensterArray, GumbelT, TOI, startMMdd, endMMdd, jaarmax_as='date')
            if draw_plot == True:
                self.PlotFiguur(out_folder)
            if write_table ==True:
                self.SchrijfTabellen(shp_key=shp_key, init_tabel=False)
        print ('save csv-file')
        ...
        locations_df = locations_df.set_index(shp_key).join(self.stats.df_table2.set_index(shp_key)).reset_index()
        locations_df.to_file(os.path.join(out_folder,'waterstandstatistiek_output_2.shp'), driver='ESRI Shapefile')
        self.stats.df_table2 = locations_df
        print ('done')

    def _initTabellen(self,shp_key):
        """
        ...
        self.stats.df_table2 = pd.DataFrame(columns=[shp_key,'T10_LANDELIJK','T10_STEDELIJK',
                                                     'T25','T50','T100','r^2','a','b'])        
        
      
    def SchrijfTabellen(self,shp_key,decimals=3, init_tabel=True):
        """
        ...
        try:
            table2_list.append({shp_key:self.stats.stap1.ID,
                                'T10_LANDELIJK':self.stats.df_table1[(self.stats.df_table1['T'] == 10) & 
                                                (self.stats.df_table1['methode'] == 'gewogen gemiddelde') & 
                                                (self.stats.df_table1['seizoen'] == 'alleen zomer') &
                                                (self.stats.df_table1[shp_key] == self.stats.stap1.ID)]['H'].values[0],
                                'T10_STEDELIJK':self.stats.df_table1[(self.stats.df_table1['T'] == 10) & 
                                                (self.stats.df_table1['methode'] == 'gewogen gemiddelde') & 
        ...
        if init_tabel==True:
            return self.stats.df_table1, self.stats.df_table2
        else:        
            return 