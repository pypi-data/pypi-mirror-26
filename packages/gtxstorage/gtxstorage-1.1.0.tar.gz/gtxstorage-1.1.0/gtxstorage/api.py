# -*- coding: utf-8 -*-
# GTXStorage Python Library for Amazon S3 Compatible Cloud Storage, (C)
# 2015, 2016, 2017 GTXStorage, Inc.
#
...
# GTXStorage (OS; ARCH) LIB/VER APP/VER .
_DEFAULT_USER_AGENT = 'GTXStorage {0} {1}'.format(
...
class GTXStorage(object):
    """
    Constructs a :class:`GTXStorage <GTXStorage>`.

    Examples:
        client = GTXStorage('play.gtxstorage.io:9000')
        client = GTXStorage('s3.amazonaws.com', 'ACCESS_KEY', 'SECRET_KEY')

        # To override auto bucket location discovery.
        client = GTXStorage('play.gtxstorage.io:9000', 'ACCESS_KEY', 'SECRET_KEY',
                       region='us-east-1')
    ...
    :return: :class:`GTXStorage <GTXStorage>` object
    """
    ...
    def make_bucket(self, bucket_name, location='us-east-1'):
        ...
        Examples:
            gtxstorage.make_bucket('foo')
            gtxstorage.make_bucket('foo', 'us-west-1')
    ...
    def list_buckets(self):
        ...
        Example:
            bucket_list = gtxstorage.list_buckets()
    ...
    def bucket_exists(self, bucket_name):
        ...
        # If the bucket has not been created yet, GTXStorage will return a "NoSuchBucket" error.
    ...
    def listen_bucket_notification(self, bucket_name, prefix='', suffix='',
        ...
        if url_components.hostname == 's3.amazonaws.com':
            raise InvalidArgumentError(
                'Listening for event notifications on a bucket is a GTXStorage '
    ...
    def fput_object(self, bucket_name, object_name, file_path,
        ...
        Examples:
            gtxstorage.fput_object('foo', 'bar', 'filepath', 'text/plain')
    ...
    def fget_object(self, bucket_name, object_name, file_path, request_headers=None):
        ...
        Examples:
            gtxstorage.fget_object('foo', 'bar', 'localfile')
        ...
        # Write to a temporary file "file_path.part.gtxstorage" before saving.
        file_part_path = file_path + stat.etag + '.part.gtxstorage'
    ...
    def get_object(self, bucket_name, object_name, request_headers=None):
        ...
        Examples:
            my_object = gtxstorage.get_partial_object('foo', 'bar')
    ...
    def get_partial_object(self, bucket_name, object_name, offset=0, length=0, request_headers=None):
        ...
        Examples:
            partial_object = gtxstorage.get_partial_object('foo', 'bar', 2, 4)
    ...
    def put_object(self, bucket_name, object_name, data, length,
        ...
        Examples:
         ...
         with open('hello.txt', 'rb') as data:
             gtxstorage.put_object('foo', 'bar', data, file_stat.size, 'text/plain')
    ...
    def list_objects(self, bucket_name, prefix=None, recursive=False):
        ...
        Examples:
            objects = gtxstorage.list_objects('foo')
            ...
            objects = gtxstorage.list_objects('foo', prefix='hello/')
            for current_object in objects:
                print(current_object)
            # hello/world/

            objects = gtxstorage.list_objects('foo', recursive=True)
            for current_object in objects:
                print(current_object)
            # hello/world/1
            # world/world/2
    ...
    def list_objects_v2(self, bucket_name, prefix=None, recursive=False):
        ...
        Examples:
            objects = gtxstorage.list_objects_v2('foo')
            for current_object in objects:
                print(current_object)
            # hello
            # hello/
            ...
            objects = gtxstorage.list_objects_v2('foo', prefix='hello/')
            for current_object in objects:
                print(current_object)
            # hello/world/

            objects = gtxstorage.list_objects_v2('foo', recursive=True)
            for current_object in objects:
                print(current_object)
            # hello/world/1
            # world/world/2
    ...
    def list_incomplete_uploads(self, bucket_name, prefix=None,
        ...
        Examples:
            incomplete_uploads = gtxstorage.list_incomplete_uploads('foo')
