...
def load_fields(basename = 'mesh_only_140129',slicename = 'xz',fieldname = 'poynting',has_mesh = True):
    ...
    if has_mesh:
        try:
            ...
            raise ValueError("Error loading the mesh -- possible that mesh simply doesn't exist for this cross-section -- try setting 'has_mesh' to False")
        temp[~isfinite(temp)] = -1
        model_data['mesh'] = temp
...
def contour_power(S_data, model_data,
    ...
    residual : bool
        ...
        The residual is the Gaussian approximation minus the HFSS field.
    z_shift : double
    ...
    z_limit : double
        The lower limit on z in the plots.
