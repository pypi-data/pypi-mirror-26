...
'''
    ...
    if sys.argv[1] == 'EVINCE':
        ...
        def source_view_handler(input_file, source_link, timestamp):
            ...
            print 'yes, inside gvim'
            print 'gvim --servername "' + gvim_server_name + '" --remote +' + str(source_link[0]) + ' ' + input_file
            os.system('gvim --servername "' + gvim_server_name + '" --remote +"' + str(source_link[0]) + '<Enter>zO" ' + input_file)
            #os.system('gvim --servername "' + gvim_server_name + '" --remote-send "<Esc>zO"')
