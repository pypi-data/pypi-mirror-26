...
from serial.errors import VersionError
        ...
        for n, p in tuple((im or cm).properties.items()):
            if p.versions is not None:
                ...
                if specification_match and (not version_match):
                    ...
                    v = getattr(data, n)
                    if v is not None:
                        tn = type(data).__name__
                        mn = type(data).__module__
                        if mn not in ('builtins', '__builtin__', '__main__'):
                            tn = mn + '.' + tn
                        raise VersionError(
...
class Object(object):
    ...
    _hooks = None  # type: Optional[serial.hooks.Hooks]
...
class Array(list):
    ...
    def __init__(
        ...
        if item_types is None:
            if isinstance(items, Array) and (items.item_types is not None):
                item_types = copy(items.item_types)
    ...
    def __deepcopy__(self, memo=None):
        ...
        return self.__class__(
            (deepcopy(i, memo=memo) for i in self),
    ...
    def _marshal(self):
        ...
        a = tuple(
            marshal(i, types=a.item_types) for i in a
        )
        if (h is not None) and (h.after_marshal is not None):
            a = h.after_marshal(a)
        return a
    ...
    def _validate(self):
        ...
        if a.item_types is not None:
            for i in a:
                validate(i, a.item_types)
        if (h is not None) and (h.after_validate is not None):
            h.after_validate(a)
...
class Dictionary(OrderedDict):
    ...
    def __init__(
        ...
        if value_types is None:
            if isinstance(items, Dictionary) and (items.value_types is not None):
                value_types = copy(items.value_types)
    ...
    def _validate(self):
        ...
        if d.value_types is not None:
            for k, v in d.items():
                validate(v, d.value_types)
        if (h is not None) and (h.after_validate is not None):
            h.after_validate(d)
