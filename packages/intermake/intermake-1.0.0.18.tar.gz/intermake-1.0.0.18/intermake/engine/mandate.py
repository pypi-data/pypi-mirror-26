from typing import Optional, Tuple, Dict, Sequence, Iterable, TypeVar, List, Union, IO
...
from intermake.engine.progress_reporter import ProgressReporter, ActionHandler
...
class Mandate:
    ...
    def enumerate( self, iterable, title, count = None, interesting = None ):
        """
        ...
        """
        return enumerate( self.iterate( iterable, title, count, interesting ) )
    
    
    def iterate( self, iterable: Union[List[T], Tuple[T, ...], Sequence[T], IO, Iterable[T]], title, count = None, interesting = None ) -> Iterable[T]:
        """
        ...
        :param interesting:     If set the progress counter displays the string representation of the current iteration using this function.
        ...
        with self.action( title, count, interesting ) as action:
            for x in iterable:
                if count_is_file:
