...
class _AutoStore:
    ...
    def bind( self, key: str, value: object ):
        """
        ...
        return SimpleProxy( source = (lambda x: lambda: x)( value ), watch = self.__proxy_changed )
...
class LocalData:
    """
    Manages $(APP_DISPLAY_NAME)'s primary working directory, usually "~/$(APP_INTERNAL_NAME)-data" (UNIX) or "%user%/$(APP_INTERNAL_NAME)-data" (Windows).
    """
    ...
    def default_workspace( cls ) -> str:
        ...
        abv = MENV.abv_name.lower()
        
        if "untitled" in abv.lower():
            raise ValueError( "Attempt to the obtain default workspace without setting the application name." )
        
        return path.join( "~", ".intermake-data", abv )
