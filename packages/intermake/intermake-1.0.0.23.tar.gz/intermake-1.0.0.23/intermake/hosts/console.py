...
_COLOUR_PROGRESS_SPACE_RIGHT = Style.RESET_ALL + Back.WHITE + Fore.LIGHTBLACK_EX
...
class ConsoleHost( PluginHost, IProgressReceiver ):  # this host is always single-threaded so it acts as its own progress receiver
    ...
    def call_virtual_run( self, plugin: Plugin, args: ArgsKwargs, callback: Optional[Callable[[AsyncResult], None]], sync: bool ) -> Optional[object]:
        ...
        * Raises exceptions in case of error, since everything is verbatim
        ...
        title = plugin.name
        progress = ProgressReporter( self, title, self.__configuration.cluster_index, self.__configuration.cluster_count, 0.1 )
    ...
    def __draw_progress_bar( self, info: UpdateInfo ):
        ...
        if info.num_threads != 1:
            cell_thread = m_print_helper.ljust( str( info.thread_index ), " ", WIDTH_THREAD ) + _CHAR_COLUMN_SEPARATOR
        else:
            cell_thread = ""
        
        # TITLE COLUMN
        col_title_text = "▸" * (len( info.depth ) - 1) + info.text
        cell_title = Fore.BLACK + Back.LIGHTGREEN_EX + m_print_helper.ljust( col_title_text, " ", WIDTH_TITLE ) + " "
        
        # CURRENT COLUMN
        cell_progress = Fore.BLACK + Back.LIGHTBLUE_EX + " " + m_print_helper.ljust( info.format_progress(), " ", WIDTH_CURRENT ) + " "
        
        _colour_progress_left = _COLOUR_PROGRESS_SPACE_LEFT_ARRAY[len( info.depth ) % len( _COLOUR_PROGRESS_SPACE_LEFT_ARRAY )]
        ...
        if info.max > 0:
            ...
            if 0 <= val < WIDTH_BAR:
                # Normal bar
                cell_bar = (_COLOUR_PROGRESS_SIDE
                            + _CHAR_PROGRESS_SIDE_LEFT
                            + _colour_progress_left
                            + m_print_helper.ljust( info.value, _CHAR_PROGRESS_SPACE_LEFT, val )
                            + _COLOUR_PROGRESS_POINT
                            + _CHAR_PROGRESS_POINT
                            + _COLOUR_PROGRESS_SPACE_RIGHT
                            + m_print_helper.rjust( info.max - info.value, _CHAR_PROGRESS_SPACE_RIGHT, (WIDTH_BAR - val) )
                            + _COLOUR_PROGRESS_SIDE
                            + _CHAR_PROGRESS_SIDE_RIGHT
                            + Style.RESET_ALL)
            elif val == WIDTH_BAR:
                # Full "done" bar
                cell_bar = (Fore.BLACK + Back.LIGHTBLUE_EX + ("▒" * (WIDTH_BAR + 3)))
            else:
                # Overfull bar
                val = WIDTH_BAR // 2
                cell_bar = (_COLOUR_PROGRESS_SIDE
                            + _CHAR_PROGRESS_SIDE_LEFT
                            + _colour_progress_left
                            + m_print_helper.ljust( "?", "?", val )
                            + _COLOUR_PROGRESS_POINT
                            + "?"
                            + _COLOUR_PROGRESS_SPACE_RIGHT
                            + m_print_helper.rjust( "?", "?", (WIDTH_BAR - val) )
                            + _COLOUR_PROGRESS_SIDE
                            + _CHAR_PROGRESS_SIDE_RIGHT
                            + Style.RESET_ALL)
        else:
            ...
            cell_bar = (_COLOUR_PROGRESS_SIDE
                        + _CHAR_PROGRESS_SIDE_LEFT
                        + _COLOUR_PROGRESS_SPACE_RIGHT
                        + (_CHAR_PROGRESS_SPACE_LEFT * self._gval)
                        + _COLOUR_PROGRESS_POINT
                        + _CHAR_PROGRESS_POINT_BOUNCY
                        + _COLOUR_PROGRESS_SPACE_RIGHT
                        + (_CHAR_PROGRESS_SPACE_LEFT * (WIDTH_BAR_BOUNCY - self._gval))
                        + _COLOUR_PROGRESS_SIDE
                        + _CHAR_PROGRESS_SIDE_RIGHT
                        + Style.RESET_ALL)
            
            self._gval += self._dgval
            
            if self._gval == WIDTH_BAR_BOUNCY or self._gval == 0:
                self._dgval = -self._dgval
        ...
        cell_time = Fore.BLACK + Back.LIGHTBLUE_EX + Style.DIM + m_print_helper.ljust( time_remaining_text, " ", WIDTH_TIME ) + Style.RESET_ALL
        
        # FORMAT EVERYTHING
        text = constants.INFOLINE_PROGRESS + cell_thread + cell_title + _CHAR_COLUMN_SEPARATOR + cell_progress + _CHAR_COLUMN_SEPARATOR + cell_bar + _CHAR_COLUMN_SEPARATOR + cell_time + Style.RESET_ALL + "  "
