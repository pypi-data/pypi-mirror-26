...
class __Environment:
    ...
    def unlock( self, name: Optional[str] ) -> bool:
        ...
        if name:
            if name.lower() not in (self.name.lower(), self.abv_name.lower()):
                warnings.warn( "Unlock failed, name «{}» is not «{}» or «{}».".format( name, self.name, self.abv_name ) )
                return False
    ...
    def name( self, value: str ):
        ...
        if self.__name != constants.DEFAULT_NAME:
            warnings.warn( "A new package is attempting to modify the environment's «{}» («{}»), but the environment has already been configured by «{}». Please see the 'Troubleshooting: locked environment' section of Intermake's readme for details.".format( property_name, property_value, self.name ) )
            exit( 1 )
